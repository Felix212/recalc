...
f_log2csv(0,2,"[of_retrieve_dsjobs_new] Delete "+string(sqlca.sqlnrows)+" old Jobs with dstop_computing is null and nprocess_status > 4: "+string(sqlca.sqlcode) )
...
if SQLCA_LOG.SQLCODE <> 0 then
	guoLog.uf_allways("["+this.classname()+".of_log_jobstate] Error Insert into SYS_QUEUE_FLIGHT_LOG("+string(arg_mes_type)+" "+arg_mes_text+")")
	guoLog.uf_allways("["+this.classname()+".of_log_jobstate] Error: "+string(SQLCA_LOG.SQLCODE)+" "+SQLCA_LOG.SQLERRTEXT)
...
wf_chc_trace(0, "[of_process_skyscope_copy] Finish with return code: " + string(li_ret) +" => " + this.sreturn_error_message_short)
...
wf_chc_trace(0, "[of_process_skyscope_refresh] Finish with return code: " + string(li_ret) +" => " + this.sreturn_error_message_short)
if li_ret <> 3 then of_log_jobstate(li_ret, 'Job finished' )
