HA$PBExportHeader$uo_parent_dw.sru
$PBExportComments$Parent aller Datawindows
forward
global type uo_parent_dw from datawindow
end type
type s_setdw from structure within uo_parent_dw
end type
end forward
...
global uo_parent_dw uo_parent_dw
...
public function integer uf_setmessage (string scolumn, string smessage)
public function boolean uf_edit_style_freeform ()
public function boolean uf_add_validation (string scolumn)
public function long uf_style_dddw_freeform (string scolumn)
end prototypes
...
event ue_accepttext;IF bFocus = FALSE THEN
	this.accepttext()
END IF
end event
...
// ue_validation
//
// Pr$$HEX1$$fc00$$ENDHEX$$fung der Eingabe und setzten von numerischen Keys
//
// Wird in den Fenstern w_treeview_master usw. $$HEX1$$fc00$$ENDHEX$$berlagert!
//
...
// uf_setmessage
//
// Setzt eine Validation-Message
//
string	sValidationMSG, sReturn


sValidationMSG = uf.translate(sMessage) 

sReturn = this.modify(sColumn + ".ValidationMSG = '" + sValidationMSG + "'")
If sReturn <> "" Then return -1
...
// --------------------------------------
	integer i,iColumns
	string  sColumnType,sColumnText,sColumnName,sDataType, sTag
	string  sMText
	string  sReturn
	long	  lDDDWwidth,lWidth,lPercentWidth
...
// -------------------------------
	...
	For i = 1 to iColumns
		sColumnType = this.describe("#" + string(i) + ".Edit.Style")
		sDataType 	= lower(this.Describe ("#" + string(i) + ".ColType"))
		sTag			= lower(this.Describe ("#" + string(i) + ".Tag"))
		// --------------------------------------
		// nur Columns die eine Width haben !
		// --------------------------------------
		If integer(this.Describe("#" + string(i) + ".Width")) > 0 Then
			...
			// Neu Dateformat
			// --------------------------------------
			If Match (sDataType, "date") and sTag <> "timestamp" Then
				this.Modify("#" + string(i) + ".Edit.Format='" + s_app.sDateformat   + "'")
				this.Modify("#" + string(i) + ".Format='" + s_app.sDateformat   + "'")
			End if	
			// -------------------------------------------
			...
			Elseif Upper(sColumnType) = "DDDW" Then
				...
				lDDDWwidth = uf_style_dddw_freeform(this.describe("#" + string(i) + ".Name"))
				lWidth     = integer(this.Describe("#" + string(i) + ".Width"))
...
public function long uf_style_dddw_freeform (string scolumn);//
// uf_style_dddw_freeform
//
// Die Font-Gr$$HEX2$$f600df00$$ENDHEX$$e im DDDW auf "nette" 8 Pixel
//
	...
	if this.Describe("DataWindow.Units") <> dw_child.Describe("DataWindow.Units") then
		MessageBox("Hallo Entwickler", "Parent (" + this.dataobject + ")und Datawindowchild haben unterschiedliche Einheiten!")
	end if
...
//	dw_child.Modify("DataWindow.Detail.Height='80'")
	...
	// --------------------------------------
		...
		// --------------------------------------
			...
			dw_child.Modify("#" + string(i) + ".Font.Pitch='2'")
			dw_child.Modify("#" + string(i) + ".Font.Family='2'")
			dw_child.Modify("#" + string(i) + ".Font.Height='-8'")
			dw_child.Modify("#" + string(i) + ".y='0'")
			dw_child.Modify("#" + string(i) + ".Height='76'")
...
NewMenu.m_data.PopMenu(w_mdi_master.PointerX(), w_mdi_master.PointerY())
end event

on uo_parent_dw.create
end on

on uo_parent_dw.destroy
end on
...
//
//if row > 0 then
//	if trigger event ue_validation(dwo.name, row, data) <> 0 then
//		return 1
//	end if
//end if
//
...
// Masterupdate
//
Parent.PostEvent('ue_masterupdate')
