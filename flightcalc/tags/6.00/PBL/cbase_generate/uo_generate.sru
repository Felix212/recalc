...
public function boolean of_generate_flightcalcjobs (integer arg_function, datastore arg_dscenout)
...
string	 				sClassNameArray[], sAircraftConfiguration, ls_CFLIGHT_KEY, sClassName, sClassText, sVersion, sVersionFormat, ls_filter
...
elseif li_PaxTypeLocal >=5 and li_PaxTypeLocal <=11 then
		...
		if iImportSpmls=1 then 
			// 04.12.2019 HR: COMapp Weiche 
			if ib_use_comapp_data then
				dsCITPFlightDataSPML.setfilter( "csource = 'COMapp' " )
			else
				dsCITPFlightDataSPML.setfilter( "csource = 'CITP' " )
			end if
...
//      	dsCenOut.retrieve(this.lairlinekey, this.dGenerationDate, this.sClient, this.sUnit)
	...
	if  ii_freeze >= 1 then
		if ii_freeze = 1 or ii_freeze = 3 then
			if not of_generate_flightcalcjobs(15, dsCenOut) then
		...
		if ii_freeze = 2 or ii_freeze = 3 then
			if not of_generate_flightcalcjobs(17, dsCenOut) then
...
//	sArrivalTime				= uoConvUtcLoc.is_new_time
	sArrivalTime	= string( f_dt_add(dtDepartureDateLoc, 60), "hh:mm")
end if

// 2. Ankunftszeit UTC als DateTime
//if sDepartureTime > sArrivalTime then // 08.06.2016 HR: Wir k$$HEX1$$f600$$ENDHEX$$nnen hier nicht UTC mit LOC vergleichen
if string(dtDepartureDateUtc, "hh:mm") > sArrivalTime then
	a = 1
else
	a = 0
end if
...
elseif li_ret = 2 then
	...
	if ii_aut_mahlzeitenverteilung = 1 then
		if not of_generate_flightcalcjobs(10, dsCenOut) then
			return false
		end if
	...
	if not of_generate_flightcalcjobs(1, lds) then
		destroy lds
		return false
	end if
...
For i = 1 to dsjobs2generate.rowcount()
	...
	If isnull(iJobParameter) Then iJobParameter = 0
	 ...
	 guoLog.uf_allways("["+ this.classname( )+".of_generate_jobs] Starting job   # " + String(i, "000") + " - "  + this.of_get_job_name(i))
	...
	if bReturn = false then
		if ii_generate_jobtype=2 then
	  	...
	  	guoLog.uf_allways("["+ this.classname( )+".of_generate_jobs]         job   # " + String(i, "000") + " - " + sGenerierungsstatus + " - Returned FALSE!!")
	...
	guoLog.uf_allways("["+ this.classname( )+".of_generate_jobs] Processing job # " + String(i, "000") + " of " + String(dsjobs2generate.rowcount(), "000") + " took " + string((lEnd - lStart) / 1000, "0.00") + " seconds")
...
for l3=1 to dsCenOut.rowcount()
	...
	guoLog.uf_debug("["+ this.classname( )+".of_change_handling] Changed Flight "+ string(dsCenOut.Getitemnumber(l3,"nflight_number"))+" to " +arg_sHandlingShortText)
next
...
// --------------------------------------------------------------------------------------------------------------------
// 14.09.2021 HR: 
// 18.03.2022 HR: 8473: Schleige nur, wenn DW dsCenOutNewSPML auch vorhanden
// --------------------------------------------------------------------------------------------------------------------
if arg_module_type = 0 and isvalid(dsCenOutNewSPML) then
	lb_NewClass = true
	for i = 1 to dsCenOutNewSPML.RowCount()	
...
guoLog.uf_allways("["+ this.classname( )+".of_get_handling_from_text] Change Handling from " + sHandlingShortText + " to " + arg_handling)
...
guoLog.uf_error("["+ this.classname( )+".of_generate_flights_cfs] Rows in dsGenerateSchedule: "+string(dsGenerateSchedule.rowcount()))
...
for i=1 to dsExtCatSched.rowcount()
	...
	//guoLog.uf_error("["+ this.classname( )+".of_generate_flights_cfs]  100,"Flight "+string(i) +" of "+string(dsExtCatSched.rowcount())+": "+string(lFlightNumber)+sSuffix+" "+sTLCFrom+sTLCTo)
...
public function integer of_lock_flights (integer arg_start);long i, lresult_key, lfrom
