...
// 09.03.2017 HR: UK-CR-1977: Aufl$$HEX1$$f600$$ENDHEX$$sung der Nachfahrten
ib_explode_additional_delivery = ( of_cen_profilestring("OnlineExplosion","ExplodeAdditionalDelivery","0",is_Mandant) = "1" )
...
for j = 1 to dsCenOutMasterLevel1.RowCount()
	...
	k = lds_explosion.retrieve(ll_index_key, dGenerationDate, 1, sPlant, lAirlineKey,  dcQuantity, dcquantity_version, lMealExplosionWithBillingonly, ll_reckoning, string(tdepartureTime, "hh:mm"), &
	                                      iCNXFlag, iConvertGtoKG, iDoNotSet0ForCxFlights, ls_sales_rel, ls_def_storage_location, ls_goods_recipient, ll_status, llaccount_key, lsAccount, il_FillEmptyCustomerPL, lResultKey )
	...
	if k < 1 then
		guoLog.uf_error("["+ this.classname( )+".of_generate_sub_pl_explosion_plsql] Processing item list: " + dsCenOutMasterLevel1.GetItemString(j, "cpackinglist") + " Return: "+string(k)+" Parameter: cbase_explosion.pf_explode_pl("+string(ll_index_key)+&
						 ...
						 string(ll_status)+", "+string(llaccount_key)+", '"+lsAccount+"', "+string(il_FillEmptyCustomerPL)+", "+string(lResultKey) +");") 
	end if
...
//	lds_explosion.saveas("D:\TESTXLS\TEST-"+string(j,"00000")+"-RETURN.XLS",EXCEL!,TRUE)
	...
	for k = 1 to lds_explosion.RowCount()
		...
		if lFind > 0 then
			...
			// --------------------------------------------------------------------------------------------------------------------
			if not isnull( lds_explosion.getItemDatetime(k,"dprod_date")) then
				k = k
			end if
			...
			lcheck = lds_explosion_container.setitem(a,"nworkschedule_index", lds_explosion.getItemNumber(k,"nworkschedule_index"))
			lcheck = lds_explosion_container.setitem(a,"npltf_flight_index_group", lds_explosion.getItemNumber(k,"npltf_flight_index_group"))
			lcheck = lds_explosion_container.setitem(a,"npltf_flight_index", lds_explosion.getItemNumber(k,"npltf_flight_index"))
