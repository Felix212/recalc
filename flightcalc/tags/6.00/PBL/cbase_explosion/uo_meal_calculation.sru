...
*/	
	...
	if ids_PaxLoadCfgData.rowcount() = 1 then
		for i=1 to 9		
			if ids_PaxLoadCfgData.getitemnumber(1, "cen_pax_load_cfg_ngalley_region_key_"+string(i)) = il_GALLEY_REGION_KEY then
				lPaxManual 			= ids_PaxLoadCfgData.getitemnumber(1, "cen_pax_load_cfg_detail_npax_load_"+string(i))	
				iManualInput 		= 1
				ib_ask4passenger	= false
				exit
			end if
		next
	elseif lPax = 0 then 
		// 10.07.2023 HR: $$HEX1$$dc00$$ENDHEX$$bernahme aus TRUNK
		lPaxManual 			= 0
		iManualInput 		= 1
		ib_ask4passenger	= false	
	else
		...
		if pos(s_app.sTitle, "CBASEclassic") > 0 and lAircraftKey > 0 then
			uf.mbox("Autofill", "The new Feature Autofill is turned on here, but no data was found. Class: " + sClass +" / Pax: " + String(lPax)+" (" + string( ids_PaxLoadCfgData.rowcount() ) +")")
		else
...
from cen_out
where nresult_key = :this.lResultKey;
ll_Override_Rows = ids_spmlreplace.Retrieve( ll_handling_spml_detail_key, laircraftkey, ll_tlc_from, ll_tlc_to, dgenerationdate )
