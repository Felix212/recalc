...
for i = 1 to dsCenOutMeals.RowCount()
	...
	if ids_cen_meals_det_deduction.rowcount()>0 then
		...
		for lFind1 = 1 to ids_cen_meals_det_deduction.rowcount()			
			...
			sFind = "noverload  > nreduced and nhandling_detail_key = "+string(ids_cen_meals_det_deduction.getitemnumber(lFind1, "nhandling_detail_key"))				
...
//	if sPL = "5213701" then	
//		Messagebox(" - 1 -    ", sFind + "~r~rlFind= " + string(lFind))
//	end if 
...
for i = 1 to dsCenOutMeals.RowCount()
	...
	if ids_cen_meals_det_deduction.find("nhandling_detail_key = "+string(lHandlingDetailKey), 1, ids_cen_meals_det_deduction.rowcount())=0 then
		...
		lds.retrieve(lHandlingDetailKey, lResultKey)
...
if sProductionTextReplace<>"" then
	dsCenOutMeals.SetItem(lRow,"cproduction_text",			sProductionTextReplace)
else
	// 25.06.2014 HR: Falls wir keinen Produktionstext haben, dann nehmen wir den SL-Text
	if isnull(sProductionText) then
		dsCenOutMeals.SetItem(lRow,"cproduction_text",			sPackinglistText)
	else
		dsCenOutMeals.SetItem(lRow,"cproduction_text",			sProductionText)
	end if
end if 
...
/*  Lokale Ersetzung f$$HEX1$$fc00$$ENDHEX$$r EU-Labeling wieder raus 
			if idsLocalEuSubst.rowcount()>0 then
				...
				// 19.09.2014 HR: Lokale Ersetzung f$$HEX1$$fc00$$ENDHEX$$r EU-Labeling
				// --------------------------------------------------------------------------------------------------------------------
				ls_find = "loc_pl_subst_npl_index_key = "+string(dsCenMeals.getitemnumber(j,"cen_meals_detail_npackinglist_index_key"))
				...
				if lfound > 0 then
					...
					ii_partial_substitution 	=  idsLocalEuSubst.getitemnumber(lfound,"loc_pl_subst_flights_npartial_substitution")
					...
					dsCenMeals.setitem(j,"cen_meals_detail_npackinglist_index_key",idsLocalEuSubst.getitemnumber(lfound,"loc_pl_subst_npl_index_key_subst"))
					dsCenMeals.setitem(j,"cen_packinglist_index_cpackinglist",idsLocalEuSubst.getitemstring(lfound,"cen_packinglist_index_cpackinglist"))
					dsCenMeals.setitem(j,"cen_packinglists_npackinglist_detail_key",idsLocalEuSubst.getitemnumber(lfound,"cen_packinglists_npackinglist_detail_key"))
					dsCenMeals.setitem(j,"cen_packinglists_ctext",idsLocalEuSubst.getitemstring(lfound,"cen_packinglists_ctext"))
					dsCenMeals.setitem(j,"cen_meals_detail_cproduction_text"	,idsLocalEuSubst.getitemstring(lfound,"cen_packinglists_ctext"))
					dsCenMeals.setitem(j,"cen_packinglists_npackinglist_key", dsCenMeals.getitemnumber(j,"cen_packinglists_npackinglist_key"))
					
					ls_sub_new = string(idsLocalEuSubst.getitemnumber(lfound,"loc_pl_subst_npl_index_key_subst"))
					ls_sub_new += " "+idsLocalEuSubst.getitemstring(lfound,"cen_packinglist_index_cpackinglist")
					ls_sub_new += " "+idsLocalEuSubst.getitemstring(lfound,"cen_packinglists_ctext")
					
					if ii_partial_substitution = 1 then
						
						dsCenMeals.rowscopy(j, j, primary!, dsCenMeals, j +1, primary!)
						
						dsCenMeals.setitem(j,"cen_meals_detail_npackinglist_index_key",dsCenMeals.getitemnumber(j,"npl_index_key_ori"))
						dsCenMeals.setitem(j,"cen_packinglist_index_cpackinglist",dsCenMeals.getitemstring(j,"cpackinglist_ori"))
						dsCenMeals.setitem(j,"cen_meals_detail_cproduction_text",dsCenMeals.getitemstring(j,"ctext_ori"))
						
						dsCenMeals.setitem(j,"cen_meals_detail_ncalc_id", 1) //FESTE MENGE
						dsCenMeals.setitem(j,"cen_meals_detail_npercentage", 0)
						dsCenMeals.setitem(j,"cen_meals_detail_nvalue", 0)  //Alte SL wollen wir mit Menge 0 eintragen
						dsCenMeals.setitem(j,"cen_meals_detail_ncomponent_group", 0)

						ls_sub_new += " [teilweise]"
						
						j++
					end if
					
					uf_trace(3,"uf_compile(): Local EU Substitution: Handling_MEAL_KEY: "+string(lHandlingMealKey)+" -> ORI: " +ls_sub_ori)
					uf_trace(3,"uf_compile(): Local EU Substitution: Handling_MEAL_KEY: "+string(lHandlingMealKey)+" -> NEW: " +ls_sub_new)
...
if iIgnoreReserve = 0 then
	if lReserveQuantity > 0 then
		if iReserveType = 0 then
			lCalcBasis = lPax + lReserveQuantity
			il_calcbasis_ver = iVersion+ lReserveQuantity
...
if lPax > iVersion then
	...
	if iIgnoreTopOff = 0 then
		if lTopOffQuantity > 0 then
			if iTopOffType = 0 then
				lCalcBasis = lCalcBasis + lTopOffQuantity
				il_calcbasis_ver += lTopOffQuantity //HR 26.11.2012
