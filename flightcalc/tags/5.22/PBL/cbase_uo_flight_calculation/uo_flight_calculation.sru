...
s_app.swerk			= dsJobs.GetItemstring(lrow, "cunit")
...
if iprocess_status = 1 then // 29.11.2011 HR:
...
//			this.dsJobs.SetItem(lRow, "cerror", this.sTypeText  +this.sreturn_error_message)
		...
		//elseif  ifunction=11 then
			...
			if ii_use_mzvspawn=1 then
				...
				if this.dsJobs.getitemnumber(lRow, "comp_sum_open_mlzspawn") <= this.ii_numberrunmzvspawn then
					...
					if run(a) = 1 then
						f_Log2Csv(0,1,"[of_process_row] Mealdistributuion: Start  "+a)
					else
	...
	// --------------------------------------------------------------------------------------------------------------------
				...
				if pos(gsFunction10ExcludeFlights, a)>0 then
					...
					this.is_web_message = "Flight in Exlusionlist"
...
//26.04.2010	Aus w_change_center $$HEX1$$fc00$$ENDHEX$$bernommen		
...
if dw_handling.RowCount() = 0 and dw_single.GetItemNumber(dw_single.GetRow(),"nleg_number") = 1 then
	...
	elseif dw_single.GetItemString(dw_single.Getrow(),"chandling_type_text") = "DLV" or &
			 dw_single.GetItemString(dw_single.Getrow(),"chandling_type_text") = "SPC" then	
		//
...
public function integer of_process_mealdistribution (long arg_lrow);long 					lResult_key
uo_client_label 		uoClientLabel
n_copy_settings	inv_copy_settings

lResult_key 					= dsJobs.GetItemNumber(arg_lrow, "nresult_key")

uoClientLabel 				= create uo_client_label

//luo_ClientLabel.inv_doc_gen_settings = anv_Settings

inv_copy_settings.ib_copy_activated = TRUE
inv_copy_settings.ib_move_after_copy = FALSE
inv_copy_settings.ib_Copy_Page = TRUE
inv_copy_settings.is_archive_path = sLogPath
inv_copy_settings.is_source_path = sLogPath
inv_copy_settings.is_target_path = ""
inv_copy_settings.is_single_page_path  = sLogPath

uoClientLabel.inv_copy_settings = inv_copy_settings

// 13.04.2016 HR:
uoClientLabel.is_log_prefix	= sLogfile+"-" + sInstance+ "_" + String(Today(), "yyyymmdd") +"_"
...
uoClientLabel.bsecondarydistribution			= true
...
f_Log2Csv(0,1,"[of_process_mealdistribution] uoClientLabel.of_create_cart_diagram() Return: "+string(li_Return))	
...
FOR ll_row = 1 TO ll_rowcount_details
	...
	IF ll_row_work > 0 THEN
		// Zeile gabs schon
		// Quantity setzen, wenn sie sich ge$$HEX1$$e400$$ENDHEX$$ndert hat
	...
	for i = 1 to dw_meal.RowCount()
		...
		if dscenmealsdetdeduction.rowcount()>0 then
			...
			for lFind1 = 1 to dscenmealsdetdeduction.rowcount()			
				...
				// 25.09.2015 HR: $$HEX1$$dc00$$ENDHEX$$bernahme $$HEX1$$c400$$ENDHEX$$nderung aus GUI (Overload Korrektur - CBASE-HKG-EM-15001)
				sFind = "noverload  > nreduced and nhandling_detail_key = "+string(dscenmealsdetdeduction.getitemnumber(lFind1, "nhandling_detail_key"))				
...
datastore 					lds
date 							ld_return, ld_GenerationDate
long 							i, ll_FlightNumber, ll_LegNumber, ll_result_key, ll_result_key_group, ll_Airlinekey
string 						ls_Airline, ls_Suffix, ls_TLCFrom, ls_TLCTo 

ld_return 				= date(dw_single.getitemdatetime(1,"dreturn_date"))
ld_GenerationDate		= date(dw_single.getitemdatetime(1,"ddeparture"))
ls_Airline					= dw_single.getitemstring(1,"cairline")
ll_FlightNumber			= dw_single.getitemnumber(1,"nflight_number")
ls_Suffix					= dw_single.getitemstring(1,"csuffix")
ls_TLCFrom				= dw_single.getitemstring(1,"ctlc_from")
ls_TLCTo					= dw_single.getitemstring(1,"ctlc_to")
ll_LegNumber			= dw_single.getitemnumber(1,"nleg_number")
ll_result_key				= dw_single.getitemnumber(1,"nresult_key")
ll_result_key_group	= dw_single.getitemnumber(1,"nresult_key_group")
ll_Airlinekey				= dw_single.getitemnumber(1,"nairline_key")
...
lds.retrieve(ld_GenerationDate, ld_return, ls_Airline, ll_FlightNumber, ls_Suffix, ls_TLCFrom, ls_TLCTo, ll_LegNumber, ll_result_key, ll_result_key_group, ll_Airlinekey)

DELETE FROM cen_out_packets  
          WHERE cen_out_packets.nresult_key = :ll_result_key ;
...
lds.settransobject(SQLCA)
lds.retrieve(ld_GenerationDate, ld_return, ls_Airline, ll_FlightNumber, ls_Suffix, ls_TLCFrom, ls_TLCTo, ll_LegNumber, ll_result_key, ll_result_key_group)

DELETE FROM cen_out_los  
          WHERE cen_out_los.nresult_key = :ll_result_key ;
