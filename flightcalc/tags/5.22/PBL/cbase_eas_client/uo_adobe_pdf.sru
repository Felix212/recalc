...
global type uo_adobe_pdf from uo_basisclass_catering_object
integer ipreviewcopycount = 0
integer icopycount = 0
integer ifunction = 0
string swhereami = ""
end type
...
// --------------------------------------------------------------------------------
// Objekt : uo_adobe_pdf
// Methode: uf_isvalidatdate (Function)
//
// Argument(e):
// none
//
// Beschreibung:		check, ob document g$$HEX1$$fc00$$ENDHEX$$ltig am dDepartureDate
//
// Aenderungshistorie:
// Version 		Wer			Wann			Was und warum
// 1.0 			<unknown>	       		Erstellung
// 1.1 			O.Hoefer	25.06.2014		CBASE-NAM-CR-14024, im NICHT-deliverynote-only Modus Objekte weglassen
//
//
// Return: boolean
//
// --------------------------------------------------------------------------------
	...
	If bdeliverynoteonly = FALSE Then
		// ------------------------------------------
		// NICHT-Delivery Note Modus (NAM-CR-14025 )
		// ------------------------------------------		
		...
		WHERE				ncatering_userobject_id	= :lInheritCateringID AND
							dvalid_from <= :dDepartureDate AND  
							dvalid_to >= :dDepartureDate  AND
							(nprint_with_deliverynote = 0 OR nprint_with_deliverynote IS NULL)
		USING				SQLCA;
	Else
		// --------------------------------------
		// Delivery Note Modus (NAM-CR-14025 )
		// --------------------------------------
