...
Long	I, ll_flight, a, b, ll_rows, ll_details, ll_sub_details, ll_index_key, ll_detail_key, ll_row_id, ll_distribution, ll_sub_index_key, ll_sub_detail_key, ll_row_detail_key
...
this.dsLoadingHeader.Sort()
// of_debug_xls("BEFORE_PROCESSING_")

for ll_rows = 1 to this.dsLoadingHeader.Rowcount()

	if this.dsLoadingHeader.GetitemNumber(ll_rows, "ncontent_sheet") <> 1 then continue
	...
	// --------------------------------------------------------------------------
	dsLoadingContents.SetFilter("nrowid=" + string(ll_row_id) + " and ndetail_key=" + string(ll_row_detail_key) )
