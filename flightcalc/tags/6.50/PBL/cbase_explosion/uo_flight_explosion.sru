...
// ---------------------------------
	...
	boolean						ibGenerateLoading // 14.05.2025 HR: request #11290 New dynamic loading settings on loading lists on CBASEclassic
	DataStore					ds_pax // 14.05.2025 HR: request #11290 New dynamic loading settings on loading lists on CBASEclassic
end variables
...
Long		ll_dynrow, ll_Dyn_key, ll_From, ll_To
Integer	li_Check
String	ls_Class, ls_PL, ls_filter
Long		ll_pax , ll_oneclassonly, lfind

if ibGenerateLoading then
	
	FOR ll_dynrow = rads_Loading.RowCount() to 1 Step -1
		ll_dyn_key = rads_Loading.getitemnumber(ll_dynrow, "nloadinglist_dynload_key"  )
		// check all rows with one class
		ll_oneclassonly = rads_Loading.getitemnumber(ll_dynrow, "onlyoneclass"  )
		IF ll_oneclassonly = 1 THEN
			ls_Class = rads_Loading.getitemstring(ll_dynrow, "cclass_1"  )
			ll_From = rads_Loading.getitemnumber(ll_dynrow, "npax_from"  )
			ll_to = rads_Loading.getitemnumber(ll_dynrow, "npax_to"  )
			//ls_PL = rads_Loading.getitemstring(ll_dynrow, "cen_packinglist_index_cpackinglist"  )
...
//			select npax into :ll_pax from cen_out_pax where nresult_key = :lResultKey and cclass = :ls_Class;
			
			ls_filter = " cen_out_pax_cclass = '"+ ls_Class + "'" 
			ds_pax.setfilter(ls_filter)
			ds_pax.filter()
			
			if ds_pax.rowcount() > 0 then
				ll_pax = ds_pax.getitemnumber(1, "cen_out_pax_npax")
				IF ll_pax < ll_From OR ll_pax > ll_to THEN
					// remove row
					guoLog.uf_allways("["+ this.classname( )+".uf_dyn_loading] dynamic loading remove " + ls_PL + " for class " + ls_Class +" Pax: " + String(ll_pax) + " not fit in Range:" + String(ll_From) + " - " +  String(ll_To)  )
...
if ibGenerateLoading then
	guoLog.uf_allways("["+ this.classname( )+".uf_retrieve_detail] copy " + string( arg_ds_pax.rowcount() ) +" Paxe")
	
	for i = 1 to arg_ds_pax.rowcount()
		ds_pax.insertrow(1)
		ds_pax.setitem(1, "cen_out_pax_cclass", 	arg_ds_pax.getitemstring(i, "cclass") )
		ds_pax.setitem(1, "cen_out_pax_npax", 		arg_ds_pax.getitemnumber(i, "npax") )
	next
	...
	guoLog.uf_allways("["+ this.classname( )+".uf_retrieve_detail] Found  " + string( upperbound(lFirstArgRetrieve) ) +" Loadinglists and " + string( upperbound(lSecondArgRetrieve)  ) +" Supplements" )
...
// -------------------------------------
	...
	ds_pax.dataobject 	= "dw_exp_pax_of_one_flight"
