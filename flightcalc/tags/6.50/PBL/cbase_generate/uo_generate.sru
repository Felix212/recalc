...
elseif li_PaxTypeLocal >=5 and li_PaxTypeLocal <=11 then
		...
		if iImportSpmls>=1 and dsCITPFlightData.rowcount()>0 then 
			...
			if dsCITPFlightDataSPML.rowcount()=0 and is_spml_source	= "FC" then
				guoLog.uf_info("["+ this.classname( )+".of_generate_version] is_spml_source="+is_spml_source+", ls_CFLIGHT_KEY="+ls_CFLIGHT_KEY+", Rows in dsCITPFlightDataSPML = " + string(dsCITPFlightDataSPML.rowcount()) +" " + ls_filter_spml +" => "+ string(i))
	...
	If dsCenOut.GetitemString(i,"cunit") = sUnit Then
		...
		if iImportSpmls = 1 then
			...
			for i2 = 1 to dsCenOutNewSPML.rowcount()
				...
				next
				if lFindDef = 0 then
					guoLog.uf_debug("["+ this.classname( )+".of_generate_handling_objects] "+ls_rowcount+sOut+": "+lsClass+" "+lsSPML+" nicht zul$$HEX1$$e400$$ENDHEX$$ssig in Verbindung mit den angewiesenen Mahlzeiten!")
...
if iDays_Offset <1 then 
   guoLog.uf_allways("["+ this.classname( )+".of_generate_schedule] Offset "+string(iDays_Offset)+" is less than 1, so job is ignored.") 
   return false
end if
...
guoLog.uf_trace("["+ this.classname( )+".of_generate_jobs] sServiceName "+sServiceName)
...
Choose Case lType
	...
	case 180
		sReturn += 	"Tagesflugplan CCS  " + " - "
	case 	190
		sReturn += 	"Erstelle Checkpoints " + " - "
		lb_parameter 	= false
	case 200 // 17.05.2023 HR: 8456 new generation type to close flights
		sReturn += 	"Close Flights " + " - "
		lb_paxtype 		= false
		lb_parameter 	= false
		lb_forecast	 	= false
	case 210 // 17.05.2023 HR: 8358 2 new generation jobs requested
		sReturn += 	"Distribution " + " - "
		lb_paxtype 		= false
		lb_parameter 	= false
		lb_forecast	 	= false
	case 220 // 17.05.2023 HR: 8358 2 new generation jobs requested
		sReturn += 	"Flight Calculation " + " - "
		lb_paxtype 		= false
		lb_parameter 	= false
		lb_forecast	 	= false
	Case else
...
sReturn += 	" (Prio " + string(ii_job_prio)+")"
	...
	// 09.05.2025 HR: Wir nehmen den ersten, wenn es mehrere Treffer gibt
			SELECT 	cen_aircraft.naircraft_key,   
				...
				INTO 	:ll_aircraft_key,   
						...
						:ls_galleyversion
				FROM cen_aircraft  
				where cen_aircraft.naircraft_key in ( SELECT cen_aircraft_mapping.naircraft_key  
			...
			if sqlca.sqlcode=0 then
				...
				if arg_ds.GetItemString(arg_row, "cunit")= sUnit then
					...
					guoLog.uf_allways("["+ this.classname( )+".of_search_cfs_flight] Flight "+ls_find+" found no CBASE-AC-Type for AC " + ls_airline_owner_sky+" "+ls_actype+" Configuration: "+ ls_configuration_sky +", sqlcode="+string(sqlca.sqlcode)+", sqlerrtext=" + sqlca.sqlerrtext )
				end if
...
if  lds.getitemnumber(1,"nmaster") = 2 then
	f_log2csv(0,2, "Instance " + is_service_instance + " is configured as a JAVA service. PB service instance is paused")
	li_ret = 1
else
...
//ldw_loading = create datastore
//ldw_loading.dataobject = "dw_generate_loading"
//ldw_loading.settransobject(SQLCA)
