...
if li_ret = 0 then
	...
	if li_ret <> 0 then is_LastError = "Failed to set title for document: " + as_file
end if

// $$HEX1$$c400$$ENDHEX$$nderung speichern
if li_ret = 0 then
	li_ret = DocSave(ll_DocHandle, as_file)
	if li_ret <> 0 then is_LastError = "Failed to save document: " + as_file
end if
...
if li_Ret = 0 then
	...
	if li_Ret <> 0 then is_LastError = "Failed to get page count of document: " + as_File
end if
...
if li_ret = 0 then
	...
	if li_ret <> 0 then is_LastError = "Failed to split document: " + as_file
end if
