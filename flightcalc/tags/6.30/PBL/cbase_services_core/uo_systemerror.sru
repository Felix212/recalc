...
string is_systemerror_version = "1.12 vom 17.06.2020"
string is_CRLF = "~r~n"
...
public function string of_check_null (string arg_text)
public subroutine of_trace (integer arg_nr1, integer arg_nr2, string arg_text)
end prototypes
...
if this.itr_email.SQLCode <> 0 then
	of_trace(0,2,"[uo_systemerror.of_connect] "+is_servername+": SQLCODE :" + of_check_null(string(itr_email.sqlcode))+ "   SQLDBCODE: "+  of_check_null(string(itr_email.sqldbcode))+"  SQLERRTEXT: "+ of_check_null(this.itr_email.sqlerrtext))

	is_ServerName = ProfileString(is_apppath + "\UO_SYSTEMERROR.INI", "SYSTEMERROR", "MAIL_DB_BU", "")
	
	if is_ServerName = "" then 
		is_ServerName = "NONE"
		SetProfileString(is_apppath + "\UO_SYSTEMERROR.INI", "SYSTEMERROR", "MAIL_DB_BU", is_ServerName)
	end if
	
	if is_servername = "NONE" then
	...
	of_trace(0,2,"[uo_systemerror.of_connect] Connect to "+is_servername +" to send Emails")
	this.ib_Connected = true
	return 0
end if
...
of_trace(0,2,'[uo_systemerror.of_systemerror] ')
of_trace(0,2,'[uo_systemerror.of_systemerror] ########################################################################')
of_trace(0,2,"[uo_systemerror.of_systemerror] Systemerror: ")
of_trace(0,2,"[uo_systemerror.of_systemerror] Error Number: "+ of_check_null(String(error.number)) + " / " + of_check_null(error.text))
...
of_trace(0,2,'[uo_systemerror.of_dw_error] ')
of_trace(0,2,'[uo_systemerror.of_dw_error] ########################################################################')
of_trace(0,2,"[uo_systemerror.of_dw_error] of_dw_error: ")
...
of_trace(0,2,"[uo_systemerror.of_dw_error] Datawindow: " + of_check_null(arg_datawindowobject))
ls_text += "~r~nDatawindow: " + of_check_null(arg_datawindowobject)
of_trace(0,2,"[uo_systemerror.of_dw_error] ORA-Fehler: "+of_check_null(string(arg_sql_errcode)))
ls_text += "~r~nORA-Fehler: "+of_check_null(string(arg_sql_errcode))
of_trace(0,2,"[uo_systemerror.of_dw_error] ORA-Text: "+of_check_null(arg_sql_errtext))
ls_text += "~r~nORA-Text: "+of_check_null(arg_sql_errtext)
of_trace(0,2,"[uo_systemerror.of_dw_error] Fehler: "+ of_check_null(arg_text))
ls_text += "~r~nFehler: "+ of_check_null(arg_text)
...
if ls_mail = "1" then
	...
	if of_dt_diff(dtLast, dtNow) > 600 then
		...
		of_trace(0,2,"[uo_systemerror.of_dw_error] of_dw_error: No Mail send. Last Mail was send at "+string(dtLast, "dd.mm.yyyy hh:mm:ss"))
	end if
end if
...
// hilfsvariable
string ls_sendmail_dw, ls_sendmail_se, a, ls_server_bu
...
if ProfileString(is_apppath + "\UO_SYSTEMERROR.INI", "SYSTEMERROR", "USE_DEFAULT","1") = "1" then
	...
	ls_server_bu		= of_get_cen_setup("uo_systemerror", "db_bu",						"NONE")
	...
	if ProfileString(is_apppath + "\UO_SYSTEMERROR.INI", "SYSTEMERROR", "MAIL_DB_BU","") <> ls_server_bu then
		SetProfileString(is_apppath + "\UO_SYSTEMERROR.INI", "SYSTEMERROR", "MAIL_DB_BU", ls_server_bu)
	end if
...
IF Handle(GetApplication()) = 0 THEN
	of_trace(1,1, "[uo_systemerror.of_send_mail] No Mail while the development ");
	return 1
end if
...
if ln_ret <> 0 then 
	of_trace(1,1, "[uo_systemerror.of_send_mail] Error sending email: " + of_check_null(is_ErrorMessage));
else
	of_trace(1,1, "[uo_systemerror.of_send_mail] Sending email: Successful");
end if
...
public function string of_check_null (string arg_text);if isnull(arg_text) then
	return ' < !! Variable was NULL !!> '
else
	return arg_text
end if
end function
