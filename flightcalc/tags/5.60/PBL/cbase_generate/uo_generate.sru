...
public function boolean of_generate_flightcalcjobs (integer arg_function)
...
//      	dsCenOut.retrieve(this.lairlinekey, this.dGenerationDate, this.sClient, this.sUnit)
	...
	if  ii_freeze = 1 then
		if not of_generate_flightcalcjobs(15) then
			of_write_protocol(iJob_Type,iGenerierungsstatus,1,iFehler,sAirline,string(dGenerationDate,s_app.sDateformat),"Can't write FlightCalcJobs.")
			rollback using sqlca;
			return false
		end if
		if dsCenOut.update()<> 1 then
...
elseif li_ret = 2 then
	...
	if ii_aut_mahlzeitenverteilung = 1 then
		if not of_generate_flightcalcjobs(10) then
			return false
		end if
	end if
...
for i=1 to dsCenOut.rowcount()
	...
	// --------------------------------------------------------------------------------------------------------------------
	if dsCenOut.GetItemnumber(i, "nleg_number") > 1 and  arg_function = 10 then continue
	...
	lds.SetItem(1, "cdescription",dsCenOut.GetItemString(i,"cairline") + " " + String(dsCenOut.GetItemNumber(i,"nflight_number")) + " " + String(Date(dsCenOut.GetItemDatetime(i,"ddeparture"))))
...
if ii_delete_flights = 1 then
	...
	/* --------------------------------------------------------------------------------------------------------------------
	     ...
	     -------------------------------------------------------------------------------------------------------------------- */
	of_trace("of_generate_flights", 1,"start delete from cen_out_ppm_flights")
	...
	If sqlca.sqlcode <> 0 then
		of_trace("of_generate_flights", 1,"delete from cen_out_ppm_flights - error " + string(sqlca.sqldbcode) + ": " + sqlca.sqlerrtext)
		...
		of_trace("of_generate_flights", 1,"delete from cen_out_ppm_flights: " + string(sqlca.sqlnrows))
	end if
	
	of_trace("of_generate_flights", 1,"start delete from cen_out_ppm_act_flights")
	...
	If sqlca.sqlcode <> 0 then
		of_trace("of_generate_flights", 1,"delete from cen_out_ppm_act_flights - error " + string(sqlca.sqldbcode) + ": " + sqlca.sqlerrtext)
		...
		of_trace("of_generate_flights", 1,"delete from cen_out_ppm_act_flights: " + string(sqlca.sqlnrows))
	end if
...
if ii_delete_flights = 1 then
	/* --------------------------------------------------------------------------------------------------------------------
	     20.04.2017 HR: F$$HEX1$$fc00$$ENDHEX$$r PPM m$$HEX1$$fc00$$ENDHEX$$ssen bei einer Generierung die bereits eingefrorenen Daten erst mal
	                               gel$$HEX1$$f600$$ENDHEX$$scht werden
	     -------------------------------------------------------------------------------------------------------------------- */
	of_trace("of_generate_flights_cfs", 1,"start delete from cen_out_ppm_flights")
	DELETE FROM cen_out_ppm_flights  
     WHERE cen_out_ppm_flights.nresult_key in (SELECT NRESULT_KEY
	  														    FROM cen_out 
															  WHERE dgeneration_date 	= :dGenerationDate and 
	...
	If sqlca.sqlcode <> 0 then
		of_trace("of_generate_flights_cfs", 1,"delete from cen_out_ppm_flights - error " + string(sqlca.sqldbcode) + ": " + sqlca.sqlerrtext)
		of_write_protocol(iJob_Type,iGenerierungsstatus,1,iFehler,sAirline,string(dGenerationDate,s_app.sDateformat),"Can't delete ppm flights, sqlcode " + string(sqlca.sqlcode) + ".")
		rollback using sqlca;
		return 0
	else
		of_trace("of_generate_flights_cfs", 1,"delete from cen_out_ppm_flights: " + string(sqlca.sqlnrows))
	end if
	
	of_trace("of_generate_flights_cfs", 1,"start delete from cen_out_ppm_act_flights")
	DELETE FROM cen_out_ppm_act_flights  
     WHERE cen_out_ppm_act_flights.nresult_key in (SELECT NRESULT_KEY
	  														    FROM cen_out 
															  WHERE dgeneration_date 	= :dGenerationDate and 
	...
	If sqlca.sqlcode <> 0 then
		of_trace("of_generate_flights_cfs", 1,"delete from cen_out_ppm_act_flights - error " + string(sqlca.sqldbcode) + ": " + sqlca.sqlerrtext)
		of_write_protocol(iJob_Type,iGenerierungsstatus,1,iFehler,sAirline,string(dGenerationDate,s_app.sDateformat),"Can't delete ppm act flights, sqlcode " + string(sqlca.sqlcode) + ".")
		rollback using sqlca;
		return 0
	else
		of_trace("of_generate_flights_cfs", 1,"delete from cen_out_ppm_act_flights: " + string(sqlca.sqlnrows))
	end if
