...
public function integer of_retrieve (boolean arg_b_pdf, ref datastore ref_ds_datastore[], ref string ref_s_pdf[])
...
* Argument(e):
* boolean arg_b_pdf
*  ref datastore ref_ds_datastore[]
*
* Return: integer
*
*
...
Long 		lLogo
String	sLogo
Integer	iLanguage, i, j, k
String 	sTitle
long		lcnt
integer	iTextinfo
Integer	li_Barcode
Long		ll_Succ
//*** G. Brosch, 14.04.2009, Steuerung ge$$HEX1$$e400$$ENDHEX$$ndert >>>
//alle Flags auf false, um if-Struktur zu vereinfachen...
boolean  bcustomer_number   = false
boolean  bload_ratios_meals = false
boolean  bload_ratios_extra = false
Integer	li_Succ
String	sParamName
String	sPDFFile
String	ls_unit
String	ls_Mod, ls_Error
Long		ll_Leg_Counter
String	ls_BarCode_picture, ls_picture_list[]
Long		ll_Result_keys[], ll_transactions[], ll_empty[]
Long		ll_key
...
//27.08.2010 DB, Bugfix falsches Papierformat/Layout
long lPaperOrientation = 0
long lPaperSize = 0
// Queue Functions (sys_bill_unit_queue)
uo_rtn_queue	luo_rtn_queue

//MB 25.04.2013 CustomBoardingBill
long ll_found
//MB 20.06.2013: $$HEX1$$dc00$$ENDHEX$$berpr$$HEX1$$fc00$$ENDHEX$$fen auf offene explosions & INT007_Loads
integer llreturn

llreturn = of_check_int007(lresultkey)

if llreturn < 0 then
	this.iStatus 	= -1
	if llreturn = -1 then
		this.sError		= "Error: There are still open Explosions!"
	elseif  llreturn = -2 then
...
// Drucker festlegen
//
string	sCateringOrderPrinter

sCateringOrderPrinter = f_profilestring("changebrowser", "PRINTER1",s_app.sDefaultprinter)
...
// ---------------------------------
// AirlineKey und Transaktionsnummer
// einsammeln
// ---------------------------------
// 16.03.2009 HR: Departuredate mit ermitteln

select cen_out.nairline_key, cen_out.ddeparture into :this.lAirlineKey, :ldt_departure from cen_out where cen_out.nresult_key = :this.lResultKey;

if sqlca.sqlcode <> 0 then
	this.iStatus 	= -1
	this.sError		= "Error getting AirlineKey ~r~r Errorcode: " + string(sqlca.sqlcode) + "~r~r" + sqlca.sqlerrtext
	return this.iStatus
end if

select cen_out.ntransaction into :this.lTransaction from cen_out where cen_out.nresult_key = :this.lResultKey;

if sqlca.sqlcode <> 0 then
	this.iStatus 	= -1
	this.sError		= "Error getting Transaction ~r~r Errorcode: " + string(sqlca.sqlcode) + "~r~r" + sqlca.sqlerrtext
	return this.iStatus
end if
...
// einsammeln
// ---------------------------------
select nlogo into :lLogo from cen_airlines where nairline_key = :this.lAirlineKey;
...
select ccolor_logo into :sLogo from sys_logos where nlogo_key = :lLogo ;
...
sLogo = f_gettemppath() + sLogo

// 16.03.2009 HR: Standardlieferschein oder CrystalReport?
SELECT cen_airlines.ncustom_bbill  INTO :iCustomerBill  FROM cen_airlines  WHERE cen_airlines.nairline_key = :this.lAirlineKey;

if iCustomerBill=1  then
	...
	If Upperbound(ll_Result_keys) > 0 Then
		...
		For ll_Leg_Counter = 1 To Upperbound(ll_Result_keys)
			li_Barcode = lnv_dn_barcode.of_create_and_save( iCustomerBill, ll_Result_keys[ll_Leg_Counter], ll_transactions[ll_Leg_Counter], lAirlineKey, ldt_departure, ls_picture_list[ll_Leg_Counter])
	...
	Else
		// One Leg
		li_Barcode = lnv_dn_barcode.of_create_and_save( iCustomerBill, lResultKey, ltransaction, lAirlineKey, ldt_departure, ls_BarCode_picture)			
	End If
	...
	integer nactive = -1
	datetime dvalid_from, dvalid_to
	...
	if ll_found > 0 then
		nactive     = dsCustomBoardingBill.getitemnumber(ll_found, "nactive")
		dvalid_from = dsCustomBoardingBill.getitemdatetime(ll_found, "dvalid_from")
		dvalid_to   = dsCustomBoardingBill.getitemdatetime(ll_found, "dvalid_to")
	end if


	if nactive = 1 and dvalid_from <= ldt_departure and ldt_departure <= dvalid_to then
		...
		if this.iStatus = 0 then
			sTextFile = f_gettemppath() + "CBASE-CrystalReport-" + String(CPU(), "000000000000") + ".RPT"
			f_blob_to_file(sTextFile, bBlob)
		end if
		...
		if this.iStatus = 0 then
			...
			if luo_CR.of_load(sTextFile) <> 0 then
				this.iStatus = -1
		...
		if this.iStatus = 0 then			
			...
			for i = 1 to luo_CR.of_get_parameter_count()
				sParamName = luo_CR.of_get_parameter_name(i)
				
				choose case Trim(Upper(sParamName))
					case "RESULTKEY"
						if luo_CR.of_set_parameter(sParamName, this.lresultkey) <> 0 then
							this.iStatus = -1
		...
		if this.iStatus = 0 then
			for k = 1 to this.iCopies
				if arg_b_pdf then
					if ib_web then
						sPDFFile = is_filepath + "CBASE-DELIVERYNOTE-" + String(Rand(32767)) + String(now(), "hhmmss") + ".PDF"
					else
						sPDFFile = f_gettemppath() + "CBASE-DELIVERYNOTE-" + String(Rand(32767)) + String(now(), "hhmmss") + ".PDF"
					end if
					...
					li_Succ = luo_CR.of_export_pdf(sPDFFile)
					
					if li_Succ = 0 then
						ref_s_pdf[upperbound(ref_s_pdf) +1] = sPDFFile
					else
						this.iStatus = -1
						this.sError	= "Error: Creating PDF-File " + sPDFFile
						exit
					end if					
				else
					if sPrinter <> "" then
						...
						luo_CR.of_print(sCateringOrderPrinter)
					end if
				end if
			end for
		end if
		...
		FileDelete(sTextFile)
	elseif nactive = 1 and (dvalid_from > ldt_departure or ldt_departure > dvalid_to) then
	...
	// -----------------------------------------
	lStatus = dsSetup.GetItemNumber(1, "nloadinglist")
	//dsReport.Modify("dw_handling.visible=" + string(lStatus))
	if lStatus = 0 then dsReport.Modify("Destroy dw_handling")
	...
	// -----------------------------------------
	lStatus	= dsSetup.GetItemNumber(1, "nversion")



	//dsReport.Modify("dw_version.visible=" + string(lStatus))
	if lStatus = 0 then dsReport.Modify("Destroy dw_version")
	// -----------------------------------------
	// Sollen die MahlzeitenInfos gedruckt werden
	// -----------------------------------------
	lStatus = dsSetup.GetItemNumber(1, "nmeals")
	//dsReport.Modify("dw_meals.visible=" + string(lStatus))
	if lStatus = 0 then
		dsReport.Modify("Destroy dw_meals")
	else
		lStatus = dsSetup.GetItemNumber(1, "nuse_flags")

		if lStatus = 1 then // Stammdaten-Schalter soll verwendet werden
	...
	// -----------------------------------------
	lStatus = dsSetup.GetItemNumber(1, "nspmls")
	//dsReport.Modify("dw_meals.visible=" + string(lStatus))
	if lStatus = 0 then 	dsReport.Modify("Destroy dw_spml")
	// -----------------------------------------
	// Sollen die ExtraInfos gedruckt werden
	// -----------------------------------------
	lStatus = dsSetup.GetItemNumber(1, "nextra_loading")
	//dsReport.Modify("dw_extra.visible=" + string(lStatus))
	if lStatus = 0 then
		dsReport.Modify("Destroy dw_extra")
	else
		lStatus = dsSetup.GetItemNumber(1, "nuse_flags_add_loading")
	...
	// -------------------------------------------------------
	iTextinfo = dsSetup.GetItemNumber(1, "ntext_info")
	if iTextinfo = 0 or isnull(iTextinfo) then
	...
	if dsSetup.GetItemNumber(1, "nloadinglist_detail") = 1 then
		...
		if this.of_get_details(bcustomer_number) <> 0 then
			return this.iStatus
		end if
	...
	if dwcHeader.RowCount() > 0 then
		lAircraftKey = dwcHeader.GetItemNumber(1, "naircraft_key")
	end if
	...
	if sPrinter <> "" then
		...
		dsReport.Modify("datawindow.printer='" + sCateringOrderPrinter + "'")
	end if
	...
	// -----------------------------------------
	if dsSetup.GetItemNumber(1, "nloadinglist_by_truck") = 1 then
		dsGalleyGroups.Retrieve(lAircraftKey)

		// for i = 1 to dsGalleyGroups.RowCount()
		// 17.02.2010 Ulrich Paudler [UP]
		if arg_b_pdf then
			// Druckfertiges DW in DS kopieren
			ref_ds_datastore[upperbound(ref_ds_datastore) + 1] = create datastore
			//dsDetailReport.GetFullState( lbBlob)
			// 24.02.2010, KF: Falscher Report wurde geblobbt
			dsReport.GetFullState( lbBlob)
			
			ref_ds_datastore[upperbound(ref_ds_datastore)].SetFullState(lbBlob)
		...
		if arg_b_pdf then
			...
			If s_app.itrace = 1 then
				ref_ds_datastore[upperbound(ref_ds_datastore)].saveas("c:\temp\cbase\deliverynote_"+String(cpu()) + ".psr",psreport!,true)
			end if
	...
	if dsSetup.GetItemNumber(1, "nloadinglist_detail") = 1 then
		...
		dsDetailReport.Modify("t_title.text = '" + sTitle + " Detail'" )
		dsDetailReport.Modify("p_logo.filename = '" + sLogo + "'")
