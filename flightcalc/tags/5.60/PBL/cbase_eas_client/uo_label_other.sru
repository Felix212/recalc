...
for i = 1 to UpperBound(this.strLabelTypes)
	...
	if this.strLabelTypes[i].lLabelType = lLabelType then
		...
		dsErrorLayout.Modify("datawindow.printer='" + this.strLabelTypes[i].sPrinter + "'")
...
dsErrorLayout.Modify("datawindow.printer='" + this.strLabelTypes[1].sPrinter + "'")
dsErrorLayout.Reset()
// -----------------------------------------
...
for i = 1 to UpperBound(this.strLabelTypes)
	...
	if this.strLabelTypes[i].lLabelType = lLabelType and this.strLabelTypes[i].bBelly  = this.isBelly then
...
For I = 1 to dwData.RowCount()
	...
	//ls_New_Counter = String(ll_Counter) + " $$HEX1$$a900$$ENDHEX$$@ " + String(ll_Number_Of_Group_Members)
...
For lRows = 1 to dwData.RowCount()
	...
	if lResultKey <> dwData.GetItemNumber(lRows, "nresult_key") then
		...
		li_Succ = of_legs( )
		select cactype, cconfiguration, cKitchen_Time, cRamp_Time
		into :strFlightInfo.sActype , :strFlightInfo.sConfiguration	, :sKitchenTime	, :sRampTime
		from cen_out 
...
lStart = CPU()
// ------------------------------------------------------------
// Fehlerlabel f$$HEX1$$fc00$$ENDHEX$$r die Komponentenverteilung erstellen
// 
// 09.09.04
// Errorlabel aus dem Rennen genommen f$$HEX1$$fc00$$ENDHEX$$hrte zu Programm-
// abst$$HEX1$$fc00$$ENDHEX$$rzen nach der Migration aud PB9
// ------------------------------------------------------------
//integer 		iRowsPerLabel
//
//if sWhereAmI <> "WEB" then
//	
////	if dsDistributionErrors.RowCount() > 0 then
//		
//		iRowsPerLabel = this.of_create_error_label()
//		
//		if iRowsPerLabel > 0 then
//			
//			this.of_fill_error_columns(iRowsPerLabel)
//								
//			iRet = dsErrorLayout.GetFullState(bBlob)
//			iRetCount ++				
//			strRet[iRetCount] = Create uo_label_return
//			strRet[iRetCount].bLabel = bBlob
//			strRet[iRetCount].lLabeltype = -10
//			strRet[iRetCount].sLabel = "Distribution-Errors"
//			strRet[iRetCount].sLabelComment = "Error" 
//			strRet[iRetCount].sWorkstation = "Error" 
//			strRet[iRetCount].isBelly = false
//			strRet[iRetCount].iLabelSort = iLabelSort
//			strRet[iRetCount].iLabelPageBreak = iLabelPageBreak
//			strRet[iRetCount].sGeneratedForPrinter = this.sGeneratedForPrinter
//			
...
lEnde = CPU()
//of_log(String((lEnde - lStart) / 1000, "00.00") + " for the distribution error label")
