...
public function integer uf_fatal (string arg_file_extension, string arg_msg);uf_send_to_graylog(OPT_ERROR, arg_msg)
return uf_log(OPT_FATAL, arg_file_extension, arg_msg)
end function

public function integer uf_fatal (string arg_msg);uf_send_to_graylog(OPT_ERROR, arg_msg)
...
public function integer uf_info (string arg_file_extension, string arg_msg);uf_send_to_graylog(invo_GrayLogClient.LOG_LEVEL_INFO, arg_msg)

return uf_log(OPT_INFO, arg_file_extension, arg_msg)
end function

public function integer uf_info (string arg_msg);uf_send_to_graylog(invo_GrayLogClient.LOG_LEVEL_INFO, arg_msg)
...
Choose Case aui_MessageType
	...
	Case MESSAGE_TYPE_ERROR, MESSAGE_TYPE_SQL_ERROR
...
public function integer uf_init_graylog ();string ls_temp
  ...
  WHERE sys_login.cclient = cen_setup.cclient
		...
		and  cen_setup.cSection = 'USE'
		and sys_login.cusername = 'CBASE'   ;

if SQLCA.SQLCODE <> 0 then 
	uf_log2file("", "CEN_SETUP (GRAYLOG - USE) not set. Graylog is deactivated")
	ls_temp = "0"
end if
...
if ib_GrayLogEnabled then
  ...
  WHERE sys_login.cclient = cen_setup.cclient
 		and cen_setup.cSection = 'GRAYLOG'
		and  cen_setup.cSection = 'ENDPOINT'
		and sys_login.cusername = 'CBASE'   ;
		
	if SQLCA.SQLCODE <> 0 then 
		ib_GrayLogEnabled = false
		uf_log2file("", "CEN_SETUP (GRAYLOG - ENDPOINT) not set. Graylog is deactivated")
		return 1
	end if
	invo_GrayLogClient.of_init(ls_temp, is_log_file, is_prg_version, is_log_path)
end if
