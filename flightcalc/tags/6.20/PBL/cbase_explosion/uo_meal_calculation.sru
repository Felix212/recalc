	...
	if ii_md_dynamic_meals = 1 and ib_use_dynamic_meals then
		...
		if li_find > 0 then
			...
			if ii_md_dm_use_parent = 1 then
				...
				iPerc					= dsCenMeals.GetItemNumber( lrow, "cen_meals_detail_npercentage" )
...
for i = 1 to dsHandlingMealCover.RowCount()
	guoLog.uf_debug( "["+ this.classname( )+".uf_compile] dsHandlingMealCover row " + string(i))
...
IF lb_calcbasis_fertig and lb_calcbasis_ver_fertig THEN
	// if the calculation is finished, but dsRatiolistTyp1 is empty (because dsRatioCache was used) 
	// then retrieve dsRatiolistTyp1, so that we do run into "dcQuantity = 0"
	IF dsRatiolistTyp1.RowCount() =0 THEN
		lRows = dsRatiolistTyp1.Retrieve(lcalcdetailkey, dgenerationdate)
...
if dsRatiolistTyp1.rowcount() = 0 then
	...
	IF iConsider_SPML =1 AND (NOT lb_calcbasis_fertig) THEN 
		lFind_rl_typ1 = dsRatiolistTyp1.Find("nfrom_pax <= " + string(lCalcBasis) + &
													" and nto_pax >= " + string(lCalcBasis),1,dsRatiolistTyp1.RowCount())
	ELSE 
...
// 21.01.22, SMH #7929 
IF iConsider_SPML =1 AND (NOT lb_calcbasis_ver_fertig) THEN 
	lFind_rl_typ1 = dsRatiolistTyp1.Find("nfrom_pax <= " + string(il_calcbasis_ver) + &
	    										" and nto_pax >= " + string(il_calcbasis_ver),1,dsRatiolistTyp1.RowCount())
ELSE 
	lFind_rl_typ1 = dsRatiolistTyp1.Find("nfrom_pax <= " + string(lpax) + &
