...
if arg_module_type = 0 and isvalid(dsCenOutNewSPML) and isvalid(s_change.dsnewmeals) then
	lb_NewClass = true
	for i = 1 to dsCenOutNewSPML.RowCount()	
		if dsCenOutNewSPML.getitemnumber(i, "npt_manual")=1 then continue

		if i>1 then
			if dsCenOutNewSPML.getitemstring(i , "cclass") = dsCenOutNewSPML.getitemstring(i -1, "cclass") then
				lb_NewClass = false
			end if
		end if
		
		if lb_NewClass then
		 	for j = 1 to s_change.dsnewmeals.rowcount()	
				ll_nhandling_meal_keys = ll_empty
				if j > 1 then
					if s_change.dsnewmeals.getitemnumber(j -1, "nhandling_meal_key") = s_change.dsnewmeals.getitemnumber(j , "nhandling_meal_key") then continue
				end if
				ll_nhandling_meal_keys[upperbound(ll_nhandling_meal_keys) + 1] = s_change.dsnewmeals.getitemnumber(j , "nhandling_meal_key")
			next
		end if	

		// 07.03.2023 HR: AlmID 9086: Flight Browser is picking the wrong validity for a SPML Defintion
		dsCenOutNewSPML.SetItem(i, "cproduction_text", f_get_spml_production_text( dGenerationDate, dsCenOutNewSPML.getitemstring(i , "cspml"), ll_nhandling_meal_keys))
				...
				if i < dsCenOut.rowcount() then
					for j = i + 1 to dsCenOut.rowcount()
						if dsCenOut.getitemnumber(i, "nresult_key_group") = dsCenOut.getitemnumber(j, "nresult_key_group") then
							...
							guoLog.uf_debug("["+ this.classname( )+".of_generate_flightclose] Ignore Flight nresult_key " + string(dsCenOut.getitemnumber(j, "nresult_key"))+" as other legs are not ready to invopice")
							i = j
							ll_fehler ++
						end if
					next
...
/* --
If not wf_save() Then
	// damit der sys_flight_pricer-satz nicht reingeht !!
	rollback;
	guoLog.uf_info("["+ this.classname( )+".of_flightclose_for_one_flight] $$HEX1$$c400$$ENDHEX$$nderungen konnten nicht gespeichert werden.")
	wf_add_error_to_protocol(uf.translate("$$HEX1$$c400$$ENDHEX$$nderungen konnten nicht gespeichert werden."))
	return False
Else
	guoLog.uf_info("["+ this.classname( )+".of_flightclose_for_one_flight] Flugstatus wurde ver$$HEX1$$e400$$ENDHEX$$ndert.")
	lActCustomerKey = dsCenOut.GetItemNumber(arg_row,"ncustomer_key")
	wf_set_billing_status(lActCustomerKey)
	return True
End if		
*/
