...
global variables
string				gs_servicename 		= "cbase_flight_calculation"
...
string				gs_Profile				= "cbase_flight_calculation.ini"
...
event ue_crystal_report_load_success ( ref uo_crystal_report ruo_cr )
...
FUNCTION int GetModuleFileNameW(ulong hinstModule,REF string lpszPath,ulong cchPath) LIBRARY "kernel32" alias for "GetModuleFileNameW"
// --------------------------------------------
...
// --------------------------------------------------------------------------------------------------------------------
// 20.07.2020 HR: Wir holen uns die Version aus der EXE
// --------------------------------------------------------------------------------------------------------------------
uo_osversion 	luo_osver
luo_osver.of_get_myfileversion( gs_Version , gs_Build )

if  Handle(GetApplication()) = 0 then
	if messagebox("", "Starte Service als Instance99?", QUESTION!, YESNO!)=1 then	sCommandParm = "INSTANCE99"
end if 
// --------------------------------------------------------------------------------
...
guoLog.uf_init(sProfile, gs_Version +"-"+gs_Build, sInstance)

gs_instance_name = sInstance

// 04.04.2016 HR:
cbase.sApplicationName = "CBASE" 
cbase.is_remotedesktopdefaultprinter = ""
...
guoLog.uf_allways(" ")
guoLog.uf_allways(" _____ ______  ___   _____ _____  ______ _ _       _     _            _      ")
guoLog.uf_allways("/  __ \| ___ \/ _ \ /  ___|  ___| |  ___| (_)     | |   | |          | |     ")
guoLog.uf_allways("| /  \/| |_/ / /_\ \\ `--.| |__   | |_  | |_  __ _| |__ | |_ ___ __ _| | ___ ")
guoLog.uf_allways("| |    | ___ \  _  | `--. \  __|  |  _| | | |/ _` | '_ \| __/ __/ _` | |/ __|")
guoLog.uf_allways("| \__/\| |_/ / | | |/\__/ / |___  | |   | | | (_| | | | | || (_| (_| | | (__ ")
guoLog.uf_allways(" \____/\____/\_| |_/\____/\____/  \_|   |_|_|\__, |_| |_|\__\___\__,_|_|\___|")
guoLog.uf_allways("                                              __/ |                          ")
guoLog.uf_allways("                                             |___/                           ")
guoLog.uf_allways(" ")
...
If sCommandParm = "" then
	...
	guoLog.uf_allways("Application started as GUI: " + sServiceName + " Version: " + gs_Version + " Build: " + gs_Build)
	f_connect()
	f_connect_log()
...
Else
	...
	guoLog.uf_allways("Application started as service: " + sServiceName + " Version: " + gs_Version + " Build: " + gs_Build)
