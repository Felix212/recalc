...
elseif li_PaxTypeLocal >=5 and li_PaxTypeLocal <=11 then
		...
		if iImportSpmls=1 then 
			...
			if ib_use_comapp_data then
				...
				// --------------------------------------------------------------------------------------------------------------------
				// 19.01.2023 HR: iImportSpmls=2 => Use Forecast SPMLs
				// --------------------------------------------------------------------------------------------------------------------
				if iImportSpmls=1 then 
			...
			dsCITPFlightDataSPML.Retrieve(ls_CFLIGHT_KEY)
			guoLog.uf_debug("["+ this.classname( )+".of_generate_version] ls_CFLIGHT_KEY="+ls_CFLIGHT_KEY+", Rows in dsCITPFlightDataSPML = " + string(dsCITPFlightDataSPML.rowcount()) +" " + ls_filter_spml +" => "+ string(i))
		end if
			...
			If uo_get_ac.of_get_aircraft() <> 0 then
				...
				sAircraftConfiguration = of_get_configuration(lAircraftKey)
					if sAircraftConfiguration <> uo_get_ac.sVersionString then
						...
						guoLog.uf_debug( "["+ this.classname( )+".of_generate_version] (CITP) No valid aircraft type found: Airline-Key=" + string(lAirlineKey) + " Flightnumber=" + string(lFlightnumber) + &
...
// --------------------------------------
	If dsCenOut.update() = -1 Then
		dsCenOut.SaveAs("C:\dsCenOut.xlsx", XLSX!,True)
		return False
	End if
// --------------------------------------
...
// --------------------------------------
	If dsCenOutHandling.update() = -1 Then
		dsCenOutHandling.SaveAs("C:\dsCenOutHandling.xlsx", XLSX!,True)
		return False
	End if
// --------------------------------------
...
// --------------------------------------
	If dsCenOutQaq.update() = -1 Then
		dsCenOutQaq.SaveAs("C:\dsCenOutQaq.xlsx", XLSX!,True)
		return False
	End if
// -----------------------------------------
...
// -----------------------------------------
	If dsCenOutHandlingNews.update() = -1 Then
		dsCenOutHandlingNews.SaveAs("C:\dsCenOutHandlingNews.xlsx", XLSX!,True)
		return False
	End if
// -----------------------------------------
...
// -----------------------------------------
	If dsCenOutNewsExtra.update() = -1 Then
		dsCenOutNewsExtra.SaveAs("C:\dsCenOutNewsExtra.xlsx", XLSX!,True)
		return False
	End if
// -----------------------------------------
...
// -----------------------------------------
	If dsCenOutPax.update() = -1 Then
		dsCenOutPax.SaveAs("C:\dsCenOutPax.xlsx", XLSX!,True)
		return False
	End if
	...
	if dsCITPFlightData.RowCount() > 0 then
		...
		If lFoundRow > 0 Then	
			...
			choose case iLocalPaxType
				...
				case 11 // Forecast Daten
					...
					if isnull(dsCITPFlightData.GetItemNumber(lFoundRow,"nfc_pax")) then
						...
						guoLog.uf_debug( "["+ this.classname( )+".of_generate_pax] CITP - Forecast  for npax " + string(iPax))
					else
						iPax = dsCITPFlightData.GetItemNumber(lFoundRow,"nfc_pax")
						guoLog.uf_debug( "["+ this.classname( )+".of_generate_pax] CITP - Forecast  for nfc_pax " + string(iPax))
						li_data_source ++
					end if
			...
			choose case li_data_source
				case 1
					dsCenOutPax.Setitem(lrow, "csource", " (CITP)")
				case 2
					dsCenOutPax.Setitem(lrow, "csource", " (FC)")
				case 3
					dsCenOutPax.Setitem(lrow, "csource", " (CA)")
				case 4
					dsCenOutPax.Setitem(lrow, "csource", " (CAFC)")
			end choose
		...
		if  dsGenerateSchedule.getitemnumber(lrow,"naircraft_key")=0 then
			...
			if sqlca.sqlcode=0 then
				...
				if arg_ds.GetItemString(arg_row, "cunit")= sUnit then
					...
					guoLog.uf_allways("["+ this.classname( )+".of_search_cfs_flight]  Flight "+ls_find+" found no CBASE-AC-Type for AC " + ls_airline_owner_sky+" "+ls_actype+" Configuration: "+ ls_configuration_sky )
				end if
...
choose case arg_was
	...
	case 2 //Speichern eines Fluges
		...
		guoLog.uf_allways("["+ this.classname( )+".of_update_flights] CEN_OUT_Handling (" + string(dsCenOutHandling.rowcount())+" rows)")
...
if dsCenOut.rowcount()>0 then
	...
	if j = 0 then
		...
		of_write_protocol(iJob_Type,iGenerierungsstatus,1,iFehler,sAirline, &
								...
								ls_log)
		f_job_error(string(dGenerationDate,"dd.mm.yyyy") + "~t" + sclient + "~t" + &
						sunit + "~t" + ls_log, sTraceFile + "-" + this.sInstanceName + ".log")
		return False
