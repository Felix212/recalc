...
$PBExportComments$Internal wrapper for "Amyuni PDF Converter" functions
forward
...
function boolean SetLicenseKey(string szCompany, string szLicKey) library "cdintfXXX.dll" alias for "SetLicenseKeyA;Ansi"
function integer Lock(long hPrinter, string szLockName) library "cdintfXXX.dll" alias for "Lock;Ansi"
function integer TestLock(long hPrinter, string szLockName) library "cdintfXXX.dll" alias for "TestLock;Ansi"
function integer Unlock(long hPrinter, string szLockName, long dwTimeout) library "cdintfXXX.dll" alias for "Unlock;Ansi"
function integer SetDocFileProps(long hPrinter, string szDocTitle, long lOptions, string szFileDir, string szFileName) library "cdintfXXX.dll" alias for "SetDocFileProps;Ansi"
function integer DocOpen(ref long pedhDocument, string szFileName, string szPassword) library "cdintfXXX.dll" alias for "DocOpenA;Ansi"
function integer DocSave(long edhDocument, string szFileName) library "cdintfXXX.dll" alias for "DocSaveA;Ansi"
function integer DocPrint(long edhDocument, string szPrinterName, long lStartPage, long lEndPage, long lCopies) library "cdintfXXX.dll" alias for "DocPrintA;Ansi"
function integer DocAppend(long edhDocument, long edhSource) library "cdintfXXX.dll" alias for "DocAppend;Ansi"
function integer DocMerge(long edhDocument, long edhSource, boolean fRepeat, boolean fAbove) library "cdintfXXX.dll" alias for "DocMerge;Ansi"
function integer DocClose(long edhDocument) library "cdintfXXX.dll" alias for "DocClose;Ansi"
function long DocSetTitle(long edhDocument, string szValue) library "cdintfXXX.dll" alias for "DocSetTitleA;Ansi"
function long SetWatermark(long hPrinter, string szWatermark, string szFont, integer fontSize, integer Orientation, ulong color, long xPos, long yPos, boolean bForeground) library "cdintfXXX.dll" alias for "SetWatermark;Ansi"
function long EnablePrinter(long hPrinter, string szCompany, string szCode) library "cdintfXXX.dll" alias for "EnablePrinter;Ansi"
function long DriverInit(string szPrinter) library "cdintfXXX.dll" alias for "DriverInit;Ansi"
function integer DocGetPageCount(long edhDocument, ref integer pageCount) library "cdintfXXX.dll"
function integer DocSplit(long edhDocument, string prefix) library "cdintfXXX.dll" alias for "DocSplitA;Ansi"
//function integer DocOptimize(long edhDocument, integer iLevel) library "cdintfXXX.dll" alias for "DocOptimize;Ansi"
//function long BatchConvertEx(long hPrinter, string szFileName) library "cdintfXXX.dll" alias for "BatchConvertEx;Ansi"

subroutine DriverEnd(long hPrinter) library "cdintfXXX.dll" alias for "DriverEnd;Ansi"
subroutine GetLastErrorMsg(ref string Msg, long MaxMsg) library "cdintfXXX.dll" alias for "GetLastErrorMsg;Ansi"
...
indirect boolean MultilingualSupport {set_multilingual_support(*value), get_multilingual_support()}
indirect boolean FullEmbed {set_full_embed(*value), get_full_embed()}
indirect boolean ServerMode {set_server_mode(*value), get_server_mode()}
...
uo_amyuni self // Reference to self in order to be used to call a dynamic event in the constructor
...
boolean ib_ServerMode = false

// 06.10.2017 HR: da es mit dem MultilanguageSupport immer wieder Probleme gibt, dies jetzt als Schalter
boolean ib_MultilingualSupport = false
boolean ib_FullEmbed = false
...
public function integer of_prepare_printer (string as_output, string as_doctitle, boolean ab_grayscale, boolean ab_watermark, boolean ab_encrypted, boolean ab_multilingualsupport, boolean ab_fullembed)
private function boolean get_multilingual_support ()
private function boolean get_scale_to_fit ()
private subroutine set_multilingual_support (boolean ab_value)
private subroutine set_full_embed (boolean ab_value)
private function boolean get_full_embed ()
...
private subroutine set_server_mode (boolean ab_value)
private function boolean get_server_mode ()
protected subroutine of_emit_error (string as_function, string as_error)
end prototypes
...
try
	...
	if il_PrinterHandle <= 0 then li_ret = -1
...
if li_ret = 0 then 
	...
	of_emit_error("set_printer_name", of_get_last_error())
end if
end subroutine
...
if UpperBound(as_InputFiles) > 0 then
	...
	if li_ret <> 0 then of_emit_error("of_concatenate", "Failed to open document: " + as_InputFiles[1])
else
	of_emit_error("of_concatenate", "Not enough files to concatenate.")
	li_ret = -1
end if
...
if li_ret = 0 then
	for i = 2 to UpperBound(as_InputFiles)
		...
		if li_ret <> 0 then of_emit_error("of_concatenate", "Failed to open document: " + as_InputFiles[i])
		
		// ...und an das Hauptdokument anh$$HEX1$$e400$$ENDHEX$$ngen
		if li_ret = 0 then
			li_ret = DocAppend(ll_DocHandle, ll_TempDocHandle)
			if li_ret <> 0 then of_emit_error("of_concatenate", "Failed to append document: " + as_InputFiles[i])
		end if
...
if li_ret = 0 then
	...
	if li_ret <> 0 then of_emit_error("of_concatenate", "Failed to save document: " + as_Output)
end if
...
if li_ret = 0 and ab_Delete then
	for i = 1 to UpperBound(as_InputFiles)
		...
		if FileDelete(as_InputFiles[i]) = false then
			of_emit_error("of_concatenate", "Failed to delete document: " + as_InputFiles[i])
			li_ret = -1
		end if
	next
end if
...
if ScaleToFit then
	...
	if li_ret = 0 then
		...
		if lole_pdf.Open(as_file, "") then 
			...
			of_emit_error("of_print", "Failed to open document: " + as_file)
		end if
	...
	if li_ret <> 0 then of_emit_error("of_print", "Failed to open document: " + as_file)
	
	// Lizenz aktivieren, damit mehr als eine Seite gedruckt wird
	if li_ret = 0 then
		if SetLicenseKey(is_LicenseCompany, is_LicenseKey) = false then
			li_ret = -1
			of_emit_error("of_print", "Failed to activate license!")
		end if
	end if
	...
	if li_ret = 0 then
		...
		if li_ret <> 0 then of_emit_error("of_print", "Failed to print document: " + as_file)
	end if
...
*	Version	Wer			Wann				Was und warum
*	1.0		Dirk Bunk		10.12.2012		Erstellung
*	1.1		Dirk Bunk		15.03.2013		Vor dem L$$HEX1$$f600$$ENDHEX$$schen nochmal pr$$HEX1$$fc00$$ENDHEX$$fen, ob das Dokument $$HEX1$$fc00$$ENDHEX$$berhaupt vorhanden ist
*
* Return: 
*   0: OK
*  -1: Fehler
...
if li_ret <> 0 then of_emit_error("of_merge", "Failed to open document: " + as_target)
...
if li_ret = 0 then
	...
	if li_ret <> 0 then of_emit_error("of_merge", "Failed to open document: " + as_source)
end if
...
if li_ret = 0 then
	...
	if li_ret <> 0 then of_emit_error("of_merge", "Failed to merge documents: " + as_target + " | " + as_source)
end if

// Das gemischte Dokument speichern
if li_ret = 0 then
	li_ret = DocSave(ll_TargetDocHandle, as_output)
	if li_ret <> 0 then of_emit_error("of_merge", "Failed to save document: " + as_output)
end if
...
if li_ret = 0 and ab_delete then
	...
	if FileExists(as_target) then
		if FileDelete(as_target) = false then
			of_emit_error("of_merge", "Failed to delete document: " + as_target)
			li_ret = -1
		end if
	end if
	...
	if FileExists(as_source) then
		if FileDelete(as_source) = false then
			of_emit_error("of_merge", "Failed to delete document: " + as_source)
			li_ret = -1
		end if
	end if
end if
...
// ls_msg = Trim(String(Blob(ls_msg, EncodingUTF8!), EncodingANSI!))
ls_msg = Trim(ls_msg)
...
try
	if EnablePrinter(il_PrinterHandle, is_LicenseCompany, is_LicenseKey) <> 1 then
		of_emit_error("of_enable_printer", "Error activating Amyuni printer " + PrinterName)
		li_ret = -1
	end if
catch (RuntimeError ex)
	of_emit_error("of_enable_printer", "RuntimeError activating Amyuni printer " + PrinterName)
	li_ret = -1
end try
...
try
	...
	if li_ret = 0 then
		...
		for i = 1 to ll_PageCount
			lole_ObjArray = lole_PDF.ObjectAttribute("Pages[" + String(i) + "]", "Objects")
			
			// Alle Objekte der Seite durchlaufen und Bilder auf Graustufen setzen (Funktioniert nicht bei jedem Bild o_0)
			for j = 1 to UpperBound(lole_ObjArray)
				try
					if lole_ObjArray[j].Attribute("ObjectType") = OBJECT_TYPE_PICTURE then
						lole_ObjArray[j].Attribute("GrayScale", true)
					end if
				catch (RuntimeError ex2)
				end try
			next
...
return of_prepare_printer(as_Output, as_DocTitle, false, MultilingualSupport, FullEmbed)
end function
...
return of_prepare_printer(as_Output, as_DocTitle, false, false, false, MultilingualSupport, FullEmbed)
end function
...
// Das Dokument $$HEX1$$f600$$ENDHEX$$ffnen
li_ret = DocOpen(ll_DocHandle, as_file, "")
if li_ret <> 0 then of_emit_error("of_set_title", "Failed to open document: " + as_file)

// Lizenz aktivieren
if li_ret = 0 then
	if SetLicenseKey(is_LicenseCompany, is_LicenseKey) = false then
		li_ret = -1
		of_emit_error("of_set_title", "Failed to activate license!")
	end if
end if
...
if li_ret = 0 then
	...
	if li_ret <> 0 then of_emit_error("of_set_title", "Failed to set title for document: " + as_file)
end if

// $$HEX1$$c400$$ENDHEX$$nderung speichern
if li_ret = 0 then
	li_ret = DocSave(ll_DocHandle, as_file)
	if li_ret <> 0 then of_emit_error("of_set_title", "Failed to save document: " + as_file)
end if
...
try
	...
	if li_Ret = 0 then
		if lole_PDF.Open(as_File, "") then
			lole_PDF.SetLicenseKey(is_LicenseCompany, is_LicenseKey)
			lole_PDF.RootBookmark.InsertChild(0, as_Bookmark, "", lole_NewBookmark)
			lole_PDF.Save(as_Output, 0)
		end if
	...
	if li_Ret = 0 then
		if lole_PDF.Open(as_file, "") then
			...
			For ll_Counter = 1 To ll_RootCount
				lole_Bookmark = lole_PDF.RootBookmark.Child(ll_Counter - 1)
				ras_Bookmarks[UpperBound(ras_Bookmarks) + 1] = lole_Bookmark.Title
			Next
		end if
...
try
	...
	if li_ret = 0 then
		if lole_PDF.Open(as_file, "") then
			...
			li_CurrentPage = lole_PDF.CurrentPage
		end if
...
return li_CurrentPage
end function

private subroutine set_scale_to_fit (boolean ab_value);/*
* Objekt : uo_amyuni
* Methode: set_scale_to_fit (Setter Function)
* Autor  : Dirk Bunk
* Datum  : 11.07.2017
*
* Argument(e):
...
*   ab_Watermark				Gibt an, ob f$$HEX1$$fc00$$ENDHEX$$r den Export das aktuell gesetzte Wasserzeichen verwendet werden soll.
*   ab_Encrypted    			Gibt an, ob der Export verschl$$HEX1$$fc00$$ENDHEX$$sselt sein soll (schreibgesch$$HEX1$$fc00$$ENDHEX$$tzt).
*   ab_MultilingualSupport	Gibt an, ob das Dokument internationale Zeichens$$HEX1$$e400$$ENDHEX$$tze unterst$$HEX1$$fc00$$ENDHEX$$tzen soll.
*   ab_FullEmbed    			Gibt an, ob die verwendeten Schriftarten in das Dokument eingebettet werden sollen.
*
* Beschreibung: Bereitet den aktuellen Amyuni Drucker f$$HEX1$$fc00$$ENDHEX$$r den Druck eines Dokuments vor.
*
* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
*	1.1		Dirk Bunk				20.12.2012    	Flags direkt mit in die Funktion aufgenommen
*	1.2		Dirk Bunk				21.10.2014    	Multilanguage-Support und Font-Embedding ist jetzt immer an
*	1.3		Heiko Rothenbach	06.10.2017		Multilanguage-Support und Font-Embedding als Parameter
*
* Return: 
*   0: OK
*  -1: Fehler
...
if il_PrinterHandle > 0 then	
	...
	if li_ret = 0 then
		...
		of_emit_error("of_prepare_printer", "Error locking printer " + PrinterName)
	end if
	...
	if li_ret = 0 then
		...
		if ab_MultilingualSupport	then ll_Options += OPTION_MULTILINGUAL_SUPPORT
		if ab_FullEmbed 			then ll_Options += OPTION_FULL_EMBED 
		
		li_ret = SetDocFileProps(il_PrinterHandle, is_LockName, ll_Options, ls_Path, ls_Path + ls_File)
      	
		if li_ret <> 0 then of_emit_error("of_prepare_printer", "Error SetDocFileProps")
	end if
else
	of_emit_error("of_prepare_printer", "No printer handle")
	li_ret = -1
end if
...
private function boolean get_multilingual_support ();/*
* Objekt : uo_amyuni
* Methode: get_multilingual_support (Getter Function)
* Autor  : Dirk Bunk
* Datum  : 21.09.2020
*
* Argument(e):
*   keine
*
* Beschreibung: Gibt ein Flag zur$$HEX1$$fc00$$ENDHEX$$ck, das angibt, ob das PDF internationale Zeichens$$HEX1$$e400$$ENDHEX$$tze unterst$$HEX1$$fc00$$ENDHEX$$tzen soll.	
*
* $$HEX1$$c400$$ENDHEX$$nderungshistorie:
*	Version	Wer			Wann			Was und warum
*	1.0		Dirk Bunk		21.09.2020	Erstellung
*
* Return: 
*   Das MultilingualSupport Flag.
*/

return ib_MultilingualSupport
end function
...
private subroutine set_multilingual_support (boolean ab_value);/*
* Objekt : uo_amyuni
* Methode: set_multilingual_support (Setter Function)
* Autor  : Dirk Bunk
* Datum  : 21.09.2020
*
* Argument(e):
*   ab_value Das MultilingualSupport Flag
*
* Beschreibung: Setzt ein Flag, das angibt, ob das PDF internationale Zeichens$$HEX1$$e400$$ENDHEX$$tze unterst$$HEX1$$fc00$$ENDHEX$$tzen soll.	
*
* $$HEX1$$c400$$ENDHEX$$nderungshistorie:
*	Version	Wer			Wann			Was und warum
*	1.0		Dirk Bunk		21.09.2020	Erstellung
...
ib_MultilingualSupport = ab_value
end subroutine

private subroutine set_full_embed (boolean ab_value);/*
* Objekt : uo_amyuni
* Methode: set_full_embed (Setter Function)
* Autor  : Dirk Bunk
* Datum  : 21.09.2020
*
* Argument(e):
*   ab_value Das FullEmbed Flag
*
* Beschreibung: Setzt ein Flag, das angibt, ob die verwendeten Schriftarten im PDF eingebetten werden sollen.	
*
* $$HEX1$$c400$$ENDHEX$$nderungshistorie:
*	Version	Wer			Wann			Was und warum
*	1.0		Dirk Bunk		21.09.2020	Erstellung
...
ib_FullEmbed = ab_value
end subroutine

private function boolean get_full_embed ();/*
* Objekt : uo_amyuni
* Methode: get_full_embed (Getter Function)
* Autor  : Dirk Bunk
* Datum  : 21.09.2020
*
* Argument(e):
*   keine
*
* Beschreibung: Gibt ein Flag zur$$HEX1$$fc00$$ENDHEX$$ck, das angibt, ob die verwendeten Schriftarten im PDF eingebetten werden sollen.
*
* $$HEX1$$c400$$ENDHEX$$nderungshistorie:
*	Version	Wer			Wann			Was und warum
*	1.0		Dirk Bunk		21.09.2020	Erstellung
...
public function string of_get_version ();return is_Version
end function
...
integer li_Ret, li_PageCount
long ll_DocHandle

// Das Dokument $$HEX1$$f600$$ENDHEX$$ffnen
li_Ret = DocOpen(ll_DocHandle, as_File, "")
if li_Ret <> 0 then of_emit_error("of_get_page_count", "Failed to open document: " + as_File)

// Lizenz aktivieren
if li_Ret = 0 then
	if SetLicenseKey(is_LicenseCompany, is_LicenseKey) = false then
		li_ret = -1
		of_emit_error("of_get_page_count", "Failed to activate license!")
	end if
end if

// Zeitenzahl auslesen
if li_Ret = 0 then
	li_Ret = DocGetPageCount(ll_DocHandle, li_PageCount)
	if li_Ret <> 0 then of_emit_error("of_get_page_count", "Failed to get page count of document: " + as_File)
end if
...
// Das Dokument $$HEX1$$f600$$ENDHEX$$ffnen
li_ret = DocOpen(ll_DocHandle, as_file, "")
if li_ret <> 0 then of_emit_error("of_split", "Failed to open document: " + as_file)

// Lizenz aktivieren, damit mehr als eine Seite gedruckt wird
if li_ret = 0 then
	if SetLicenseKey(is_LicenseCompany, is_LicenseKey) = false then
		li_ret = -1
		of_emit_error("of_split", "Failed to activate license!")
	end if
end if

// Das Dokument aufteilen
if li_ret = 0 then
	li_ret = DocSplit(ll_DocHandle, as_prefix)
	if li_ret <> 0 then of_emit_error("of_split", "Failed to split document: " + as_file)
end if
...
private subroutine set_server_mode (boolean ab_value);/*
* Objekt : uo_amyuni
* Methode: set_server_mode (Setter Function)
* Autor  : Dirk Bunk
* Datum  : 11.07.2017
*
* Argument(e):
*   ab_value Das ServerMode Flag
*
* Beschreibung: Setzt ein Flag, das angibt, ob Amyuni in einer Server-Umgebung l$$HEX1$$e400$$ENDHEX$$uft	
*
* $$HEX1$$c400$$ENDHEX$$nderungshistorie:
*   Version	Wer			Wann			Was und warum
*   1.0		Dirk Bunk		07.10.2020	Erstellung
*
* Return: 
*   none
*/

ib_ServerMode = ab_value
end subroutine

private function boolean get_server_mode ();/*
* Objekt : uo_amyuni
* Methode: get_server_mode (Getter Function)
* Autor  : Dirk Bunk
* Datum  : 11.07.2017
*
* Argument(e):
...
return ib_ServerMode
end function

protected subroutine of_emit_error (string as_function, string as_error);is_LastError = "[" + ClassName() + "." + as_Function + "] " + as_Error
GetApplication().EVENT DYNAMIC ue_amyuni_error(is_LastError)
end subroutine
...
is_LicenseKey = "PASTE VERSION DEPENDENT LICENSE KEY HERE"

// PDF Creator OLE festlegen
is_PDFCreatorOLE = "PDFCreactiveX.PDFCreactiveX.{MAJOR_VERSION}.{MINOR_VERSION}"

// Version festlegen
is_Version = "{MAJOR_VERSION}.{MINOR_VERSION}"
...
// Activate the following line in the version object (uo_amyuni_xxx) and don't inherit from this 
