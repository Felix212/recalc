...
/*******************************************PRIVATE VARIABLES******************************************/
PRIVATE:
uo_crystal_report self // Reference to self in order to be used to call a dynamic event in the constructor
transaction itr_SQL
...
constant long PARAM_PRODUCTCATEGORY		= 39 // Product Catgeory  [PT] 
...
// CR Menu Grid: Handling Types
constant long PARAM_HANDLING_TYPE			= 44 // Handling Type for Menu Grid [PT] 
constant long PARAM_FREETEXTLIST_PT		= 45 // String List [PT] 
constant long PARAM_COSTCENTERGROUP		= 46 // Cost Center Group #4621
...
public subroutine of_set_transaction (ref transaction rtr_sql)
end prototypes
...
DELETE FROM cen_report_parameters WHERE dtimestamp < sysdate - 7 USING itr_SQL;

// Alle Parameter aus dem Array lesen und in die Tabelle eintragen
for i = 1 to UpperBound(al_parameters)
	// Neue Sequence lesen
	SELECT seq_cen_report_parameters.nextval INTO :ll_seq FROM dual USING itr_SQL;
	...
	VALUES (:ll_seq, :ll_report_group_key, :al_parameters[i], sysdate) USING itr_SQL;
next
...
for i = 1 to UpperBound(as_parameters)
	...
	VALUES (:ll_seq, :ll_report_group_key, :as_parameters[i], sysdate) USING itr_SQL;
next
...
*   1.6        O.H$$HEX1$$f600$$ENDHEX$$fer      28.02.2019    Login ggf. mit anderem User (St$$HEX1$$fc00$$ENDHEX$$cklistensprache)
*
* Return: 
*   0: OK
*  -1: Fehler
...
// Call a dynamic load success event on application level to be able to configure instances of this object in one place
GetApplication().EVENT DYNAMIC ue_crystal_report_load_success(self)
...
PrintSetPrinter(as_Printer) // We do not set the standard printer, this only sets the printer for the next call (PrintGetPrinter)
...
choose case al_type
	// Long Parameter
	case PARAM_LONG, PARAM_AIRLINE, PARAM_REPORTMASTERDATA, PARAM_ITEMLISTSINGLE, PARAM_COSTCATEGORY, PARAM_HANDLING_TYPE
	...
	// 18.02.2015 HR: Neue Auswahl Betriebe $$HEX1$$fc00$$ENDHEX$$ber Parametertabelle
	case PARAM_RANGELIST2, PARAM_UNITLIST2, PARAM_PRODUCTCATEGORY, PARAM_AIRL_FLAG_CLASS, PARAM_AIRL_FLAG_CYCLE, PARAM_FREETEXTLIST_PT
		//f_trace("uo_crystal_report.of_set_parameter " + as_name + ": " + as_value)
...
*	1.5		Oliver H$$HEX1$$f600$$ENDHEX$$fer		14.09.2017		Create Block erg$$HEX1$$e400$$ENDHEX$$nzt (issue #3031)
*	1.6		Dirk Bunk			28.10.2014		Support f$$HEX1$$fc00$$ENDHEX$$r Amyuni 6.0 hinzugef$$HEX1$$fc00$$ENDHEX$$gt
*	1.7		Dirk Bunk			21.09.2020		Get Amyuni object from uo_pdf_handler to be more flexible regarding the versions
*
* Return: 
*   0: OK
*  <0: Fehler
...
uo_pdf_handler luo_PDF
uo_amyuni luo_Amyuni

try
	luo_PDF = create uo_pdf_handler
	luo_Amyuni = luo_PDF.of_get_amyuni_instance()
...
IF Not IsNull(iole_CRReport) and IsValid(iole_CRReport) THEN
...
public subroutine of_set_transaction (ref transaction rtr_sql);/*
* Objekt: uo_crystal_report
* Methode: of_set_transaction
* Autor: Dirk Bunk
* Datum: 13.04.2021
*
* Argument(e):
*   rtr_SQL The transaction object to use.
*
* Beschreibung: Sets a transaction object to use for SQL transactions. Default is SQLCA
*
* $$HEX1$$c400$$ENDHEX$$nderungshistorie:
*   Version	Wer			Wann				Was und warum
*   1.0		Dirk Bunk		13.04.2021		Erstellung
*
* Return: 
*   nichts
*/
...
// Set default transaction to global SQLCA
of_set_transaction(SQLCA)

// Call a dynamic initialization event on application level to be able to configure instances of this object in one place
GetApplication().EVENT DYNAMIC ue_crystal_report_init(self)
end event
