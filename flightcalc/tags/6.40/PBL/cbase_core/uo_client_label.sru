...
if dsWorkstations.RowCount() > 0 then
	...
	For j = 1 to dsLoading.RowCount()
		...
		f_trace(classname() + "of_set_labeltype nprint=" + String(lprint) + " " + ls_Temp)
...
//if dsCheckSheet.RowCount() = 0 then
//	ls_Filter = dsCheckSheet.Describe("DataWindow.Table.Filter")
//	dsCheckSheet.SetFilter("")
//	dsCheckSheet.Filter()
//
//end if
...
//	If bchecksheet Then
//		uoDistribution.bchecksheet = TRUE
//	End If
//End If	
	...
	if this.iAddLabelOption = 1 then
		...
		For I = 1 to this.dsLoading.RowCount()
			if Len(Trim(this.of_checknull(this.dsLoading.GetItemString(I, "cdistributed_components")))) > 0 then
				...
				f_trace(classname() + ".of_check_sheet => nprint = 1" )
...
For I = 1 to this.dsLoading.RowCount()
	...
	ls_Temp = dsLoading.GetItemString(i, "cen_packinglist_index_cpackinglist")
	//	f_trace(classname() + "of_check_sheet Set Item " + ls_Temp)
...
if bCheckSheetOnlyUsed and not blabel then
	...
	for a = this.dsCheckSheet.RowCount() to 1 Step -1
		...
		if len(sChecker) = 0 then
			...
			f_trace(classname() + "of_check_sheet remove " + ls_Temp)
...
//	If IsnULL(ll_Stowage_key) THEN
//		f_trace("uo_ClientLabel.of_cartdiagram_sheet ll_Stowage_key Is NULL")
//	ELSE
//		f_trace("uo_ClientLabel.of_cartdiagram_sheet ll_Stowage_key " + String(ll_Stowage_key))
//	END IF
...
if NOT ib_enable_masterdata_view Then
	if is_CartDiagramSort > "" Then
		...
		f_trace("of_cartdiagram_sheet is_CartDiagramSort " + is_CartDiagramSort)
	End If
End If 
...
//	ll_rowid	=	dsCartdiagramSheet.GetItemNumber(ll_Row, "nrowid")
//	//ll_rowid += 1000000
//	If ll_rowid >= 1000000 Then
//		ll_rowid = ll_Row + 1000000
//	Else
//		ll_rowid = ll_Row
//	End If
//		
//	li_Succ = dsLoading.SetItem(ll_Row, "nrowid", ll_rowid)
////	if ii_Do_NOT_Trace= 0 then
//		f_trace("of_cartdiagram_sheet RowId="  + String(ll_rowid) )
////	End If
//Next
...
Long		ll_Result_Key, ll_Transaction	
Long		ll_Success
String	ls_MSG
...
IF  SQLCA.sqlcode <> 0 THEN
	//		//Error DECLAREing ProcedureCbase_label.pf_prepare_flightlabels: " + SQLCA.SQLErrText)
	//		//Return -1
	//uf.mbox("Error", "Declare Error Cbase_label.pf_prepare_flightlabels " + SQLCA.SQLErrText)
	f_trace("Declare Error Cbase_label.pf_prepare_flightlabels " + SQLCA.SQLErrText)	
End If
...
IF SQLCA.sqlcode <> 0 THEN
	...
	uf.mbox("Achtung", "ERROR pf_prepare_flightlabels")
Else
	ll_Success = SQLCA.sqlcode	
End If
...
DECLARE pp_update_stowages PROCEDURE FOR Cbase_ppm_uk.pp_update_stowages(:ll_result_key, :ll_transaction ) USING SQLCA;

IF  SQLCA.sqlcode <> 0 THEN
	//Error DECLAREing Procedure Cbase_ppm_uk.pp_update_stowages: " + SQLCA.SQLErrText)
	//Return -1
	uf.mbox("Error", "Declare Error Cbase_ppm_uk.pp_update_stowages " + SQLCA.SQLErrText)
End If
...
Else
	...
	EXECUTE pp_update_stowages;
	
	IF SQLCA.sqlcode <> 0 THEN
		ll_Success = SQLCA.sqlcode
		uf.mbox("Achtung", "ERROR pp_update_stowages")
	Else
		ll_Success = SQLCA.sqlcode	
	End If
	
	 
	FETCH pp_update_stowages INTO :ls_MSG;
	
	f_trace( ls_MSG )
	
	IF SQLCA.sqlcode <> 0 THEN
		ll_Success = SQLCA.sqlcode
		uf.mbox("Achtung", "FETCH ERROR pp_update_stowages")
	Else
		ll_Success = SQLCA.sqlcode	
	End If
...
f_trace("uo_client_label.of_get_trolley_key Number of drawers in gtt_parameter: " + String(ll_Count))
...
f_trace("#GTT Cbase_label.pf_create_transporter_cart(" + String(al_result_key) +",'"  + as_id + "','" + as_param_group + "');")
