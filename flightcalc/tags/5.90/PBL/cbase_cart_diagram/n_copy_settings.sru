...
if not fileexists(as_file) then
	f_trace("File Copy " + as_file + "  DOES NOT EXIST")
end if
...
//	f_log("FILE_COPY", "File Copy " + as_file + " (" + String(ll_FileSize_SRC) +") => " + as_target + " (" + String(ll_FileSize_TRG)+ ")")
	f_trace("File Copy " + as_file + " (" + String(ll_FileSize_SRC) +") => " + as_target + " (" + String(ll_FileSize_TRG)+ ")")
	...
	End If
Else
	//f_trace("File Copy FAILED " + as_file + " => " + as_target)
	f_trace("File Copy FAILED " + as_file + " (" + String(ll_FileSize_SRC) +") => " + as_target )
	Return -1
End if

//f_log("File Copy", "File Copy " + as_file + " (" + String(ll_FileSize_SRC) +") => " + as_target + " (" + String(ll_FileSize_TRG)+ ")")
