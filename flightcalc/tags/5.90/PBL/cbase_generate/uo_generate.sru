...
// 19.03.2019 HR: um Forecast erweitert 
...
elseif li_PaxTypeLocal >=5 and li_PaxTypeLocal <=11 then
		...
		if dsCITPFlightData.RowCount() > 0 then
			...
			// 15.03.2019 HR: 
			of_retrieve_bob_figures( dsCITPFlightData.GetItemNumber(1,"cen_request_flight_nreq_flight_key"), lcenoutrow )
		...
		If lFoundRow > 0 Then	
			choose case iLocalPaxType
				...
				case 11 // Forecast Daten
					// 15.03.2019 HR:
					//  cen_request_pax_nfc_pax  cen_request_pax_nfc_margin 
					if isnull(dsCITPFlightData.GetItemNumber(lFoundRow,"cen_request_pax_nfc_pax")) then
						iPax = dsCITPFlightData.GetItemNumber(lFoundRow,"cen_request_pax_npax")
					else
						iPax = dsCITPFlightData.GetItemNumber(lFoundRow,"cen_request_pax_nfc_pax")
					end if
			...
			if iImportSpmls >= 1 then
				// 15.03.2019 HR:
				if iImportSpmls = 1 or isnull(dsCITPFlightData.GetItemNumber(lFoundRow,"cen_request_pax_nfc_pax")) then
					iPax = dsCITPFlightData.GetItemNumber(lFoundRow,"cen_request_pax_npax_spml")
				else
					iPax = dsCITPFlightData.GetItemNumber(lFoundRow,"cen_request_pax_nfc_spml")
				end if
				dsCenOutPax.Setitem(lrow,"npax_spml",iPax)
...
// -----------------------------------------------------------------------------------------------------------
of_trace("of_update", 1,"start delete from cen_out: ")
...
If sqlca.sqlcode <> 0 then
	of_trace("of_update", 1,"delete from cen_out - error " + string(sqlca.sqldbcode) + ": " + sqlca.sqlerrtext)
	of_write_protocol(iJob_Type,iGenerierungsstatus,1,iFehler,sAirline,string(dGenerationDate,s_app.sDateformat),"Can't delete flight schedule, sqlcode " + string(sqlca.sqlcode) + ".")
	rollback using sqlca;
	return false
else
	of_trace("of_update", 1,"finish delete from cen_out: " + string(sqlca.sqlnrows))
end if
...
For i = 1 to lRowcount
	...
	If dsCenOut.GetitemString(i,"cunit") = sUnit Then
		...
		//	uo_generate_meals.dsCenOutMeals.Reset()
		of_trace("of_generate_handling_objects", 50, ls_rowcount+sOut +" Create Meals")
		...
		End if		
		of_trace("of_generate_handling_objects", 50, ls_rowcount+sOut +" Reduce Overload")
		...
		of_trace("of_generate_handling_objects", 50, ls_rowcount+sOut +" Create Extra")
...
if not uo_generate_meals.ib_SLReplaceForACType then
   of_trace("of_generate_schedule", 1, "SLReplaceForACType is set to FALSE")
end if
...
elseif li_ret = 2 then
	...
	if ii_create_checkpts  = 1 then
		...
		of_trace("of_generate_schedule", 1, "After create Checkpoints") 
	end if
	
	of_trace("of_generate_schedule", 1, "of_update start") 
	...
	of_trace("of_generate_schedule", 1, "of_update finish") 
	...
	// --------------------------------------------------------------------------------------------------------------------
	of_trace("of_generate_schedule", 1, "of_update_history start") 

	If not of_update_history() Then
		rollback using sqlca;	
		return False
	End if
	of_trace("of_generate_schedule", 1, "of_update_history finish") 
	...
	if ii_aut_mahlzeitenverteilung = 1 then
		...
		of_trace("of_generate_schedule", 1, "mealdistribution jobs created")
	end if
	...
	if  ii_freeze >= 1 then
		if ii_freeze = 1 or ii_freeze = 3 then
			...
			of_trace("of_generate_schedule", 1, "freeze jobs created")
		end if
		...
		if ii_freeze = 2 or ii_freeze = 3 then
			...
			of_trace("of_generate_schedule", 1, "preprod freeze jobs created")
		end if
	end if
	...
	if ii_document_generation = 1 then
		...
		of_trace("of_generate_schedule", 1, "docengine jobs created")
	end if
...
For i = 1 to dsjobs2generate.rowcount()
	...
	if of_checkconnection() <> 0 then
		this.of_trace("of_generate_jobs", 0, " We stop processing, due to dabase problems ")
		return false
	end if
	...
	if ii_generate_jobtype=2 then
		...
		this.of_trace("of_generate_jobs", 1, "Exit Loop due to Onetimejob finsihed")
		exit 
	end if
...
sqlca_temp.ServerName = ProfileString(s_App.sProfile,"Database","Server","")
sqlca_temp.DBMS 			= ProfileString(s_App.sProfile,"Database","DBMS","ODBC")
sqlca_temp.DBParm 		= ProfileString(s_App.sProfile,"Database","DBPARM","")
sqlca_temp.LogPass 		= ProfileString(s_App.sProfile,"Database","LogPass","")
sqlca_temp.LogId 			= ProfileString(s_App.sProfile,"Database","LogId","")
...
this.of_trace("of_generate_docengine_jobs", 1, "DOC Engine - Jobs erstellt: "+string(li_ok)+" / Ignore: "+string(li_ignore)+" / mit Fehler: "+string(li_error))
...
for i=1 to arg_cen_out.rowcount()
	...
	if lds_chkpt.rowcount()=0 then
		of_trace("of_create_checkpts", 1,"No CheckPoints for Routing "+ arg_cen_out.getitemstring(i, "crouting") +" found.")
		continue
	end if
...
if dsCenOutCheckPt.update() = -1 then
	...
	this.of_trace("of_generate_checkpts", 3, "Rows in dsCenOutCheckPt: "+string(dsCenOutCheckPt.rowcount()))	
...
if f_cen_profilestring("MealDistribution","SaveDistribution","0") = "0" and arg_function = 10 then
	of_trace("of_generate_flightcalcjobs("+string(arg_function)+")", 1,"SaveDistribution is switched off. No Flightcalc-Jobs are created")
	return true
end if
...
for i=1 to arg_dsCenOut.rowcount()
	...
	if lsequence = -1 then
		of_trace("of_generate_flightcalcjobs("+string(arg_function)+")" , 1,"Error getting Sequence:  " + string( SQLCA.SQLCODE) + " - " + SQLCA.SQLERRTEXT)
		destroy lds
		return false
	end if 
...
if lds.update()=1 then
	of_trace("of_generate_flightcalcjobs("+string(arg_function)+")", 1,"Generate " + string( lds.rowcount()) + " FlightCalc Jobs.")
	...
	of_trace("of_generate_flightcalcjobs("+string(arg_function)+")", 1,"Update Error:  " + string( lds.lSQLErrorDBCode)+" "+lds.sSQLErrorText)
...
/* --------------------------------------------------------------------------------------------------------------------
	  20.04.2017 HR: F$$HEX1$$fc00$$ENDHEX$$r PPM m$$HEX1$$fc00$$ENDHEX$$ssen bei einer Generierung die bereits eingefrorenen Daten erst mal
										 gel$$HEX1$$f600$$ENDHEX$$scht werden
	  -------------------------------------------------------------------------------------------------------------------- */
of_trace("of_generate_flights_deletebefore", 1,"start delete from CEN_OUT_PPM_PROD_LABEL_PDF")	  
DELETE FROM CEN_OUT_PPM_PROD_LABEL_PDF      
          WHERE NBATCH_SEQ in ( SELECT NBATCH_SEQ 
											 FROM CEN_OUT_PPM, 
											 		  CEN_OUT
											  WHERE CEN_OUT.NRESULT_KEY 		= CEN_OUT_PPM.NRESULT_KEY 
												  AND CEN_OUT.dgeneration_date 	= :dGenerationDate  
												  AND CEN_OUT.cclient	  				= :sclient 
												  AND CEN_OUT.cunit		  				= :sunit 
												  AND CEN_OUT.nairline_key 			= :lAirlineKey  
												  AND CEN_OUT.NNOTREGENERATE 	= 0 );

If sqlca.sqlcode <> 0 then
	of_trace("of_generate_flights_deletebefore", 1,"delete from CEN_OUT_PPM_PROD_LABEL_PDF - error " + string(sqlca.sqldbcode) + ": " + sqlca.sqlerrtext)
	of_write_protocol(iJob_Type,iGenerierungsstatus,1,iFehler,sAirline,string(dGenerationDate,s_app.sDateformat),"Can't delete ppm prod label pdf, sqlcode " + string(sqlca.sqlcode) + ".")
	rollback using sqlca;
	return 0
else
	of_trace("of_generate_flights_deletebefore", 1,"delete from CEN_OUT_PPM_PROD_LABEL_PDF: " + string(sqlca.sqlnrows))
end if	  
	  
of_trace("of_generate_flights_deletebefore", 1,"start delete from cen_out_ppm_flights")
DELETE FROM cen_out_ppm_flights  
  WHERE cen_out_ppm_flights.nresult_key in (SELECT NRESULT_KEY
															 FROM cen_out 
														  WHERE dgeneration_date 	= :dGenerationDate and 
																	cclient	  				= :sclient and
																	cunit		  				= :sunit and
...
If sqlca.sqlcode <> 0 then
	of_trace("of_generate_flights_deletebefore", 1,"delete from cen_out_ppm_flights - error " + string(sqlca.sqldbcode) + ": " + sqlca.sqlerrtext)
	of_write_protocol(iJob_Type,iGenerierungsstatus,1,iFehler,sAirline,string(dGenerationDate,s_app.sDateformat),"Can't delete ppm flights, sqlcode " + string(sqlca.sqlcode) + ".")
	rollback using sqlca;
	return 0
else
	of_trace("of_generate_flights_deletebefore", 1,"delete from cen_out_ppm_flights: " + string(sqlca.sqlnrows))
end if

of_trace("of_generate_flights_deletebefore", 1,"start delete from cen_out_ppm_act_flights")
DELETE FROM cen_out_ppm_act_flights  
  WHERE cen_out_ppm_act_flights.nresult_key in (SELECT NRESULT_KEY
															 FROM cen_out 
														  WHERE dgeneration_date 	= :dGenerationDate and 
																	cclient	  				= :sclient and
																	cunit		  				= :sunit and
...
If sqlca.sqlcode <> 0 then
	of_trace("of_generate_flights_deletebefore", 1,"delete from cen_out_ppm_act_flights - error " + string(sqlca.sqldbcode) + ": " + sqlca.sqlerrtext)
	of_write_protocol(iJob_Type,iGenerierungsstatus,1,iFehler,sAirline,string(dGenerationDate,s_app.sDateformat),"Can't delete ppm act flights, sqlcode " + string(sqlca.sqlcode) + ".")
	rollback using sqlca;
	return 0
else
	of_trace("of_generate_flights_deletebefore", 1,"delete from cen_out_ppm_act_flights: " + string(sqlca.sqlnrows))
end if

of_trace("of_generate_flights_deletebefore", 1,"start delete from CEN_OUT_PPM_PROD_LABEL")
delete from CEN_OUT_PPM_PROD_LABEL pl 
       where PL.NPPM_PROD_LABEL_KEY in ( select PLD.NPPM_PROD_LABEL_KEY 
		                                                       from CEN_OUT_PPM_PR_LAB_DETAIL pld, 
											                      cen_out co 
									                     where PLD.NRESULT_KEY = CO.NRESULT_KEY
															and co.dgeneration_date 	= :dGenerationDate 
															and co.cclient	  				= :sclient
															and co.cunit		  				= :sunit
															and co.nairline_key 			= :lAirlineKey
															and co.NNOTREGENERATE 	= 0 );

If sqlca.sqlcode <> 0 then
	of_trace("of_generate_flights_deletebefore", 1,"delete from CEN_OUT_PPM_PROD_LABEL - error " + string(sqlca.sqldbcode) + ": " + sqlca.sqlerrtext)
	of_write_protocol(iJob_Type,iGenerierungsstatus,1,iFehler,sAirline,string(dGenerationDate,s_app.sDateformat),"Can't delete ppm prod labels, sqlcode " + string(sqlca.sqlcode) + ".")
	rollback using sqlca;
	return 0
else
	of_trace("of_generate_flights_deletebefore", 1,"delete from CEN_OUT_PPM_PROD_LABEL: " + string(sqlca.sqlnrows))
end if
...
// -----------------------------------------------------------------------------------------------------------
of_trace("of_generate_flights_deletebefore", 1,"start lock flights in cen_out")
...
if lds.retrieve(dGenerationDate, sclient, sunit, lAirlineKey) = -1 then
	of_trace("of_generate_flights_deletebefore", 1,"retrieve for update cen_out - error " + string(sqlca.sqldbcode) + ": " + sqlca.sqlerrtext)
	...
	for i = 1 to lds.rowcount()
		...
		If sqlca.sqlcode <> 0 then
			of_trace("of_generate_flights_deletebefore", 1,"Lock flight " + string(lds.getitemnumber(i, "nresult_key")) +" - error " + string(sqlca.sqldbcode) + ": " + sqlca.sqlerrtext)
			rollback using sqlca;
			return 0
		end if
	next
...
of_trace("of_generate_flights_deletebefore", 1,"finish Lock flights: " + string(lds.rowcount()))

destroy lds
/* 
of_trace("of_generate_flights_deletebefore", 1,"start delete from cen_out")
...
If sqlca.sqlcode <> 0 then
	of_trace("of_generate_flights_deletebefore", 1,"delete from cen_out - error " + string(sqlca.sqldbcode) + ": " + sqlca.sqlerrtext)
	of_write_protocol(iJob_Type,iGenerierungsstatus,1,iFehler,sAirline,string(dGenerationDate,s_app.sDateformat),"Can't delete flight schedule, sqlcode " + string(sqlca.sqlcode) + ".")
	rollback using sqlca;
	return 0
else
	of_trace("of_generate_flights_deletebefore", 1,"finish delete from cen_out: " + string(sqlca.sqlnrows))
end if
 */
...
if sqlca.sqlcode <> 0 then
	...
	this.of_trace("of_checkconnection", 0, SQLCA.ServerName +": ErrorCode = " + String(sqlca.sqlcode))
	this.of_trace("of_checkconnection", 0, SQLCA.ServerName +": ErrorText = " + String(sqlca.sqlerrtext))
	...
	if sqlca.sqlcode = 0 then
		this.of_trace("of_checkconnection", 0, SQLCA.ServerName +": Transaction re-created" )
		...
		this.of_trace("of_checkconnection", 0, SQLCA.ServerName +": ReConnect-ErrorCode = " + String(sqlca.sqlcode))
		this.of_trace("of_checkconnection", 0, SQLCA.ServerName +": ReConnect-ErrorText = " + String(sqlca.sqlerrtext))
...
if ldsBobSched.rowcount()=0 then
	...
	if ll_flight_key = -1 then
		of_trace("[of_retrieve_bob_figures]", 0, "Fehler bei der Ermittlung der SEQ_CEN_BOB_SCHEDULE" )
		destroy ldsBobSched
		return 0
	end if		
	...
	if sqlca.sqlcode <> 0 then
		of_trace("[of_retrieve_bob_figures]", 0, "Fehler bei der Anlage cen_bob_schedule (" + string(sqlca.sqlcode)+") "+sqlca.sqlerrtext )
		destroy ldsBobSched
		return 0
	end if	
else
...
for li_row = 1 to ldsSysQueueFlightBob.rowcount()
	...
	if li_found > 0 then
		...
		if ll_seq = -1 then
			of_trace("[of_retrieve_bob_figures]", 0, "Fehler bei der Ermittlung der SEQ_CEN_BOB_PAX_COUNTS" )
			destroy ldsBobSched
			return 0
		end if		
...
of_trace("[of_retrieve_bob_figures]", 0, "Create or Update " + string( ldsBobPaxCount.rowcount() ) +" BOB-Items Rows")
