HA$PBExportHeader$uo_tree_detail_datawindow.sru
forward
global type uo_tree_detail_datawindow from uo_parent_dw
end type
end forward
...
global uo_tree_detail_datawindow uo_tree_detail_datawindow
...
integer	iDragDrop 				= 0
integer	iSetDetailHeight 		= 1
integer	iMarkFocusedColumn 	= 0
...
//string	sFromColumnName 		= "dvalid_from"
//string   sToColumnName	 		= "dvalid_to"
Integer	iValidationCheck		= 0
Integer  iMaxTabOrder

DateTime	dtCheckFrom
DateTime	dtCheckTo
...
public function integer of_get_form_status ()
...
public function integer of_validation_check ()
...
event ue_mouse_up;// -------------------------------------
// Pr$$HEX1$$fc00$$ENDHEX$$fen, ob G$$HEX1$$fc00$$ENDHEX$$ltigkeit ge$$HEX1$$e400$$ENDHEX$$ndert werden
// darf
// -------------------------------------
if this.of_validation_check() <> 0 then 
	uf.mbox("Achtung",this.sErrorMessage, StopSign!)
	return 0
end if
...
if iFound = 0 then
	uf.mbox("Hallo Entwickler", "Linie l_anchor im Header fehlt")
	return -1
end if
...
// -----------------------------------------------------------------------
// Die zugeordneten Rechtecke suchen
// -----------------------------------------------------------------------
...
For i = 1 to dsObjects.RowCount()
	...
	if dsObjects.GetItemString(i, "stype") = "column" then
		...
		lX 		= dsObjects.GetItemNumber(i, "xpos")
		lY 		= dsObjects.GetItemNumber(i, "ypos")
		lWidth 	= dsObjects.GetItemNumber(i, "iwidth")
		lHeight 	= dsObjects.GetItemNumber(i, "iheight")
		
		lFound = dsObjects.Find("stype = 'rectangle' and " +  & 
										 "xpos <= " + string(lX) +                   " and " + &
										 "xpos + iWidth >= " + String(lX + lWidth) + " and " + &
										 "ypos < " + string(lY) +                    " and " + &
										 "ypos + iheight > " + String(lY + lHeight), 1, dsObjects.RowCount())
		
		if lFound > 0 then 
			
			dsObjects.SetItem(i, "srectangle", dsObjects.GetItemString(lFound, "sobject"))
...
For i = 1 to strLoad.dsDetail.RowCount()
	...
	if strLoad.dsDetail.GetItemString(i, "stype") = "column" then
		...
		if lFound <= 0 then
			...
			//Messagebox("-1    - "   + string(lFound), sFind + ";" + this.dataobject + ";" + sObject)
			Beep(2)
			return -1
...
public function integer of_validation_check ();// ------------------------------------------------------
//
// Pr$$HEX1$$fc00$$ENDHEX$$fen, ob der aktuelle Datens$$HEX1$$e400$$ENDHEX$$tz ge$$HEX1$$e400$$ENDHEX$$ndert werden darf
//
// Pr$$HEX1$$e400$$ENDHEX$$misse: Es d$$HEX1$$fc00$$ENDHEX$$rfen keine Daten ge$$HEX1$$e400$$ENDHEX$$ndert werden, die 
//           entweder eine abgelaufene G$$HEX1$$fc00$$ENDHEX$$ltigkeit haben
// 			 oder deren G$$HEX1$$fc00$$ENDHEX$$ltigkeit auf das aktuelle Datum
//           f$$HEX1$$e400$$ENDHEX$$llt .....
// ------------------------------------------------------
Datetime dtToday

if this.iValidationCheck = 0 then
	return 0
end if
	
dtToday = DateTime(Today())
// ------------------------------------------------------
// aktuelle G$$HEX1$$fc00$$ENDHEX$$ltigkeit darf nicht ge$$HEX1$$e400$$ENDHEX$$ndert werden
// ------------------------------------------------------
if this.dtCheckFrom <= dtToday and this.dtCheckTo >= dtToday then
	sErrorMessage = uf.translate("Aktuelle G$$HEX1$$fc00$$ENDHEX$$ltigkeiten d$$HEX1$$fc00$$ENDHEX$$rfen nicht ge$$HEX1$$e400$$ENDHEX$$ndert werden!")
	return -1
end if
	
// ------------------------------------------------------
// abgelaufene G$$HEX1$$fc00$$ENDHEX$$ltigkeit darf nicht ge$$HEX1$$e400$$ENDHEX$$ndert werden
// ------------------------------------------------------
if this.dtCheckFrom < dtToday and this.dtCheckTo < dtToday then
	sErrorMessage = uf.translate("Abgelaufene G$$HEX1$$fc00$$ENDHEX$$ltigkeiten d$$HEX1$$fc00$$ENDHEX$$rfen nicht ge$$HEX1$$e400$$ENDHEX$$ndert werden!")
	return -2
end if
...
// Messagebox("", this.sSection +"|"+ sKey +"|"+ sValue )
	...
	if sBand = "detail" and (sType = "column" or sType = "compute" or sType = "text" or sType = "rectangle") then
		...
		if iY <= 8 then
			...
			if sType = "column" then
				...
				if Integer(this.Describe(sObjects[i] + ".tabsequence")) > this.iMaxTabOrder then
					this.iMaxTabOrder = Integer(this.Describe(sObjects[i] + ".tabsequence"))
				end if
...
// darf
// -------------------------------------
if sField <> "dvalid_to" then
	if this.of_validation_check() <> 0 then 
		this.uf_setmessage(sField,this.sErrorMessage)
...
on uo_tree_detail_datawindow.create
call super::create
end on

on uo_tree_detail_datawindow.destroy
call super::destroy
end on
...
event itemfocuschanged;call super::itemfocuschanged;String sObject, sRectangle, sRet
long 	 lFound, lFoundPrev
Long	 lTabOrder, lPrevTaborder

if row > 0 then
	
	if iMarkFocusedColumn = 1 then
		
		sObject = dwo.name
		
		lFound = this.dsObjects.Find("sobject='" + sObject + "'", 1, this.dsObjects.RowCount())
				
		if lFound > 0 then
		
			sRectangle = this.dsObjects.GetItemString(lFound, "srectangle")
			lTabOrder  = this.dsObjects.GetItemNumber(lFound, "nhas_taborder")
			
			if lTaborder >= 20 then
				lPrevTabOrder = lTabOrder - 10
			end if
					
			// -----------------------------------------------------------------
			// Rechteck um aktuelle Column orange f$$HEX1$$e400$$ENDHEX$$rben
			// -----------------------------------------------------------------
			this.Modify(sRectangle + ".pen.color='" + string(29360127) + "'") // rgb(255,198,111)
			this.SetPosition(sRectangle, "", True)
			this.SetPosition(sObject, "", True)
			
			if lPrevTabOrder >= 10 then
								
				lFoundPrev = this.dsObjects.Find("nhas_focus=1" , 1, this.dsObjects.RowCount()) //this.dsObjects.Find("nhas_taborder=" + String(lPrevTabOrder) , 1, this.dsObjects.RowCount())
				
				if lFoundPrev > 0 then
					// -----------------------------------------------------------------
					// Rechteck vorherige Column grau f$$HEX1$$e400$$ENDHEX$$rben
					// -----------------------------------------------------------------
					this.dsObjects.SetItem(lFoundPrev, "nhas_focus", 0)
					sRectangle = this.dsObjects.GetItemString(lFoundPrev, "srectangle")		
					this.Modify(sRectangle + ".pen.color='" + string(rgb(128,128,128))+ "'")
				end if
				
			end if
			
			this.dsObjects.SetItem(lFound, "nhas_focus", 1)
...
event losefocus;call super::losefocus;Long I
String sRectangle
...
if iMarkFocusedColumn = 1 then
	...
	for i = 1 to this.dsObjects.RowCount()
		...
		this.dsObjects.SetItem(i, "nhas_focus", 0)
		
		sRectangle = this.dsObjects.GetItemString(i, "srectangle")
		
		if not isnull(sRectangle) then
				// -----------------------------------------------------------------
				// Rechteck grau f$$HEX1$$e400$$ENDHEX$$rben
				// -----------------------------------------------------------------
				this.Modify(sRectangle + ".pen.color='" + string(rgb(128,128,128))+ "'")
				this.Modify(sRectangle + ".pen.width='1'")
		end if
