...
If sClientParm <> "" and sUnitParm <> "" Then
	...
	sFilter = "(upper(cinstance) = '" + upper(this.sInstanceName) + "' and (isNull(loc_jobs_once_cassigned_instance) or trim(loc_jobs_once_cassigned_instance)='' )) or" // Alle jobs f$$HEX1$$fc00$$ENDHEX$$r diese Instanz, die keiner anderen zugewiesen sind
	sfilter = sfilter + "(upper(loc_jobs_once_cassigned_instance) = '"+ upper(this.sInstanceName)+"')"  //Alle Jobs die manuell zugewiesen sind
	
	//loc_jobs_once_cassigned_instance
End if	
	...
	if iActiv = 0 Then
		...
		//			 of_generate_explosion()
			...
			case 180 //180 Tagesflugplan CCS (Tagesflugplan + Matdispo nur Level 1)
				sGenerierungsstatus 	= "Tagesflugplan CCS"
				iGenerierungsstatus 	= 1
				sGenerierungsstatus 	= "Insert"
				bReturn = of_generate_schedule(True) 
		...
		if  dsGenerateSchedule.getitemnumber(lrow,"naircraft_key")=0 then
			...
			if sqlca.sqlcode=0 then
				...
				of_write_protocol(iJob_Type,iGenerierungsstatus,2,iWarnung,arg_ds.getitemstring(arg_row, "cairline") + " " + string(arg_ds.getitemnumber(arg_row, "nflight_number")) + arg_ds.getitemstring(arg_row, "csuffix"),&
					...
					"Found no CBASE-AC-Type for AC " + ls_airline_owner_sky+" "+ls_actype+" Configuration: "+ ls_configuration_sky +" | Error Code : "+string( sqlca.sqlcode))
				return 0
			end if
		end if
