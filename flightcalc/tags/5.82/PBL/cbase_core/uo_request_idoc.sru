...
public function integer of_insert_generation (long arg_result, long arg_application, string sorderstatus, long arg_transaction, string arg_instance)
end prototypes
...
//  13.12.2013			  S.R.					CPRD Eintrag erlauben, wenn es noch keinen CPRD/CBIL Satz gibt.
//MB 16.02.2015: Immer nur auf result_key/transaction pr$$HEX1$$fc00$$ENDHEX$$fen. 
//IF arg_type=2 THEN
//	
//	ll_Queue_Rows = 0
//	select	count(*)
...
ll_Sequence = f_Sequence("seq_sky_idoc_request", sqlca)
	...
	Else
		...
		li_Succ = ids_request.SetItem(ll_NewRow, "nresult_key", arg_result)
		li_Succ = ids_request.SetItem(ll_NewRow, "napplication", arg_application)
		li_Succ = ids_request.SetItem(ll_NewRow, "corderstatus", '')
		li_Succ = ids_request.SetItem(ll_NewRow, "ntransaction", arg_transaction)
...
ll_Sequence = f_Sequence("seq_sky_idoc_request", sqlca)
	...
	Else
		...
		li_Succ = ids_request.SetItem(ll_NewRow, "corderstatus", sorderstatus)
		li_Succ = ids_request.SetItem(ll_NewRow, "ntransaction", arg_transaction)
		if trim(arg_instance) <>'' and not isNull(arg_instance) then
			li_Succ = ids_request.SetItem(ll_NewRow, "cinstance", arg_instance)
		end if
