...
for i = 1 to dsFlightsOnly.RowCount()
	...
	if ii_explode_plsql = 0 then
		...
		of_trace(1,10, "[of_start_for_airline] Rows in ids_prod_time_frame_flight: "+string(ids_prod_time_frame_flight.rowcount()))
	end if
...
if ii_explode_plsql = 0 then
	...
	ids_prod_time_frame_flight.retrieve(lResultKey, ld_calc_date)
	of_trace(1,10, "[of_start_for_flight] Rows in ids_prod_time_frame_flight: "+string(ids_prod_time_frame_flight.rowcount()))
end if
...
for j = 1 to dsCenOutMasterLevel1.RowCount()
	...
	if k < 1 then
		of_trace(1,1,"[of_generate_sub_pl_explosion_plsql] Processing item list: " + dsCenOutMasterLevel1.GetItemString(j, "cpackinglist") + " Return: "+string(k)+" Parameter: cbase_explosion.pf_explode_pl("+string(ll_index_key)+&
...
of_trace(1,10,"Start filling missing Fields for Level 1 for flight " + sAirlineCode + " " + string(lFlightNumber) + " , " + &
...
ii_explode_plsql = integer(of_cen_profilestring("OnlineExplosion","ExplodeWithPLSQL","0",is_Mandant))
if ii_explode_plsql >= 1 then
	of_trace(1,1, "[of_start_for_ccsjob] "+"Online Item Explosion being performed by PLSQL" )
end if
...
of_trace(1,1, "[of_start_for_ccsjob] "+"Start Packing List Explosion for one flight: Client = " + sClient + "; Unit = " + sPlant + "; Flight = " + sAirline  +" " + string(lFlightNumber)+"/"+string(dgenerationdate,"dd.mm.yy"))
...
if ii_explode_plsql = 0 then
	...
	ids_prod_time_frame.retrieve(sPlant, ld_calc_date, ls_calc_time, "%"+string(iDayCalc)+"%", lAirlineKey )
	of_trace(1,10, "[of_start_for_ccsjob] Rows in ids_prod_time_frame: "+string(ids_prod_time_frame.rowcount()))
	ids_prod_time_frame_flight.retrieve(lResultKey, ld_calc_date)
	of_trace(1,10, "[of_start_for_ccsjob] Rows in ids_prod_time_frame_flight: "+string(ids_prod_time_frame_flight.rowcount()))
end if
...
// =============================================================================
if iCNXFlag = 1 and iDoNotSet0ForCxFlights =0 then
	of_trace(1,1, "[of_start_for_ccsjob] "+"CNX-Flag ist gesetzt => Matdispo auf 0" )
	...
	if sqlca.sqlcode <> 0 then
		of_trace(1,1, "[of_start_for_ccsjob] "+"Error updating cen_out_master for CNX" )
		iErrorCode = 9
		return false
	end if
...
of_trace(1,10, "[of_start_for_ccsjob] "+"Packing List Explosion for flight " + sAirlineCode + " " + string(lFlightNumber) + ": ResultKey = " + string(lResultKey) )
...
if dsCenOutMasterLevel1.rowcount() =0 then
	of_trace(2,1, "[of_start_for_ccsjob] No Level 1 PL found" )
else
	of_trace(2,1, "[of_start_for_ccsjob] "+string(dsCenOutMasterLevel1.rowcount() )+" Level 1 PL found" )
	
	// Aufl$$HEX1$$f600$$ENDHEX$$sung Sub-Packinglists
	if ii_explode_plsql >= 1 then
		if of_generate_sub_pl_explosion_plsql(1) <> 0 then
			sErrortext = "Error generating Sub-Packing-List Explosion."
			of_trace(2,1, "[of_start_for_ccsjob] "+sErrortext )
		end if
	else
		for i=1 to dsCenOutMasterLevel1.rowcount()
...
// 14.08.2009 HR: Protokoll eingef$$HEX1$$fc00$$ENDHEX$$gt
of_trace(50,50,"[of_generate_additional_delivery] dsCenOutMaster.rowcount() vorher: "+string(dsCenOutMaster.rowcount()))
of_trace(50,50,"[of_generate_additional_delivery] dsCenOutAddDelivery.rowcount()  "+string(dsCenOutAddDelivery.rowcount()))

for i = 1 to dsCenOutAddDelivery.RowCount()
	// 14.08.2009 HR: Protokoll eingef$$HEX1$$fc00$$ENDHEX$$gt
	of_trace(50,50,"[of_generate_additional_delivery] "+dsCenOutAddDelivery.GetItemString(i,"cen_out_add_delivery_cpackinglist")+" "+string(dsCenOutAddDelivery.GetItemNumber(i,"cen_packinglists_npackinglist_index_key"))+" - BillStat: "+string(dsCenOutAddDelivery.GetItemNumber(i,"cen_out_add_delivery_nbilling") ))
	...
	if lFind > 0 then
		// 14.08.2009 HR: Protokoll eingef$$HEX1$$fc00$$ENDHEX$$gt
		of_trace(50,50,"[of_generate_additional_delivery]  -> QUANTITY ADDED")  
		...
		// 14.08.2009 HR: Protokoll eingef$$HEX1$$fc00$$ENDHEX$$gt
		of_trace(50,50,"[of_generate_additional_delivery] -> Insert New")
	end if
next
// 14.08.2009 HR: Protokoll eingef$$HEX1$$fc00$$ENDHEX$$gt
of_trace(50,50,"[of_generate_additional_delivery] dsCenOutMaster.rowcount() "+string(dsCenOutMaster.rowcount()))
return 0
