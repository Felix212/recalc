...
ElseIf li_PaxTypeLocal = 4 and lVersionGroupKey > 0 Then //betriebliche Prognose
...
Else
	s_departure.sTime			= dsReturnFlightSchedule.GetitemString(lschedulerow,"cdeparture_time")	
	s_departure.doffset		= dsReturnFlightSchedule.GetitemDecimal(lschedulerow,"ndeparture_faktor")
...
// --------------------------------------------------------------------------------------------------------------------
// 18.12.2009 HR: Wenn UTC VorLeg gr$$HEX2$$f600df00$$ENDHEX$$er UTC Depature, dann m$$HEX1$$fc00$$ENDHEX$$ssen wir einen Tag addieren
// 21.11.2011 HR: Auch gegen ARR-UTC pruefen
// --------------------------------------------------------------------------------------------------------------------
...
If lFlightScheduleTime = 1 Then
	...
	cdeparture_vorleg = sDepartureTimeUtc
end if

dsCenOut.SetItem(lRow,"ddeparture_time_utc",dtDepartureDateUtc)

// 1b. Abflugdatum/-zeit Local aus UTC 18.12.2009 HR
...
if sArrivalTime ='00:00' or trim(sArrivalTime)='' or isnull(sArrivalTime) then // 13.02.2012 HR: Hier auch pr$$HEX1$$fc00$$ENDHEX$$fen , ob Leer oder NULL ist
	sArrivalTime 				= string(f_dt_add(dtDepartureDateUtc, 60),"hh:mm")
	
	s_Arrival.sTime				= sArrivalTime
	s_Arrival.doffset			= nArrivalFaktor
	s_Arrival.ddate				= Date(f_dt_add(dtDepartureDateUtc, 60))
	
	s_Arrival						= f_utc_to_local_time(s_Arrival)
	
	sArrivalTime				= s_Arrival.sTime //Ankunftzeit LOC
end if

// 2. Ankunftszeit UTC als DateTime
// 29.12.2011 HR: $$HEX1$$dc00$$ENDHEX$$berarbeitet, da Ankunftszeit immer als UTC verwendet wurde und dadurch zu Fehlern beim Folgelegdatum kam

If lFlightScheduleTime = 1 Then

	s_departure.sTime		= sArrivalTime 
	s_departure.doffset		= nArrivalFaktor * -1 // Hier: Funktion umgekehrt rechnen local to UTC
	s_departure.ddate			= DATE( dtDepartureDateLoc)
	
	s_departure					= f_utc_to_local_time(s_departure)

	dtArrivalDateUtc			= DateTime(s_departure.dDate,Time(s_departure.sTime)) // Abflug DateTime UTC
else
	if sArrivalTime ='00:00' or trim(sArrivalTime)='' or isnull(sArrivalTime) then // 13.02.2012 HR: Hier auch pr$$HEX1$$fc00$$ENDHEX$$fen , ob Leer oder NULL ist
		sArrivalTime 			= string(f_dt_add(dtDepartureDateUtc, 60),"hh:mm")
	end if
...
s_Arrival.sTime				= string(dtArrivalDateUtc,"hh:mm")
s_Arrival.doffset			= nArrivalFaktor
...
if ii_resuse_resultkeys=1 then
	...
	a = dsCenOutCache.find("cflight_key = '"+ls_fk+"' and nreused = 0", 1, dsCenOutCache.rowcount())
	if a > 0 then
...
If lFlightScheduleTime = 1 Then
	...
	dNewGenerationDate		= dGenerationDate
Else
	s_departure.sTime		= dsGenerateSchedule.GetitemString(lschedulerow,"cdeparture_time")	
	s_departure.doffset		= dsGenerateSchedule.GetitemDecimal(lschedulerow,"ndeparture_faktor")
	s_departure.ddate			= dGenerationDate
	s_departure					= f_utc_to_local_time(s_departure)
	sDepartureTime			= s_departure.sTime
	dNewGenerationDate		= s_departure.dDate
	dReturnFlightDate			= s_departure.dDate // 21.11.2011 HR:
	//Messagebox("", "Drin")
...
For i = 1 to lRowcount
	...
	// definieren
	// -------------------------------------------------------------------------------------
	if sCurrentHandling = "STD" or sCurrentHandling = "DLV" or sCurrentHandling = "REQ" or sCurrentHandling = "SPC" then
...
// --------------------------------------------------------------------------------------------------------------------
of_reorg_legs()
of_trace("of_generate_schedule", 10,"Rows in dsCenOut after reorg_legs: "+string(dsCenOut.rowcount()))
...
of_trace("of_generate_schedule", 10,"Rows in dsCenOut after create handling: "+string(dsCenOut.rowcount()))
...
If sqlca.sqlcode = 0 then
	...
	If iJobParameter = 1 Then
		...
		if sHandlingShortText = "DLV" or sHandlingShortText = "SPC" then 
	...
	if k=1 then
		...
		for j=1 to dsExtCatSchedTemp.rowcount()
			if bCFSCheckOnly then
				...
				if ib_CFSWithoutFlightplan then
					// --------------------------------------------------------------------------------------------------------------------
					// 16.04.2015 HR: Bei der CFS-Ohne Flugplan-Generierung m$$HEX1$$fc00$$ENDHEX$$ssen wir die
					// --------------------------------------------------------------------------------------------------------------------
					k = lds_utc.find("ntlc_key = "+string(lTLCFrom), 1, lds_utc.rowcount())
					if k > 0 then
						nDepartureFaktor = lds_utc.getitemnumber(k, "ngmt")
					else
						nDepartureFaktor = 0
					end if
					
					k = lds_utc.find("ntlc_key = "+string(lTLCTo), 1, lds_utc.rowcount())
					if k > 0 then
						nArrivalFaktor = lds_utc.getitemnumber(k, "ngmt")
					else
						nArrivalFaktor = 0
					end if
				end if
...
long			lfind, l1, l2, l3, l4 , lfound, ll_max
string 		sFind, ls_flight, ls_type
...
if  dsCenOut.getitemnumber(arg_row,"nleg_number") > 1 then
	lds_handling_types.setfilter("nmulti = 1")
	ls_type = "Multi-Leg-Flight"
else
	lds_handling_types.setfilter("nsingle = 1")
	ls_type = "Single-Leg-Flight"
end if
lds_handling_types.filter()
...
ll_max = dsCenOutPackets.getitemnumber(1,"compute_maxleg")
...
// 07.07.2014 HR: CBASE-NAM-CR-14019: Pr$$HEX1$$fc00$$ENDHEX$$fung kann raus, es wird nach dem Kennzeichen (Single/Multuleg) an der ABART gefiltert
//	if lds_handling_types.getitemnumber(l1,"nmaxleg") <> ll_max then
//		of_trace("of_get_packet_handling", 10,""+ls_flight+": "+ lds_handling_types.Getitemstring(l1,"ctext")+" not posible. (HT"+string(lds_handling_types.getitemnumber(l1,"nmaxleg"))+"/F"+string(ll_max)+")")
//		continue
//	end if
	...
	for l2 = 1 to dsCenOutLos.rowcount()
		...
		if lfind=0 then 
			of_trace("of_get_packet_handling", 10,""+ls_flight+": "+ lds_handling_types.Getitemstring(l1,"ctext")+" not posible.")
			exit
		end if
	next
...
//			
//			dsCenOut.SetItem(l3,"NHANDLING_TYPE_KEY",lHandlingTypeKey)
//			dsCenOut.SetItem(l3,"CHANDLING_TYPE_TEXT",sHandlingShortText)
//			dsCenOut.SetItem(l3,"CHANDLING_TYPE_DESCRIPTION",sHandlingLongText)
//			
//			of_trace("of_get_packet_handling", 5,""+ls_flight+": Change Flight "+ string(dsCenOut.Getitemnumber(l3,"nflight_number")))
//		next
...
For i = 1 to dsHandling.Rowcount()
	...
	if  dsHandling.getitemnumber(i, "nuse_short_packets") = 1 then
		...
		if lds_sky.rowcount()>0 then
			...
			if lFind <> 1 then
				of_trace("of_generate_handling_meals", 1,arg_rowcount+"Error Copy Rows from lds_sky to dsHandlingMealCover: "+string(lFlightNumber)+" Handlingkey: "+string(lHandling_Key))
			end if
