...
// --------------------------------------------------------
// SPML's aus cen_out_spml / cen_out_spml_detail
// --------------------------------------------------------
datastore dsSPML
...
if this.dsDistributingMeals.RowCount() = 0 then
	...
	if this.dsMeals.RowCount() > 0  then
		...
		if not isnull(lCover) then
			...
			// --------------------------------------------
			// Verteilerroutine f$$HEX1$$fc00$$ENDHEX$$r der AC-Typ suchen
			// --------------------------------------------
			if sqlca.sqlcode = 0 then
...
String			sPackingList, sFilter, sCode, sDefault, sStowageGroup
boolean			bFound
Long				ll_Height
	...
	For lSizes = 1 to this.dsPackinglistSizeFind.RowCount()
		...
		if lNextLeg = 0 then
			...
			if lSeparate = 1 then
				...
				// -----------------------------------------------
//				ll_Height = this.dsPackinglistSizeFind.GetItemNumber(lSizes, "nheight") 
//				If ll_Height > 1 Then
//					ll_Height = ll_Height / 2 
//					lWidth = this.dsPackinglistSizeFind.GetItemNumber(lSizes, "nwidth")
//					dsPackinglistSizeTemp.SetItem(lSizes, "nheight", ll_Height)
//				Else
//					lWidth = this.dsPackinglistSizeFind.GetItemNumber(lSizes, "nwidth") / 2
//				End If
			...
			//	this.dsPackinglistSizeFind.SetItem(lSizes, "nheight", ll_Height)
				...
				if this.dsPackinglistSizeFind.RowsCopy(lSizes, lSizes, Primary!, dsPackinglistSizeTemp, dsPackinglistSizeTemp.RowCount() + 1, Primary!) <> 1 then
					...
					dsPackinglistSizeTemp.SetItem(dsPackinglistSizeTemp.RowCount(), "cmeal_control_code", String(lNextLeg) + sCode)
...
//			return -1
		...
		if lFound > 0 then
			...
			this.uoStowages[lCount].lDistributionAddKey = 	this.uoStowages[lCount].lDistributionMealKey
...
public function integer of_set_sort ();
String	sDistributionKey, sDistributionType
...
for lStowages = 1 to Upperbound(this.uoStowages)
	...
	for i = 1 to this.uoStowages[lStowages].dsContent.RowCount()
		...
		Else		
			...
			// 05.11.2012 cmeal_control_code2 ber$$HEX1$$fc00$$ENDHEX$$cksichtigen
			lFound = this.uoStowages[lStowages].dsPackinglistSize.Find(	"cmeal_control_code2='" + sDistributionKey + "' and ctext='" + &
														sDistributionType + "'", 1, &
														this.uoStowages[lStowages].dsPackinglistSize.RowCount())
		...
		DO
			...
			IF ll_rows_1 > 0 THEN
				...
				DO
					...
					ls_cmeal_control_code2		=		lds_cen_out_md_ps.GetItemString(ll_rownum_1,"cmeal_control_code2")
					// CBASE-CR-NAM-12050
