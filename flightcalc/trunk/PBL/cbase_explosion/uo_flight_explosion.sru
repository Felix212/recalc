...
// ---------------------------------
	...
	boolean	bmzv_loaded
	// --------------------------------------------------------------------------------------------------------------------------
	// 14.05.2025 HR: request #11290 New dynamic loading settings on loading lists on CBASEclassic
	// --------------------------------------------------------------------------------------------------------------------------
	boolean						ibGenerateLoading 
	DataStore					ds_pax
	DataStore					ds_handling
	
	// --------------------------------------------------------------------------------------------------------------------------
	 // 20.08.2025 HR: 
	// --------------------------------------------------------------------------------------------------------------------------
	boolean						ib_load_from_cenout = false
...
public function integer uf_dyn_loading (ref datastore rads_loading)
public function long uf_retrieve_detail ()
public function long uf_retrieve_unsaved (datastore arg_ds_pax, datastore arg_ds_handling, long arg_result_key, date arg_departure, string arg_deptime)
end prototypes
...
guoLog.uf_allways("["+ this.classname( )+".uf_retrieve] Start for lResultKey="+string(lResultKey)+",  lTransaction = "+string(lTransaction))

lStart = CPU()

if ibGenerateLoading then
	ds_pax.retrieve( lResultKey )	
	guoLog.uf_allways("["+ this.classname( )+".uf_retrieve] row in ds_pax: "+string(ds_pax.rowcount()))
	
	// 18.08.2025 HR: request #12288 Wir brauchen auch Werte vom Handling
	ds_handling.retrieve(lResultKey, 1)
	guoLog.uf_allways("["+ this.classname( )+".uf_retrieve] row in ds_handling: "+string(ds_handling.rowcount()))
	
	ib_load_from_cenout = true
	//dw_loading_list_result_flight
	
end if

return uf_retrieve_detail()
...
IF bmanueldistribution = False THEN
	...
	IF lb_ret = True THEN
		lEnde = CPU()
		guoLog.uf_allways("["+ this.classname( )+".uf_retrieve] Retrieve Loading took " + string((lEnde - lStart)/1000, "0.00") + " seconds!")
...
IF bmanueldistribution = True OR lb_ret = False THEN
	...
	IF ls_plsql_switch = "0" OR ls_plsql_switch = "COMPARE" THEN
		...
		// -------------------------------------------
		ll_rows = 0
		// Issue #4918 kein retrieve mit leerem Array
		If upperbound(lFirstArgRetrieve) > 0 Then
		...
		// ----------------------------------------------------------------------
		// #11831 Dynamic Loading - remove rows
		// ----------------------------------------------------------------------
		li_Check = uf_dyn_loading( ds_loadinglist)
		...
		FOR i = 1 TO Upperbound(lSecondArgRetrieve)
			...
			// #11831 Dynamic Loading - remove rows
			// ----------------------------------------------------------------------
			li_Check = uf_dyn_loading( ds_suplementloadingslist)
...
// 14.05.2025 HR: request #11290 New dynamic loading settings on loading lists on CBASEclassic
if lTransaction		= -1 then return true
...
IF ls_save_distrtibution = "1" THEN
	...
	guoLog.uf_allways("["+ this.classname( )+".uf_store_loading] Storing Loading ...")
	...
	IF ll_rows = 0 THEN
		...
		DO
			...
			ll_ncatering_leg                                 	=      ds_loadinglist.GetItemNumber(ll_rownum,"ncatering_leg")
			...
			lds_cen_out_md_lo.SetItem(ll_newrow,"ncatering_leg",ll_ncatering_leg)
...
// 14.05.2025 HR: request #11290 New dynamic loading settings on loading lists on CBASEclassic
if lTransaction		= -1 then return false
...
IF ls_save_distrtibution = "1" THEN
	...
	IF ll_rows > 0 THEN
		...
		guoLog.uf_allways("["+ this.classname( )+".uf_restore_loading] Restoring Loading ...")		
		...
		DO
			...
			ll_ncatering_leg              			=			lds_cen_out_md_lo.GetItemNumber(ll_rownum,"ncatering_leg")
			...
			ds_loadinglist.SetItem(ll_newrow,"ncatering_leg",ll_ncatering_leg)
...
// -----------------------------------------------------------------------------------------------------------------------------------------
// Objekt : uo_flight_explosion
// Methode: uf_dyn_loading (Function)
// Autor  :
// -----------------------------------------------------------------------------------------------------------------------------------------
// Argument(e):
// none
...
//  Datum    	     Version	Autor						Kommentar
// -----------------------------------------------------------------------------------------------------------------------------------------
//  14.05.2025	1.00		Oliver Hoefer              #11831 Dynamic Loading: remove rows where pax count is not in the range specified for Loading List
// -----------------------------------------------------------------------------------------------------------------------------------------
	

Long		ll_dynrow, ll_From, ll_To
Integer	li_Check
String		ls_Class, ls_PL
Long		ll_pax ,  lfind
String		ls_filter_leg, ls_filter_class, ls_filter_ll, ls_filter

 //npax_from  npax_to  cen_loadinglist_index_cloadinglist  cclass1  cclass2  cclass3 

if ibGenerateLoading then
	ls_filter_ll = ""
	
	FOR ll_dynrow = rads_Loading.RowCount() to 1 Step -1
		//Wenn keine DynLoad Zeile, dann zur n$$HEX1$$e400$$ENDHEX$$chsten
		if rads_Loading.getitemnumber(ll_dynrow, "onlyoneclass"  ) = 0 then continue

		ls_PL = rads_Loading.getitemstring( ll_dynrow, "cen_galley_cgalley") + " " + rads_Loading.getitemstring( ll_dynrow, "cen_stowage_cstowage") + rads_Loading.getitemstring( ll_dynrow, "cen_stowage_cplace") + " / " + rads_Loading.getitemstring( ll_dynrow, "cen_packinglist_index_cpackinglist")

		ll_From = rads_Loading.getitemnumber(ll_dynrow, "npax_from"  )
		ll_to = rads_Loading.getitemnumber(ll_dynrow, "npax_to"  )
					
		if rads_Loading.getitemstring(ll_dynrow, "cen_loadinglist_index_cloadinglist"  ) <>  ls_filter_ll then
			ls_filter_ll = rads_Loading.getitemstring(ll_dynrow, "cen_loadinglist_index_cloadinglist"  )
			ds_handling.setfilter("cloadinglist = '"+ ls_filter_ll +"' ")
			ds_handling.filter()
			
			ls_filter_leg = ""
			
			if ds_handling.rowcount() = 0 then
				guoLog.uf_error("["+ this.classname( )+".uf_dyn_loading] " + ls_filter_ll +" not found in ds_handling")
				continue
			end if
			
			if ds_handling.getitemnumber(1, "nll_leg1" ) = 1 then
				ls_filter_leg = "nleg_number = 1" 
			end if
			
			if ds_handling.getitemnumber(1, "nll_leg2" ) = 1 then
				if ls_filter_leg > " " then ls_filter_leg += " or "
				ls_filter_leg += "nleg_number = 2" 
			end if
			
			if ds_handling.getitemnumber(1, "nll_leg3" ) = 1 then
				if ls_filter_leg > " " then ls_filter_leg += " or "
				ls_filter_leg += "nleg_number = 3" 
			end if
			
			if ds_handling.getitemnumber(1, "nll_leg4" ) = 1 then
				if ls_filter_leg > " " then ls_filter_leg += " or "
				ls_filter_leg += "nleg_number = 4" 
			end if
			
			if ls_filter_leg = "" then 
				guoLog.uf_error("["+ this.classname( )+".uf_dyn_loading] " + ls_filter_ll +" has no relevant leg selected. Delete Row " + ls_PL )
				li_Check = rads_Loading.deleterow( ll_dynrow )
				continue
			end if
		end if
		
		//Keine Legs ausgew$$HEX1$$e400$$ENDHEX$$hlt, dann zur n$$HEX1$$e400$$ENDHEX$$chsten Zeile
		if ls_filter_leg = "" then 
				guoLog.uf_error("["+ this.classname( )+".uf_dyn_loading] " + ls_filter_ll +" has no relevant leg selected. Delete Row " + ls_PL )
				li_Check = rads_Loading.deleterow( ll_dynrow )
				continue
		...
		if rads_Loading.getitemstring(ll_dynrow, "cclass1"  ) > " " then
			ls_filter_class = "cclass = '"+ rads_Loading.getitemstring(ll_dynrow, "cclass1" ) +"' "
		end if
		
		if rads_Loading.getitemstring(ll_dynrow, "cclass2"  ) > " " then
			if ls_filter_class > " " then ls_filter_class += " or "
			ls_filter_class += "cclass = '"+ rads_Loading.getitemstring(ll_dynrow, "cclass2" ) +"' "
		end if
		
		if rads_Loading.getitemstring(ll_dynrow, "cclass3"  ) > " " then
			if ls_filter_class > " " then ls_filter_class += " or "
			ls_filter_class += "cclass = '"+ rads_Loading.getitemstring(ll_dynrow, "cclass3" ) +"' "
		end if
		
		if rads_Loading.getitemstring(ll_dynrow, "cclass4"  ) > " " then
			if ls_filter_class > " " then ls_filter_class += " or "
			ls_filter_class += "cclass = '"+ rads_Loading.getitemstring(ll_dynrow, "cclass4" ) +"' "
		end if
		
		//Keine Klasse ausgew$$HEX1$$e400$$ENDHEX$$hlt, dann zur n$$HEX1$$e400$$ENDHEX$$chsten Zeile
		if ls_filter_class = "" then 
				guoLog.uf_error("["+ this.classname( )+".uf_dyn_loading] " + ls_filter_ll +" has no relevant Class selected. Delete Row " + ls_PL )
				li_Check = rads_Loading.deleterow( ll_dynrow )
				continue
		end if
		
		ls_filter = "(" + ls_filter_leg+") and (" + ls_filter_class +")"
		
		ds_pax.setfilter(ls_filter)
		ds_pax.filter()
		ds_pax.groupcalc()
		
		guoLog.uf_debug("["+ this.classname( )+".uf_dyn_loading] Filter Pax (" + ls_filter + " => " + string( ds_pax.rowcount() ) +" rows")
		
		if ds_pax.rowcount() > 0 then
			ll_pax = ds_pax.getitemnumber(1, "comp_pax_sum_all")
		else
			ll_pax = 0
		end if
			
		IF ll_pax < ll_From OR ll_pax > ll_to THEN
			// remove row
			guoLog.uf_allways("["+ this.classname( )+".uf_dyn_loading] dynamic loading remove " + ls_PL + "  Pax: " + String(ll_pax) + " not fit in Range:" + String(ll_From) + " - " +  String(ll_To)  )
			li_Check = rads_Loading.deleterow( ll_dynrow )
		else
			guoLog.uf_debug("["+ this.classname( )+".uf_dyn_loading] dynamic loading is okay " + ls_PL + "  Pax: " + String(ll_pax) + " fit in Range:" + String(ll_From) + " - " +  String(ll_To)  )
		END IF
	next
else
	guoLog.uf_allways("["+ this.classname( )+".uf_dyn_loading] dynamic loading not active")
end if
...
public function long uf_retrieve_detail ();// --------------------------------------------------------------------------------------------------------------------
// Objekt : uo_flight_explosion
// Methode:  uf_retrieve_detail (Function)
// Autor  : Heiko Rothenbach
// --------------------------------------------------------------------------------------------------------------------
// Argument(e):
// ew Function
// --------------------------------------------------------------------------------------------------------------------
// Return: rows in ds_loading	
// --------------------------------------------------------------------------------------------------------------------
//  Beschreibung:  new function to enable call from different objects
//
// --------------------------------------------------------------------------------------------------------------------
//  Modifikationen:
//  Datum       Version  Autor          Kommentar
// --------------------------------------------------------------------------------------------------------------------
//  14.05.2025	   1.0      H.Rothenbach   Erstellung
//
// --------------------------------------------------------------------------------------------------------------------
...
IF bmanueldistribution = False THEN
	...
	IF lb_ret = True THEN
		lEnde = CPU()
		guoLog.uf_allways("["+ this.classname( )+".uf_retrieve_detail] Retrieve Loading from DB took " + string((lEnde - lStart)/1000, "0.00") + " seconds!")
		RETURN ds_loadinglist.Rowcount()
	END IF
END IF
...
IF bmanueldistribution = True OR lb_ret = False THEN
	...
	IF ls_plsql_switch = "0" OR ls_plsql_switch = "COMPARE" THEN
		...
		uf_get_schema()
		 ...
		 // 20.08.2025 HR:  #12288 Changes on Dynamic Loading for Phase 0 & 1
		if ib_load_from_cenout then
			ds_loadinglist.dataobject="dw_loading_list_result_flight"
			ds_loadinglist.settransobject(SQLCA)
			ds_loadinglist.Retrieve(lResultKey, dtDepartureDate, sDepartureTime, sVerkehrstag, lArgschema)
			
			if guolog.uf_get_loglevel() >= 5 then ds_loadinglist.saveas("C:\TEMP\CBASE\ds_loadinglist_"+string(lResultKey)+"_", string(f_jetzt(), "yyyymmdd-hhmmss")+".XLSX", XLSX!, TRUE)
			
			if ds_loadinglist.Rowcount() > 0 then
				lEnde = CPU()
				guoLog.uf_allways("["+ this.classname( )+".uf_retrieve_detail] Retrieve with cen_out_loading (rows: "+string(ds_loadinglist.Rowcount())+") took " + string((lEnde - lStart)/1000, "0.00") + " seconds!")
			
				RETURN ds_loadinglist.Rowcount()
			else
				guoLog.uf_allways("["+ this.classname( )+".uf_retrieve_detail] NO ROWS from cen_out_loading => Try OLD WAY ")
			end if
		end if
		// -------------------------------------------
		// Retrieve des Handling Loadinglist
...
lEnde = CPU()
guoLog.uf_allways("["+ this.classname( )+".uf_retrieve_detail] Retrieve Loading took " + string((lEnde - lStart)/1000, "0.00") + " seconds!")
...
public function long uf_retrieve_unsaved (datastore arg_ds_pax, datastore arg_ds_handling, long arg_result_key, date arg_departure, string arg_deptime);// -----------------------------------------------------------------------------------------------------------------------------------------
// Objekt : uo_flight_explosion
// Methode: uf_retrieve_unsaved (Function)
// Autor  :
// -----------------------------------------------------------------------------------------------------------------------------------------
// Argument(e):
// none
...
// Return: long
// -----------------------------------------------------------------------------------------------------------------------------------------
//  Beschreibung: New Function which is called from uo_generate and uo_pl_explosion
// -----------------------------------------------------------------------------------------------------------------------------------------
//  Modifikationen:
//  Datum    	     Version	Autor						Kommentar
// -----------------------------------------------------------------------------------------------------------------------------------------
...
Integer	i, iTrafficDay

lResultKey 				= arg_result_key
lTransaction				= -1
dtDepartureDate		= datetime(arg_departure, time("00:00"))
sDepartureTime		= arg_deptime
bmanueldistribution 	= true

iTrafficDay 				= DayNumber( arg_departure )
iTrafficDay --
if iTrafficDay = 0 then iTrafficDay = 7	
sVerkehrstag			= string(iTrafficDay)

if ibGenerateLoading then
	guoLog.uf_allways("["+ this.classname( )+".uf_retrieve_unsaved] copy " + string( arg_ds_pax.rowcount() ) +" Paxe")
	
	for i = 1 to arg_ds_pax.rowcount()
		ds_pax.insertrow(1)
		ds_pax.setitem(1, "cclass", 						arg_ds_pax.getitemstring(i, "cclass") )
		ds_pax.setitem(1, "npax", 							arg_ds_pax.getitemnumber(i, "npax") )

		// 18.08.2025 HR: request #12288 Weitere Werte m$$HEX1$$fc00$$ENDHEX$$ssen kopiert werden 
		ds_pax.setitem(1, "nresult_key_group", arg_ds_pax.getitemnumber(i, "nresult_key_group") )
		ds_pax.setitem(1, "nleg_number", 			arg_ds_pax.getitemnumber(i, "nleg_number") )
	next
	
	ds_handling.reset()
	if arg_ds_handling.rowcount()> 0 then
		if arg_ds_handling.rowscopy(1, arg_ds_handling.rowcount(), primary!, ds_handling, 1, primary!)<> 1 then
			guoLog.uf_error("["+ this.classname( )+".uf_retrieve_unsaved] Error Rowscopy arg_ds_handling("+arg_ds_handling.dataobject+") => ds_handling("+ds_handling.dataobject+")")
		end if
	end if
	
	for i = 1 to arg_ds_handling.rowcount()
		if arg_ds_handling.getitemnumber(i, "nhandling_id") = 1 then
			lFirstArgRetrieve[ upperbound(lFirstArgRetrieve) + 1 ] = arg_ds_handling.getitemnumber(i, "nloadinglist_index_key")
		else
			lSecondArgRetrieve[ upperbound(lSecondArgRetrieve) + 1 ] = arg_ds_handling.getitemnumber(i, "nloadinglist_index_key")
		end if
	next
	
	guoLog.uf_allways("["+ this.classname( )+".uf_retrieve_unsaved] Found  " + string( upperbound(lFirstArgRetrieve) ) +" Loadinglists and " + string( upperbound(lSecondArgRetrieve)  ) +" Supplements" )
...
// -------------------------------------
	...
	ds_pax = create DataStore 
	ds_pax.dataobject 	= "dw_cen_out_pax_all_legs"
	ds_pax.Settransobject(SQLCA)
	
	// --------------------------------------------------------------------------------------------------------------------
	// 18.08.2025 HR: request #12288 
	// --------------------------------------------------------------------------------------------------------------------
	ds_handling = create DataStore 
	ds_handling.dataobject 	= "dw_change_cen_out_handling"
	ds_handling.Settransobject(SQLCA)
end event

event destructor;destroy(ds_loadinglist)
destroy(ds_suplementloadingslist)
destroy(ds_pax) // 14.05.2025 HR: request #11290 New dynamic loading settings on loading lists on CBASEclassic
destroy(ds_handling) // 18.08.2025 HR: request #12288 
end event
