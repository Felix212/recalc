...
boolean		ib_account4goodsrecipient	// 26.07.2024 HR: AlmID 10606 activate column "Sales" in Item List Screen for MYPA tool

boolean		ib_full_explosion	= True  	// 02.06.2016 HR: CBASE-CR-2016-003

integer		iErrorCode 			= 9		// 02.03.2018 HR: Default Error Number
...
//
// =============================================================================================
uo_generate_datastore						dsLocalWorkstations		// lokale Bereiche f$$HEX1$$fc00$$ENDHEX$$r cen_out_master

uo_generate_datastore						dsFlightsOnly				// Alle Fl$$HEX1$$fc00$$ENDHEX$$ge f$$HEX1$$fc00$$ENDHEX$$r eine Unit

uo_generate_datastore						dsFlightsOfTheDay			// Alle Fl$$HEX1$$fc00$$ENDHEX$$ge f$$HEX1$$fc00$$ENDHEX$$r eine Unit inkl. 
																	// Pax-Zahlen

uo_generate_datastore						dsExplosionHandling		// Alle LL und SLL zu einem Flug

uo_generate_datastore						dsPackingList				// 

uo_generate_datastore						dsNewspaperLoading		// Alle Lesematerial LL zu einem Flug

uo_generate_datastore						dsNewspaperExtra			// Extrabeladung Lesematerial zu einem Flug

uo_generate_datastore						dsNewsLoadingList			// Inhalt einer Lesematerial LL

uo_generate_datastore						dsCenOutNFB					// NonFlightBusiness Auftr$$HEX1$$e400$$ENDHEX$$ge in cen_out_nfb

uo_generate_datastore						dsLoadingNFB				// Beladung NonFlightBusiness

uo_generate_datastore						dsSimulation				// f$$HEX1$$fc00$$ENDHEX$$r Simulation

uo_generate_datastore						dsCenOutSMPL				// SPML-Aufl$$HEX1$$f600$$ENDHEX$$sung
...
uo_generate_datastore 		dsForecast
uo_generate_datastore 		dsForecast_AS
...
integer				ii_batch							// 14.08.2018 TS: Batch-Flag
long					il_nworkschedule_index		// 02.02.2017 HR: nworkschedule_index auch merken
long					il_NPLTF_FLIGHT_INDEX_GROUP		// 02.02.2017 HR: NPLTF_FLIGHT_INDEX_GROUP auch merken
long 					il_npltf_flight_index			// 08.02.2017 HR: npltf_flight_index auch merken auch merken
date					id_prod_date					// 21.03.2013 HR: Produktionstag
uo_generate_datastore			ids_prod_time_frame			// 19.03.2013 HR: Datastore mit den Produktionszeitfenstern f$$HEX1$$fc00$$ENDHEX$$r St$$HEX1$$fc00$$ENDHEX$$cklisten
uo_generate_datastore			ids_prod_time_frame_flight	// 19.03.2013 HR: Datastore mit den Produktionszeitfenstern f$$HEX1$$fc00$$ENDHEX$$r St$$HEX1$$fc00$$ENDHEX$$cklisten
...
// --------------------------------------------------------------------------------------------------------------------
// 26.09.2016 HR: CR  CCS-CR-1541: Neue Werte sollen mit aufgeloest werden
// --------------------------------------------------------------------------------------------------------------------
long 					il_NSERVICE_SEQUENCE
string					is_CSAP_CODE
string					is_PROD_CAT_TEXT

// --------------------------------------------------------------------------------------------------------------------
// 09.03.2017 HR: UK-CR-1977: Aufl$$HEX1$$f600$$ENDHEX$$sung der Nachfahrten
// --------------------------------------------------------------------------------------------------------------------
boolean				ib_explode_additional_delivery
uo_generate_datastore			dsCenOutAddDelivery
...
public function integer of_fill_level1_fields ()
public function boolean of_start_for_ccsjob (long arg_resultkey)
public function integer of_get_account4goodsrecipient (long arg_customer_key, string arg_we, ref string arg_caccount, ref long arg_naccount_key, ref string arg_caccount2)
public function integer of_resolve_packinglist (long arg_index_key, decimal arg_master_quantity, integer arg_level, integer arg_reckoning, decimal arg_master_reserve, integer arg_master_percentage, long arg_master_area, long arg_master_ws, longlong arg_master_detail_key, longlong arg_ancester_detail_key, string arg_cclass, integer arg_nclass_number, string arg_csalesrel, string arg_cdefstoragelocation, string arg_cgoodsrecipient, string arg_caccount, long arg_naccount, decimal arg_master_quantity_vers, decimal arg_master_reserve_vers, long arg_nservice_sequence, string arg_csap_code)
public function integer of_generate_additional_delivery ()
public function string of_cen_profilestring (string ssection, string skey, string sdefault)
end prototypes
...
guoLog.uf_allways("["+ this.classname( )+".of_generate_extra_newspaper] Start newspaper extra handling.")
...
// --------------------------------------------------------------------------------------------------------------------
// 21.08.20176 CR 2314 Product Category Text in Explosion
// --------------------------------------------------------------------------------------------------------------------
dsCenOutDetail.SetItem(lRow,"CPROD_CAT_TEXT",is_PROD_CAT_TEXT)
...
// --------------------------------------------------------------------------------------------------------------------
// 26.09.2016 HR: CR CCS-CR-1541: Neue Werte sollen mit aufgeloesst werden
// --------------------------------------------------------------------------------------------------------------------
il_NSERVICE_SEQUENCE = 0
is_CSAP_CODE = ""
...
iFillForcastFields 		= integer(of_cen_profilestring("OnlineExplosion","FillForcastFields","0"))

// 06.06.2012 HR: Neuer Schalter f$$HEX1$$fc00$$ENDHEX$$r Gruppierung in CEN_OUT_MASTER
ii_GroupPlBySslZblAc2 = integer(of_cen_profilestring("OnlineExplosion","GroupPlBySslZblAc2","0"))

// 09.03.2012 HR:
ii_explode_plsql = integer(of_cen_profilestring("OnlineExplosion","ExplodeWithPLSQL","0"))
if ii_explode_plsql >= 1 then
	guoLog.uf_allways("["+ this.classname( )+".of_start_for_airline] Online Item Explosion being performed by PLSQL" )
end if
...
// --------------------------------------------------------------------------------------------------------------------
iDoNotSet0ForCxFlights = integer(of_cen_profilestring("OnlineExplosion","DoNotSet0ForCxFlights","0"))


guoLog.uf_allways("["+ this.classname( )+".of_start_for_airline] Start Packing List Explosion for Client = " + sClient + "; Unit = " + sPlant + &
...
guoLog.uf_allways("["+ this.classname( )+".of_start_for_airline] dsLocalWorkstations contains: " + String(dsLocalWorkstations.RowCount()) + " rows")
guoLog.uf_allways("["+ this.classname( )+".of_start_for_airline]       dsFlightsOfTheDay contains: " + String(dsFlightsOfTheDay.RowCount()) + " rows")
guoLog.uf_allways("["+ this.classname( )+".of_start_for_airline]       dsFlightsOnly contains: " + String(dsFlightsOnly.RowCount()) + " rows")

for i = 1 to dsFlightsOnly.RowCount()

	//21.10.2024 HR: Logging Job-Nr and Result_key to Graylog
	gll_log_result_key	= dsFlightsOnly.GetItemNumber(i,"cen_out_nresult_key") 
	...
	guoLog.uf_allways("["+ this.classname( )+".of_start_for_airline] Packing List Explosion for flight (" + string(i) + " of " + string(dsFlightsOnly.RowCount()) + ") " + sAirlineCode + " " + &
	...
	if ii_explode_plsql = 0 then
		...
		if f_unitprofilestring("Default","USE_RAMPTIME_FOR_PRODUCTION","0", sPlant)  = "0" then
			ld_calc_date = dgenerationdate
			ls_calc_time  = dsFlightsOnly.GetItemString(i,"cen_out_cdeparture_time") 
		else
			if dsFlightsOnly.GetItemString(i,"cramp_time") > dsFlightsOnly.GetItemString(i,"cen_out_cdeparture_time") then
				ld_calc_date = RelativeDate(dgenerationdate, -1)
			else
				ld_calc_date = dgenerationdate
			end if
			ls_calc_time  = dsFlightsOnly.GetItemString(i,"cramp_time") 
		end if
		    
		iDayCalc = DayNumber(ld_calc_date)
		iDayCalc --
		if iDayCalc = 0 then iDayCalc = 7
				
		ids_prod_time_frame.retrieve(sPlant, ld_calc_date, ls_calc_time, "%"+string(iDayCalc)+"%", lAirlineKey )
		guoLog.uf_allways("["+ this.classname( )+".of_start_for_airline] Rows in ids_prod_time_frame: "+string(ids_prod_time_frame.rowcount()))
		ids_prod_time_frame_flight.retrieve(lResultKey, ld_calc_date)
		guoLog.uf_allways("["+ this.classname( )+".of_start_for_airline] Rows in ids_prod_time_frame_flight: "+string(ids_prod_time_frame_flight.rowcount()))
	end if
	...
	if of_generate_loading(i) <> 0 then
		...
		guoLog.uf_error("["+ this.classname( )+".of_start_for_airline] "+ sErrortext + " [of_start_for_airline]" )
	end if
	...
	// 02.06.2016 HR: Es gibt eine neuen Job in der Generierung, der nur noch Level 1 aufl$$HEX1$$f600$$ENDHEX$$st
	if ib_full_explosion then
		if ii_explode_plsql >= 1 then
			if of_generate_sub_pl_explosion_plsql(i) <> 0 then
				sErrortext = "Error generating Sub-Packing-List Explosion."
				guoLog.uf_error("["+ this.classname( )+".of_start_for_airline] "+ sErrortext + " [of_start_for_airline]" )
			end if
		else
			if of_generate_sub_pl_explosion(i) <> 0 then
				sErrortext = "Error generating Sub-Packing-List Explosion."
				guoLog.uf_error("["+ this.classname( )+".of_start_for_airline] "+ sErrortext + " [of_start_for_airline]" )
			end if
		end if
	else
		of_fill_level1_fields()
	end if	
	// =============================================================================
	//
	...
	if of_write_loading() <> 0 then
		...
		// Fehler
		//
		guoLog.uf_error("["+ this.classname( )+".of_start_for_airline] Error of_write_loading(): " + sairlineCode + " " + string(lFlightNumber) + " " + &
...
if of_check_global_explosion() then
	guoLog.uf_allways("["+ this.classname( )+".of_start_for_airline] Info: No Packing List Explosion due to global job 110. [of_start_for_airline]")
	return true
end if
...
ii_explode_plsql = integer(of_cen_profilestring("OnlineExplosion","ExplodeWithPLSQL","0"))
if ii_explode_plsql >= 1 then
	guoLog.uf_allways("["+ this.classname( )+".of_start_for_nfb] Online Item Explosion being performed by PLSQL" )
end if


guoLog.uf_allways("["+ this.classname( )+".of_start_for_nfb] Start Packing List Explosion for Non-Flight-Business: Client = " + sClient + &
...
if of_generate_loading_nfb() <> 0 then
	...
	guoLog.uf_error("["+ this.classname( )+".of_start_for_nfb] "+ sErrortext )
	return false
end if
...
if of_check_global_explosion() then
	guoLog.uf_allways("["+ this.classname( )+".of_start_for_nfb] Info: No Packing List Explosion due to global job 110. [of_start_for_nfb]")
	return true
end if
...
guoLog.uf_allways("["+ this.classname( )+".of_start_packinglist_explosion] Start Packing List Explosion for Client = " + sClient + "! [of_start_packinglist_explosion]")
...
guoLog.uf_allways("["+ this.classname( )+".of_write_loading] Start Packing List Explosion Write")
 
if lcount > 0 then
   guoLog.uf_allways("["+ this.classname( )+".of_write_loading] Delete Packing List Explosion for flight " + sAirlineCode + " " + string(lFlightNumber) + &
   ...
   if sqlca.sqlcode <> 0 then
      guoLog.uf_allways("["+ this.classname( )+".of_write_loading] Error deleting Packing List Explosion for flight " + sAirlineCode + " " + string(lFlightNumber) + &
               "; result_key = " + string(lResultKey) )
   end if
end if

guoLog.uf_allways("["+ this.classname( )+".of_write_loading] Flight " + sAirlineCode + " " + string(lFlightNumber)+" INSERT STARTED Rows: "+string(dsCenOutMasterLevel1.rowcount()))
lStart = CPU()
if ii_explode_plsql <= 1 then
   if dsCenOutMasterLevel1.Update() <> 1 then
      ...
      guoLog.uf_error("["+ this.classname( )+".of_write_loading] Error writing to database table cen_out_master: code/row/text: " + string(lSQLCode) + "/" + &
...
elseif ii_explode_plsql  = 2 then // PLSQL-Bulkupdate
   ...
   guoLog.uf_allways("["+ this.classname( )+".of_write_loading] Starting PL/SQL Bulk Update")
   uo_bulk_insert luo_bulk
   luo_bulk = Create uo_bulk_insert   
   
   if luo_bulk.of_connect(sqlca) <> 0 then
      guoLog.uf_allways("["+ this.classname( )+".of_write_loading] PL/SQL Bulk Insert connection error: " + luo_bulk.sqlerrtext)
   else
      guoLog.uf_allways("["+ this.classname( )+".of_write_loading] PL/SQL Bulk Update Array initialize started")
      ll_ret = luo_bulk.of_load_arrays(dsCenOutMasterLevel1)
      if ll_ret <= 0 then
         guoLog.uf_allways("["+ this.classname( )+".of_write_loading] PL/SQL Bulk Insert found no rows to insert")
      else
         guoLog.uf_allways("["+ this.classname( )+".of_write_loading] PL/SQL Bulk Insert Array initialize done")
         ls_ret = luo_bulk.of_bulk_insert() 
         
         guoLog.uf_allways("["+ this.classname( )+".of_write_loading] PL/SQL Bulk Insert Message: " +ls_ret )
         if luo_bulk.SqlCode <> 0 then
            guoLog.uf_allways("["+ this.classname( )+".of_write_loading] PL/SQL Bulk Insert ERROR: " + luo_bulk.sqlerrtext )
            rollback;
         else
            guoLog.uf_allways("["+ this.classname( )+".of_write_loading] PL/SQL Bulk Insert DONE")
         end if
      end if
   end if
...
lStop = CPU()
guoLog.uf_allways("["+ this.classname( )+".of_write_loading] Flight " + sAirlineCode + " " + string(lFlightNumber)+" INSERT DONE " +string(lStop - lStart, "#,##0")+" msek")
...
guoLog.uf_allways("["+ this.classname( )+".of_write_loading] Packing List Explosion Write done")
return 0
...
guoLog.uf_allways("["+ this.classname( )+".of_write_loading_detail] Start writing details for Client = " + sClient + "; Unit = " + sPlant + "; Airline = " + sAirline )
...
if dsCenOutDetail.Update() <> 1 then
	...
	guoLog.uf_allways("["+ this.classname( )+".of_write_loading_detail] Error writing the result of the Packing List Explosion! (see xls output...) ")
	guoLog.uf_allways("["+ this.classname( )+".of_write_loading_detail] Error writing to database table cen_out_detail: code/row/text: " + string(lSQLCode) + "/" + &
					string(lSQLRow) + "/" + sSQLErrorText )
	rollback;
	return -1
end if
...
guoLog.uf_allways("["+ this.classname( )+".of_write_loading_detail] End writing details for Client = " + sClient + "; Unit = " + sPlant + "; Airline = " + sAirline )
...
ii_explode_plsql = integer(of_cen_profilestring("OnlineExplosion","ExplodeWithPLSQL","0"))
if ii_explode_plsql >= 1 then
	guoLog.uf_allways("["+ this.classname( )+".of_start_for_flight] "+"Online Item Explosion being performed by PLSQL" )
end if
...
guoLog.uf_allways("["+ this.classname( )+".of_start_for_flight] "+"Start Packing List Explosion for one flight: Client = " + sClient + "; Unit = " + sPlant + "; Flight = " + sAirline  +" " + string(lFlightNumber)+"/"+string(dgenerationdate,"dd.mm.yy"))
...
if ii_explode_plsql = 0 then
	...
	if f_unitprofilestring("Default","USE_RAMPTIME_FOR_PRODUCTION","0", sPlant)  = "0" then
		ld_calc_date = dgenerationdate
		ls_calc_time  = dsFlightsOnly.GetItemString(1,"cen_out_cdeparture_time") 
	else
		if dsFlightsOnly.GetItemString(1,"cramp_time") > dsFlightsOnly.GetItemString(1,"cen_out_cdeparture_time") then
			ld_calc_date = RelativeDate(dgenerationdate, -1)
		else
			ld_calc_date = dgenerationdate
		end if
		ls_calc_time  = dsFlightsOnly.GetItemString(1,"cramp_time") 
	end if
	...
	ids_prod_time_frame.retrieve(sPlant, ld_calc_date, ls_calc_time, "%"+string(iDayCalc)+"%", lAirlineKey )
	guoLog.uf_allways("["+ this.classname( )+".of_start_for_flight] Rows in ids_prod_time_frame: "+string(ids_prod_time_frame.rowcount()))
	ids_prod_time_frame_flight.retrieve(lResultKey, ld_calc_date)
	guoLog.uf_allways("["+ this.classname( )+".of_start_for_flight] Rows in ids_prod_time_frame_flight: "+string(ids_prod_time_frame_flight.rowcount()))
end if
...
if iCNXFlag = 1 and iDoNotSet0ForCxFlights =0 then
	guoLog.uf_allways("["+ this.classname( )+".of_start_for_flight] "+"CNX-Flag ist gesetzt => Matdispo auf 0" )
	...
	if sqlca.sqlcode <> 0 then
		guoLog.uf_allways("["+ this.classname( )+".of_start_for_flight] "+"Error updating cen_out_master for CNX" )
		iErrorCode = 9
		return false
	end if
...
guoLog.uf_allways("["+ this.classname( )+".of_start_for_flight] "+"Packing List Explosion for flight " + sAirlineCode + " " + string(lFlightNumber) + ": ResultKey = " + string(lResultKey) )
...
if of_generate_loading(1) <> 0 then
	sErrortext = "Error generating Loadinglist Explosion."
	guoLog.uf_error("["+ this.classname( )+".of_start_for_flight] "+sErrortext )
end if
...
guoLog.uf_allways("["+ this.classname( )+".of_generate_loading_newspaper] Start generating newspaper loading")

if not isvalid(uo_calc) then
	guoLog.uf_error("["+ this.classname( )+".of_generate_loading_newspaper] Error: uo_calc was not instanciated! ")
	return -1
end if
...
for i = 1 to dsNewspaperLoading.RowCount()
	...
	for j = 1 to dsNewsLoadingList.RowCount()
		...
		if lRatiolist_key > 0 then
			...
			if dsNewsLoadingList.GetItemNumber(j,"cen_news_ll_nquantity" + string(iDay)) = -1 then
				...
				if lquantity = -1 then
					...
					//
					guoLog.uf_error("["+ this.classname( )+".of_generate_loading_newspaper] Error calculating the newspaper loading: of_calc_ratiolist returns -1; lresultkey = " + string(lresultkey) )
				end if
			end if
		else
			//
		...
		if isnull(lquantity) then
			guoLog.uf_error("["+ this.classname( )+".of_generate_loading_newspaper] Error calculating the newspaper loading: lquantity is null; lresultkey = " + string(lresultkey))
			lquantity = 0
		end if
...
guoLog.uf_allways("["+ this.classname( )+".of_generate_loading_nfb] Start generating Non-Flight-Business!")
...
for i = 1 to dsCenOutNFB.RowCount()
	...
	if of_generate_sub_pl_explosion(1) <> 0 then
		...
		guoLog.uf_error("["+ this.classname( )+".of_generate_loading_nfb] "+sErrortext )
	end if
	...
	if of_write_loading() <> 0 then
		...
		// Fehler
		//
		guoLog.uf_error("["+ this.classname( )+".of_generate_loading_nfb] Error writing the loading: " + sairlineCode + " " + string(lFlightNumber) + " " + &
...
// ==========================================================================
long			i, j
uo_generate_datastore	dsDetail
integer		iCountToDo, iCountDone

guoLog.uf_allways("["+ this.classname( )+".of_generate_pl_explosion] Start explosion of the packing lists! Client = " + sClient + "; Unit = " + sPlant + & 
				"; Airline = " + sAirline + "; PL-Type = " + string(lPLType) )

//
// Detail-Datastore anlegen
//
dsDetail	= Create uo_generate_datastore
...
for i = 1 to dsCenOutMasterLevel1.RowCount()
	...
	if of_check_packinglist_explosion() > 0 then
		...
		guoLog.uf_debug( "["+ this.classname( )+".of_generate_pl_explosion] Info: No need for Packing List Explosion (index-key/detail-key): (" + &
							string(lPLIndexKey) + "/" + string(lPLDetailKey) + ") [of_generate_pl_explosion]")
		iCountDone++
		continue
	else
		guoLog.uf_debug( "["+ this.classname( )+".of_generate_pl_explosion] Info: Need a new Packing List Explosion (index-key/detail-key): (" + &
							string(lPLIndexKey) + "/" + string(lPLDetailKey) + ") [of_generate_pl_explosion]")
	...
	for j = 1 to dsDetail.RowCount()
		choose case lPLType
			case 1
				...
				is_PROD_CAT_TEXT        = dsDetail.GetItemString(j,"CPROD_CAT_TEXT")
		...
		if iPackinglistLevel = 3 then
			if of_insert_cen_out_detail() <> 0 then
				guoLog.uf_error("["+ this.classname( )+".of_generate_pl_explosion] Error writing the packing list details! Packing List Index Key = " + &
									string(lPLIndexKey))
			end if
...
guoLog.uf_debug( "["+ this.classname( )+".of_generate_pl_explosion] Info: Total amount of Packing Lists computed: " + string(iCountToDo) + &
...
guoLog.uf_allways("["+ this.classname( )+".of_generate_pl_explosion_for_one_flight] Start Packing List Explosion: Result-Key = " + string(lResultKey) )
...
for i = 1 to dsCenOutMasterLevel1.RowCount()
	lPLIndexKey		= dsCenOutMasterLevel1.GetItemNumber(i,"npl_index_key")
	lPLDetailKey	= dsCenOutMasterLevel1.GetItemNumber(i,"npl_detail_key")
	is_PROD_CAT_TEXT = dsCenOutMasterLevel1.GetItemString(i,"CPROD_CAT_TEXT")
	//
	// Nur/mindestens einmal t$$HEX1$$e400$$ENDHEX$$glich sollte eine Packinglist 
	// in cen_out_detail aktualisiert werden!
	//
	if of_check_packinglist_explosion() > 0 then
		...
		// Packinglist wurde schon aufgel$$HEX1$$f600$$ENDHEX$$st!
		//
		guoLog.uf_debug( "["+ this.classname( )+".of_generate_pl_explosion_for_one_flight] Info: No need for Packing List Explosion (index-key/detail-key): (" + &
							string(lPLIndexKey) + "/" + string(lPLDetailKey) + ")")
		iCountDone++
		continue
	else
		guoLog.uf_debug( "["+ this.classname( )+".of_generate_pl_explosion_for_one_flight] Info: Need a new Packing List Explosion (index-key/detail-key): (" + &
							string(lPLIndexKey) + "/" + string(lPLDetailKey) + ")")
		//
		// Anm.: Es gibt Packing Lists, die keine Inhalte haben und dadurch wieder
		//			und wieder eine Aufl$$HEX1$$f600$$ENDHEX$$sung ben$$HEX1$$f600$$ENDHEX$$tigen, da der count() auf cen_out_detail
	...
	if dsDetail.rowcount()>0 then // 13.09.2011 HR: Wie bisher
		...
		for j = 1 to dsDetail.RowCount()
			...
			if iPackinglistLevel = 3 then
				if of_insert_cen_out_detail() <> 0 then
					guoLog.uf_allways("["+ this.classname( )+".of_generate_pl_explosion_for_one_flight] Error at of_insert_cen_out_detail; Packing List Index Key = " + string(lPLIndexKey) )
				end if
...
//			end if
			...
			guoLog.uf_error( "["+ this.classname( )+".of_generate_pl_explosion_for_one_flight] Fehler Ermittlung Packinglistlevel f$$HEX1$$fc00$$ENDHEX$$r "+string(lPLIndexKey)+": "+sqlca.sqlerrtext)
		end if
	end if

next

guoLog.uf_debug( "["+ this.classname( )+".of_generate_pl_explosion_for_one_flight] Info: Total amount of Packing Lists computed: " + string(iCountToDo) + &
				"; amount of Packing Lists already computed: " + string(iCountDone) )
...
guoLog.uf_allways("["+ this.classname( )+".of_generate_loading] Start generation of loading" )
guoLog.uf_allways("["+ this.classname( )+".of_generate_loading] Parameters: lPLType = " + string(lPLType) + "; iDay for " + string(dgenerationdate) + " = " + string(iDay) )
...
if not isvalid(uo_loading4flight) then
	guoLog.uf_error("["+ this.classname( )+".of_generate_loading] Error: uo_loading4flight is not valid for lResultKey = " + string(lResultKey) + "  lTransaction = " + string(lTransaction) )
	return -1
end if

uo_loading4flight.ltransaction = -1 //lTransaction // 15.05.2025 HR: art #11831 Dynamic Loading Change 
uo_loading4flight.lresultkey = lResultKey //lTransaction // 21.05.2025 HR: art #11831 Dynamic Loading Change 
...
lMealExplosionWithBillingonly = long(of_cen_profilestring("OnlineExplosion","WithBillingOnly","0"))

// 09.03.2017 HR: UK-CR-1977: Aufl$$HEX1$$f600$$ENDHEX$$sung der Nachfahrten
ib_explode_additional_delivery = ( of_cen_profilestring("OnlineExplosion","ExplodeAdditionalDelivery","0") = "1" )

// 23.04.2009 TBR:
ilGroupPlByClass = long(of_cen_profilestring("OnlineExplosion","GroupPlByClass","0"))

// 17.09.2015 HR: Russland braucht die Klassen bei den LL
lbo_ll_with_class = ( of_cen_profilestring("OnlineExplosion","LL_With_Class","0") = "1")

// Leere Customer PL bef$$HEX1$$fc00$$ENDHEX$$llen
il_fillemptycustomerpl = Long(of_cen_profilestring("OnlineExplosion","FillEmptyCustomerPL","0"))
...
if uo_loading4flight.uf_retrieve() <= 0 then
	guoLog.uf_error("["+ this.classname( )+".of_generate_loading] uo_loading4flight.uf_retrieve(): " + sairlineCode + " " + string(lFlightNumber) + " " + " (key " + string(lResultKey) + "): " + uo_loading4flight.sStatusText  )
end if

//f_print_datastore(uo_loading4flight.ds_loadinglist)

//uo_loading4flight.ds_loadinglist.saveas("D:\LOADINGLISTINHALT_1.XLSX",XLSX!,TRUE)
...
for j = 1 to uo_loading4flight.ds_loadinglist.RowCount()
	...
	is_PROD_CAT_TEXT = uo_loading4flight.ds_loadinglist.GetItemString(j,"CPROD_CAT_TEXT")
	...
	elseif sSalesRel="1" then
		// 31.08.2016 HR: CBASEalm ID #1555:  Bei abweichenden WE muss der AC2 geleert werden
		of_get_Account4GoodsRecipient(il_customer_key, sGoodsRecipient, saccount, lAccount_Key, sAdditionalAccount)
	end if
	...
	if iFillForcastFields=1 and dsForecast.rowcount()>0 then
		...
		if lfind=0 then
			...
			guoLog.uf_debug( "["+ this.classname( )+".of_generate_loading] not found sFind = "+sFind )
		else
			...
			guoLog.uf_debug( "["+ this.classname( )+".of_generate_loading] found sFind = "+sFind )
	...
	sFind = "naction = 1 and npl_type = " + string(lpltype) + " and npl_index_key = " + &
				...
				" and NSERVICE_SEQUENCE = "+string(il_NSERVICE_SEQUENCE) /* 26.09.2016 HR: CCS-CR-1541 New Parameter passed through to the lowest item list level */ +&
				" and CSAP_CODE = '"+ is_CSAP_CODE+"'" /* 26.09.2016 HR: CCS-CR-1541 New Parameter passed through to the lowest item list level */ 
...
//dsCenOutMasterLevel1.saveas("D:\LOADINGLIST_dsCenOutMasterLevel1-1.XLSX",XLSX!,TRUE)
//
...
// 09.03.2017 HR: Wir l$$HEX1$$f600$$ENDHEX$$sen evtl auch Teile der Nachfahrt aus
if ib_explode_additional_delivery then of_generate_additional_delivery()
...
guoLog.uf_debug( "["+ this.classname( )+".of_generate_spml_explosion] dsCenOutMaster.rowcount() vorher: "+string(dsCenOutMaster.rowcount()))
guoLog.uf_debug( "["+ this.classname( )+".of_generate_spml_explosion] dsCenOutSMPL.rowcount(): "+string(dsCenOutSMPL.RowCount()))

for i = 1 to dsCenOutSMPL.RowCount()
	// 14.08.2009 HR: Protokoll eingef$$HEX1$$fc00$$ENDHEX$$gt
	guoLog.uf_debug( "["+ this.classname( )+".of_generate_spml_explosion] "+dsCenOutSMPL.GetItemString(i,"cpackinglist")+" "+string(dsCenOutSMPL.GetItemNumber(i,"npackinglist_index_key"))+" - BillStat: "+string(dsCenOutSMPL.GetItemNumber(i,"nbilling_status") ))
	//
	// 02.09.2005: Reine Abrechnungss$$HEX1$$e400$$ENDHEX$$tze werden ignoriert
	// nbilling_status = 2
	//
  ...
  SELECT cen_packinglists.cdef_storage_location  
	...
	if lFind > 0 then
		// 14.08.2009 HR: Protokoll eingef$$HEX1$$fc00$$ENDHEX$$gt
		guoLog.uf_debug( "["+ this.classname( )+".of_generate_spml_explosion]  -> QUANTITY ADDED")  
		...
		// 14.08.2009 HR: Protokoll eingef$$HEX1$$fc00$$ENDHEX$$gt
		guoLog.uf_debug( "["+ this.classname( )+".of_generate_spml_explosion] -> Insert New")

	end if
next

// 14.08.2009 HR: Protokoll eingef$$HEX1$$fc00$$ENDHEX$$gt
guoLog.uf_debug( "["+ this.classname( )+".of_generate_spml_explosion] dsCenOutMaster.rowcount() "+string(dsCenOutMaster.rowcount()))
...
// 14.08.2009 HR: Protokoll eingef$$HEX1$$fc00$$ENDHEX$$gt
guoLog.uf_debug( "["+ this.classname( )+".of_generate_meal_explosion] dsCenOutMaster.rowcount() vorher: "+string(dsCenOutMaster.rowcount()))
guoLog.uf_debug( "["+ this.classname( )+".of_generate_meal_explosion] dsCenOutMeals.rowcount()  "+string(dsCenOutMeals.rowcount()))

for i = 1 to dsCenOutMeals.RowCount()
	// 14.08.2009 HR: Protokoll eingef$$HEX1$$fc00$$ENDHEX$$gt
	guoLog.uf_debug( "["+ this.classname( )+".of_generate_meal_explosion] "+dsCenOutMeals.GetItemString(i,"cpackinglist")+" "+string(dsCenOutMeals.GetItemNumber(i,"npackinglist_index_key"))+" - BillStat: "+string(dsCenOutMeals.GetItemNumber(i,"nbilling_status") ))
	...
	if isnull(lPLDetailKey) then
		...
		guoLog.uf_allways("["+ this.classname( )+".of_generate_meal_explosion] npackinglist_detail_key is null in dsCenOutMeals for PL "+sPackinglist)
	else
		
		  
		  
		  SELECT cen_packinglists.cdef_storage_location  , cen_packinglists.cprod_cat
		 	INTO :sDefStorageLocation , :ls_Prod_Cat
		 	FROM cen_packinglists  
			WHERE ( cen_packinglists.npackinglist_index_key = :lPLIndexKey ) AND  
						( cen_packinglists.npackinglist_detail_key = :lPLDetailKey )  ;
	...
	if iFillForcastFields=1 and dsForecast.rowcount()>0 then
		...
		if lfind=0 then
			...
			guoLog.uf_debug( "["+ this.classname( )+".of_generate_meal_explosion] not found sFind = "+sFind )
		else
			il_FC_Parm1 = dsForecast.getitemnumber(lFind,"nparams_key1")
			il_FC_Parm2 = dsForecast.getitemnumber(lFind,"nparams_key2")
			il_FC_Parm3 = dsForecast.getitemnumber(lFind,"nparams_key3")

			guoLog.uf_debug( "["+ this.classname( )+".of_generate_meal_explosion] found sFind = "+sFind )
	...
	// 26.09.2016 HR: CR CCS-CR-1541: Neue Werte sollen mit aufgeloesst werden
	// --------------------------------------------------------------------------------------------------------------------
	il_NSERVICE_SEQUENCE	= dsCenOutMeals.GetItemNumber(i,"NSERVICE_SEQUENCE")
	is_CSAP_CODE				= dsCenOutMeals.GetItemString(i,"CSAP_CODE")

	if isnull(il_NSERVICE_SEQUENCE) then il_NSERVICE_SEQUENCE=0
	if isnull(is_CSAP_CODE) then is_CSAP_CODE=""
	
	is_PROD_CAT_TEXT = ""
	if NOT isnull(ls_Prod_Cat) then
		select ctext 
		into :is_PROD_CAT_TEXT
		from sys_product_category
		where csap_code = :ls_Prod_Cat;
		//sys_product_category.ctext sys_product_category.csap_code
	...
	if lFind > 0 then
		// 14.08.2009 HR: Protokoll eingef$$HEX1$$fc00$$ENDHEX$$gt
		guoLog.uf_debug( "["+ this.classname( )+".of_generate_meal_explosion]  -> QUANTITY ADDED")  
		...
		// 14.08.2009 HR: Protokoll eingef$$HEX1$$fc00$$ENDHEX$$gt
		guoLog.uf_debug( "["+ this.classname( )+".of_generate_meal_explosion] -> Insert New")
	end if
next
// 14.08.2009 HR: Protokoll eingef$$HEX1$$fc00$$ENDHEX$$gt
guoLog.uf_debug( "["+ this.classname( )+".of_generate_meal_explosion] dsCenOutMaster.rowcount() "+string(dsCenOutMaster.rowcount()))
return 0
...
if lcnt > 0 then
	...
	//
	guoLog.uf_allways("["+ this.classname( )+".of_check_global_explosion] Found global Packing List Explosion!")
	return true
else
	//
	// Es gibt keinen "globalen" Matdispo-Job, also 
	//
	guoLog.uf_allways("["+ this.classname( )+".of_check_global_explosion] Global Packing List Explosion not found!")
	return false
end if
...
long 				llaccount_key
// --------------------------------------------------------------------------------------------------------------------
// 23.09.2016 HR: CR CCS-CR-1541: Neue Werte sollen mit aufgeloesst werden
// --------------------------------------------------------------------------------------------------------------------
long 					ll_NSERVICE_SEQUENCE
string				ls_CSAP_CODE

guoLog.uf_allways("["+ this.classname( )+".of_generate_sub_pl_explosion] Start generate Sub-Packing-List-Explosion for flight " + sAirlineCode + " " + string(lFlightNumber) + " , " + &
               string(dgenerationdate) + "!")
...
for j = 1 to dsCenOutMasterLevel1.RowCount()
	...
	// 23.09.2016 HR: CR CCS-CR-1541: Neue Werte sollen mit aufgeloesst werden
	// --------------------------------------------------------------------------------------------------------------------
	ll_NSERVICE_SEQUENCE 		= dsCenOutMasterLevel1.GetItemNumber(j,"NSERVICE_SEQUENCE")
	ls_CSAP_CODE 						= dsCenOutMasterLevel1.GetItemString(j,"CSAP_CODE")

	is_PROD_CAT_TEXT = dsCenOutMasterLevel1.GetItemString(j,"CPROD_CAT_TEXT")

	if of_resolve_packinglist(lPLIndexKey, dcQuantity, 2, lReckoning, dcMasterReserve, iReservePercent, lMasterAreaKey, lMasterWorkstationKey, lMasterDetailKey, lAncesterDetailKey, ls_cclass, li_nclass_number, sSalesRel, sDefStorageLocation, sGoodsRecipient, lsAccount, llaccount_key, dcquantity_version, dcMasterReserve_version, ll_NSERVICE_SEQUENCE, ls_CSAP_CODE) <> 0 then
		return -1
	end if
next
...
for j = 1 to dsCenOutMasterLevelN.RowCount()
	...
	ii_batch					= dsCenOutMasterLevelN.GetitemNumber(j, "nbatch")
	...
	// 23.09.2016 HR: CR CCS-CR-1541: Neue Werte sollen mit aufgeloesst werden
	// --------------------------------------------------------------------------------------------------------------------
	ll_NSERVICE_SEQUENCE 		= dsCenOutMasterLevelN.GetItemNumber(j,"NSERVICE_SEQUENCE")
	ls_CSAP_CODE 						= dsCenOutMasterLevelN.GetItemString(j,"CSAP_CODE")

	is_PROD_CAT_TEXT = dsCenOutMasterLevelN.GetItemString(j,"CPROD_CAT_TEXT")
	...
	sFind = "npl_type = " + string(lPltype) + " and npl_index_key = " + string(lPLIndexKey) + &
				...
				" and cgoods_recipient ='"+sGoodsRecipient+"'" /* HR 05.09.2011 Findet erweitert um cgoods_recipient */ + &
				" and cpackinglist_xsl ='"+isPackinglistXsl+"'" /* 24.07.2013 HR: CBASE-DE-CR-2013-005: Speichern der Servicest$$HEX1$$fc00$$ENDHEX$$ckliste */ +&
				" and NSERVICE_SEQUENCE = "+string(ll_NSERVICE_SEQUENCE) /* 26.09.2016 HR: CCS-CR-1541 New Parameter passed through to the lowest item list level */ +&
				" and CSAP_CODE = '"+ ls_CSAP_CODE+"'" /* 26.09.2016 HR: CCS-CR-1541 New Parameter passed through to the lowest item list level */ 
...
//	
		...
		dsCenOutMasterLevel1.setitem(lRow,"nbatch", ii_batch)
		...
		// 26.09.2016 HR: CR CCS-CR-1541: Neue Werte sollen mit aufgeloesst werden
		// --------------------------------------------------------------------------------------------------------------------
		dsCenOutMasterLevel1.SetItem(lRow,"NSERVICE_SEQUENCE",ll_NSERVICE_SEQUENCE)
		dsCenOutMasterLevel1.SetItem(lRow,"CSAP_CODE",ls_CSAP_CODE)
		
		dsCenOutMasterLevel1.SetItem(lRow,"CPROD_CAT_TEXT",is_PROD_CAT_TEXT)
...
for j = 1 to dsCenOutMasterLevel1.RowCount()
	...
	if iConvertGtoKG = -1 then // Schalter noch nicht gelesen, dann erst mal lesen
		iConvertGtoKG = integer(of_cen_profilestring("OnlineExplosion","ConvertGtoKG","1")	)
	end if
...
uo_generate_datastore			dsLocUnitWS		// Texte f$$HEX1$$fc00$$ENDHEX$$r Bereiche
...
guoLog.uf_allways("["+ this.classname( )+".of_start_calculator] Client = " + sClient + "; Unit = " + sPlant + "; Airline = " + sAirline + "!")
...
dsLocUnitWS = create uo_generate_datastore
...
guoLog.uf_allways("["+ this.classname( )+".of_generate_loading_for_calculator] Start generation of loading" )
guoLog.uf_allways("["+ this.classname( )+".of_generate_loading_for_calculator] Parameters: lPLType = " + string(lPLType) + "; iDay for " + string(dgenerationdate) + " = " + string(iDay) )
...
if not isvalid(uo_loading4flight) then
	guoLog.uf_error("["+ this.classname( )+".of_generate_loading_for_calculator] Error generate loading: uo_loading4flight is not valid for lResultKey = " + string(lResultKey) + "  lTransaction = " + string(lTransaction) )
	return -1
end if
...
// 
if uo_loading4flight.uf_retrieve() <= 0 then
	guoLog.uf_error("["+ this.classname( )+".of_generate_loading_for_calculator] Error uo_loading4flight.uf_retrieve(): " + sairlineCode + " " + string(lFlightNumber) + "  (key " + string(lResultKey) + ")" )
	guoLog.uf_error("["+ this.classname( )+".of_generate_loading_for_calculator] Error uo_loading4flight.uf_retrieve(): " + uo_loading4flight.sStatusText )
end if
...
// --------------------------------------------------------------------------------------------------------------------
// 19.03.2013 HR: F$$HEX1$$fc00$$ENDHEX$$llen des Zeigers auf das Produktionszeitfensters
// 02.02.2017 HR: Erweiterung f$$HEX1$$fc00$$ENDHEX$$r UK
// --------------------------------------------------------------------------------------------------------------------
lfind = ids_prod_time_frame_flight.find("npackinglist_index_key = "+string(lPLIndexKey), 1, ids_prod_time_frame_flight.rowcount())
if lfind > 0 then
	setnull(il_pltimeframe_index)
	setnull(ii_batch)
	id_prod_date							= relativedate(dGenerationDate, -1 *   ids_prod_time_frame_flight.getitemnumber(lfind, "noffset"))
	il_nworkschedule_index				= ids_prod_time_frame_flight.getitemnumber(lfind, "nworkschedule_index")
	il_NPLTF_FLIGHT_INDEX_GROUP 	= ids_prod_time_frame_flight.getitemnumber(lfind, "NPLTF_FLIGHT_INDEX_GROUP")
	il_npltf_flight_index					= ids_prod_time_frame_flight.getitemnumber(lfind, "npltf_flight_index")
else
	setnull(il_NPLTF_FLIGHT_INDEX_GROUP)
	setnull(il_npltf_flight_index)
	...
	if lfind > 0 then
		...
		ii_batch								= ids_prod_time_frame.getitemnumber(lfind, "nbatch")
		id_prod_date						= relativedate(dGenerationDate, -1 *   ids_prod_time_frame.getitemnumber(lfind, "noffset"))
		il_nworkschedule_index 			= ids_prod_time_frame.getitemnumber(lfind, "nworkschedule_index")
	else
		setnull(il_pltimeframe_index)
		setnull(ii_batch)
		setnull(id_prod_date)
		setnull(il_nworkschedule_index)
	end if
end if

dsCenOutMaster.setitem(lRow,"npltimeframe_index", il_pltimeframe_index)
dsCenOutMaster.setitem(lRow,"nbatch", ii_batch)
dsCenOutMaster.setitem(lRow,"dprod_date", id_prod_date)
dsCenOutMaster.setitem(lRow,"nworkschedule_index", il_nworkschedule_index)
dsCenOutMaster.setitem(lRow,"NPLTF_FLIGHT_INDEX_GROUP", il_NPLTF_FLIGHT_INDEX_GROUP)
dsCenOutMaster.setitem(lRow,"npltf_flight_index", il_npltf_flight_index)
...
// 26.09.2016 HR: CR CCS-CR-1541: Neue Werte sollen mit aufgeloesst werden
// --------------------------------------------------------------------------------------------------------------------
dsCenOutMaster.setitem(lRow,"NSERVICE_SEQUENCE", il_NSERVICE_SEQUENCE)
dsCenOutMaster.setitem(lRow,"CSAP_CODE", is_CSAP_CODE)

dsCenOutMaster.SetItem(lRow,"CPROD_CAT_TEXT",is_PROD_CAT_TEXT)
...
DO WHILE lbo_loop
	...
	ELSEIF 	sqlca.sqlcode <> 0 THEN
				guoLog.uf_error("["+ this.classname( )+".of_get_class_number] DB-Error bei SELECT NCLASS_NUMBER FROM CEN_CLASS_NAMES: " + String(sqlca.sqlcode) + " " + sqlca.sqlerrtext )
				RETURN FALSE
...
if sqlca.sqlcode<>0 then
	a = 'n/a'
	guoLog.uf_allways("["+ this.classname( )+".of_get_goodsrecipient] SAP_CODE not found  for Airlinekey:"+string(l)+" ACCOUNT "+b)
end if
...
ii_explode_plsql = integer(of_cen_profilestring("OnlineExplosion","ExplodeWithPLSQL","0"))
if ii_explode_plsql >= 1 then
	guoLog.uf_allways("["+ this.classname( )+".of_start_for_postingbrowser] "+"Online Item Explosion being performed by PLSQL" )
end if
...
guoLog.uf_allways("["+ this.classname( )+".of_start_for_postingbrowser] Start Packing List Explosion for posting Flight: Client = " + sClient + "; Unit = " + sPlant + "; Airline = " + sAirline + "!")
...
// =============================================================================
if iCNXFlag = 1 and iDoNotSet0ForCxFlights =0 then
	guoLog.uf_allways("["+ this.classname( )+".of_start_for_postingbrowser] CNX-Flag ist gesetzt => Matdispo auf 0" )
	...
	if sqlca.sqlcode <> 0 then
		guoLog.uf_allways("["+ this.classname( )+".of_start_for_postingbrowser] Error updating cen_out_master for CNX" )
		iErrorCode = 9
		return false
	end if
...
guoLog.uf_allways("["+ this.classname( )+".of_start_for_postingbrowser] Packing List Explosion for Posting-flight " + f_check_null(sAirlineCode) + " " + &
...
ids_prod_time_frame_flight.reset()
...
if of_generate_psr_contents(1) <> 0 then
	...
	guoLog.uf_error("["+ this.classname( )+".of_start_for_postingbrowser] "+ sErrortext )
end if
...
guoLog.uf_allways("["+ this.classname( )+".of_generate_psr_contents] Start generation of loading" )
guoLog.uf_allways("["+ this.classname( )+".of_generate_psr_contents] Parameters: lPLType = " + string(lPLType) + "; iDay for " + string(dgenerationdate) + " = " + string(iDay) )
...
if lds.Retrieve(lResultKey) <= 0 then
	guoLog.uf_error("["+ this.classname( )+".of_generate_psr_contents] Error Contens-Retrieve: " + sairlineCode + " " + string(lFlightNumber) + " " + &
...
for j = 1 to lds.RowCount()
	...
	if iFillForcastFields=1 and dsForecast.rowcount()>0 then
		...
		if lfind=0 then
			...
			guoLog.uf_debug( "["+ this.classname( )+".of_generate_psr_contents] not found sFind = "+sFind )
		else
			il_FC_Parm1 = dsForecast.getitemnumber(lFind,"nparams_key1")
			il_FC_Parm2 = dsForecast.getitemnumber(lFind,"nparams_key2")
			il_FC_Parm3 = dsForecast.getitemnumber(lFind,"nparams_key3")

			guoLog.uf_debug( "["+ this.classname( )+".of_generate_psr_contents] found sFind = "+sFind )
...
//  21.08.20176   1.3      O.Hoefer       CR 2314 Product Category Text in Explosion
// --------------------------------------------------------------------------------------------------------------------

uo_generate_datastore 	lds_explosion 
uo_generate_datastore 	lds_explosion_container
long     		j,i,k,a, lcheck
...
string			ls_CSAP_CODE
String			ls_PROD_CAT_TEXT
integer       	li_replacement
string 		sfind
long 			lFind
...
lds_explosion = create uo_generate_datastore 
lds_explosion.dataobject = "dw_plsql_explode"
lds_explosion.setTransObject(SQLCA)

lds_explosion_container = create uo_generate_datastore 
...
if f_mandant_profilestring( SQLCA, s_app.smandant, 'OnlineExplosion', 'DontReplaceInExplosion', '1') = "0" then
	select count(*) into :li_replacement from LOC_PL_SUBST_FLIGHTS where nresult_key = :lResultKey;
	if li_replacement > 0 then
		
		delete from cen_out_detail where NPL_DETAIL_KEY = -1 * :lResultKey ;
		
		dsCenOutDetail.reset()
		
		li_replacement = 1 
		
		guoLog.uf_info("["+ this.classname( )+".of_generate_sub_pl_explosion_plsql] Flight has ITM/IL Substitutions (delete " + string(sqlca.sqlnrows)+" rows from CEN_OUT_DETAIL (FlightReplacements))" )
	end if
else
	li_replacement = 0 
	guoLog.uf_info("["+ this.classname( )+".of_generate_sub_pl_explosion_plsql] DontReplaceInExplosion = 1 ")
end if
...
//dsCenOutMasterLevel1.saveas("D:\TESTXLS\TEST-VOR.XLSX",XLSX!,TRUE)
...
for j = 1 to dsCenOutMasterLevel1.RowCount()
	...
	// --------------------------------------------------------------------------------------------------------------------
	// 26.09.2016 HR: CCS-CR-1541 New Parameter passed through to the lowest item list level
	// --------------------------------------------------------------------------------------------------------------------
	ll_SERVICE_SEQUENCE	= dsCenOutMasterLevel1.GetItemNumber(j, "NSERVICE_SEQUENCE")
	ls_CSAP_CODE				= dsCenOutMasterLevel1.GetItemString(j, "CSAP_CODE") 

	// --------------------------------------------------------------------------------------------------------------------
	// 21.08.20176 CR 2314 Product Category Text in Explosion
	// --------------------------------------------------------------------------------------------------------------------
	ls_PROD_CAT_TEXT		= dsCenOutMasterLevel1.GetItemString(j, "CPROD_CAT_TEXT") 
	...
	// 28.12.2023 HR: add li_replacement
	k = lds_explosion.retrieve(ll_index_key, dGenerationDate, 1, sPlant, lAirlineKey,  dcQuantity, dcquantity_version, lMealExplosionWithBillingonly, ll_reckoning, string(tdepartureTime, "hh:mm"), &
	                                      iCNXFlag, iConvertGtoKG, iDoNotSet0ForCxFlights, ls_sales_rel, ls_def_storage_location, ls_goods_recipient, ll_status, llaccount_key, lsAccount, il_FillEmptyCustomerPL, lResultKey, li_replacement )
	...
	if k < 1 then
		guoLog.uf_error("["+ this.classname( )+".of_generate_sub_pl_explosion_plsql] Processing item list: " + dsCenOutMasterLevel1.GetItemString(j, "cpackinglist") + " Return: "+string(k)+" Parameter: cbase_explosion.pf_explode_pl("+string(ll_index_key)+&
		                   ", '"+string(dGenerationDate, "dd.mm.yyyy")+"', 1, '"+sPlant+"', "+string(lAirlineKey)+", "+string(dcQuantity)+", "+string(dcquantity_version)+", "+string(lMealExplosionWithBillingonly)+", "+string(ll_reckoning)+", '"+&
						 string(tdepartureTime, "hh:mm")+"', " + string(iCNXFlag)+", "+string(iConvertGtoKG)+", "+string(iDoNotSet0ForCxFlights)+", '"+ls_sales_rel+"', '"+ls_def_storage_location+"', '"+ls_goods_recipient+"', "+&
						 string(ll_status)+", "+string(llaccount_key)+", '"+lsAccount+"', "+string(il_FillEmptyCustomerPL)+", "+string(lResultKey) +", "+string(li_replacement)+");") 
	end if
	
//	lds_explosion.saveas("D:\TESTXLS\TEST-"+string(j,"00000")+"-RETURN.XLSX",XLSX!,TRUE)
	guoLog.uf_debug("["+ this.classname( )+".of_generate_sub_pl_explosion_plsql] Processing item list: " + dsCenOutMasterLevel1.GetItemString(j, "cpackinglist") + " record  " + string(j) + " of " + string(dsCenOutMasterLevel1.RowCount())+": Rows "+string(k))
	
	for k = 1 to lds_explosion.RowCount()
		
		if lds_explosion.getItemNumber(k,"npackinglist_key") = 3 and lds_explosion.getItemNumber(k,"nlevel") > 1 then
			// 28.12.2023 HR: Copy ITMs to CEN_OUT_DETAIL
			if lds_explosion.getItemstring(k,"cpackinglist_ori") <> lds_explosion.getItemstring(k,"cpackinglist") then
				guoLog.uf_debug("["+ this.classname( )+".of_generate_sub_pl_explosion_plsql] copy artikel to CEN_OUT_DETAIL ("+ lds_explosion.getItemstring(k,"cpackinglist_ori")+" => "+ lds_explosion.getItemstring(k,"cpackinglist")+")")
				a =  lds_explosion.getItemNumber(k,"nancestor_pl_detail_key")
				
				lds_explosion.setItem(k,"nancestor_pl_detail_key", -1 * lResultKey )
				//Erst einmal setzen wir alle DetailKeys (SL + Ancestor) auf -1 * lResultKey 
				for i=1 to lds_explosion.RowCount()
					if i = k then continue
					
					if lds_explosion.getItemNumber(i, "npackinglist_index_key") =  lds_explosion.getItemNumber(k,"nancestor_pl_index_key") and &
						lds_explosion.getItemNumber(i, "npackinglist_detail_key") =  a then
						lds_explosion.setItem(i, "npackinglist_detail2_key", a )		
						lds_explosion.setItem(i, "npackinglist_detail_key", -1 * lResultKey )
					end if
					
					if lds_explosion.getItemNumber(i, "nancestor_pl_index_key") =  lds_explosion.getItemNumber(k,"nancestor_pl_index_key") and &
						lds_explosion.getItemNumber(i, "nancestor_pl_detail_key") =  a then
						lds_explosion.setItem(i, "nancestor_pl_detail_key", -1 * lResultKey )
					end if
				next	
				
				for i = 1 to lds_explosion_container.rowcount()
					if lds_explosion_container.getItemNumber(i, "npl_index_key") =  lds_explosion.getItemNumber(k,"nancestor_pl_index_key") and &
						lds_explosion_container.getItemNumber(i, "npl_detail_key") =  a then
						lds_explosion_container.setItem(i, "npl_detail2_key", a )		
						lds_explosion_container.setItem(i, "npl_detail_key", -1 * lResultKey )
					end if
				next
				
				for i = 1 to lds_explosion.RowCount()
					if lds_explosion.getItemNumber(i, "nlevel") <>  lds_explosion.getItemNumber(k,"nlevel") then continue
					if lds_explosion.getItemNumber(i, "npackinglist_key") <> 3 then continue
					if lds_explosion.getItemNumber(i, "nancestor_pl_index_key") <>  lds_explosion.getItemNumber(k, "nancestor_pl_index_key") then continue
					
					sFind = "npl_index_key = " + string(lds_explosion.getItemNumber(k, "nancestor_pl_index_key") ) 
					sFind += " and npl_detail_key = " + string(lds_explosion.getItemNumber(k, "nancestor_pl_detail_key") )
					sFind += " and nitem_index_key = " + string(lds_explosion.getItemNumber(i, "npackinglist_index_key") ) 
					sFind += " and nitem_detail_key = " + string(lds_explosion.getItemNumber(i, "npackinglist_detail_key") ) 
					
					lfind = dsCenOutDetail.find(sFind, 1, dsCenOutDetail.rowcount())
					
					if lfind = 0 then
						guoLog.uf_debug("["+ this.classname( )+".of_generate_sub_pl_explosion_plsql] Create Item " +  lds_explosion.getItemstring(i, "cpackinglist"))
						
						lSequence = f_Sequence_ll("seq_cen_out_detail", sqlca)
						If lSequence = -1 Then
							return -1
						end if
						...
						lRow = dsCenOutDetail.InsertRow(0)
						dsCenOutDetail.SetItem(lRow,"ndetail_key",					lSequence)
						dsCenOutDetail.SetItem(lRow,"ntransaction", 				0)
						dsCenOutDetail.SetItem(lRow,"npl_type",					1)
						dsCenOutDetail.SetItem(lRow,"npl_index_key",				lds_explosion.getItemNumber(k, "nancestor_pl_index_key"))
						dsCenOutDetail.SetItem(lRow,"npl_detail_key",				lds_explosion.getItemNumber(k, "nancestor_pl_detail_key"))
						dsCenOutDetail.SetItem(lRow,"nitem_index_key",			lds_explosion.getItemNumber(i, "npackinglist_index_key"))
						dsCenOutDetail.SetItem(lRow,"nitem_detail_key",			lds_explosion.getItemNumber(i, "npackinglist_detail_key"))
						dsCenOutDetail.SetItem(lRow,"citem_pl",					lds_explosion.getItemstring(i, "cpackinglist") )
   						dsCenOutDetail.SetItem(lRow,"citem_text",					lds_explosion.getItemstring(i, "ctext") )
						dsCenOutDetail.SetItem(lRow,"citem_add_on_text",		"")
						dsCenOutDetail.SetItem(lRow,"cunit",							lds_explosion.getItemstring(i, "cunit") )
						dsCenOutDetail.SetItem(lRow,"ccustomer_pl",				lds_explosion.getItemstring(i, "ccustomer_pl") )
						dsCenOutDetail.SetItem(lRow,"ccustomer_pl_text",		lds_explosion.getItemstring(i, "ccustomer_text") )
						
						if lds_explosion.getItemstring(i, "cunit") = "GR" then
							dsCenOutDetail.SetItem(lRow,"nquantity",				lds_explosion.getItemNumber(i, "nquantity") / 1000 )	
							dsCenOutDetail.SetItem(lRow,"cunit",						"KG")
						elseif lds_explosion.getItemstring(i, "cunit") = "ML" then
							dsCenOutDetail.SetItem(lRow,"nquantity",				lds_explosion.getItemNumber(i, "nquantity") / 1000 )	
							dsCenOutDetail.SetItem(lRow,"cunit",						"LT")
						else
							dsCenOutDetail.SetItem(lRow,"nquantity",				lds_explosion.getItemNumber(i, "nquantity"))	
						end if

						dsCenOutDetail.SetItem(lRow,"nsort",						lds_explosion.getItemNumber(i, "nsort"))
						dsCenOutDetail.SetItem(lRow,"dtimestamp",				dtnow)
						dsCenOutDetail.SetItem(lRow,"nstatus",						0)
						dsCenOutDetail.SetItem(lRow,"nfreq_key",					0)
						dsCenOutDetail.SetItem(lRow,"nreckoning",					lds_explosion.getItemNumber(i, "nreckoning") )
						dsCenOutDetail.SetItem(lRow,"nmaterial_index_key",		lds_explosion.getItemNumber(i, "nmaterial_index_key") )
						dsCenOutDetail.SetItem(lRow,"citem_text_short",			lds_explosion.getItemstring(i, "ctext_short") )
						dsCenOutDetail.SetItem(lRow,"csales_rel", 					lds_explosion.getItemstring(i, "csales_rel") )
						dsCenOutDetail.SetItem(lRow,"cgoods_recipient", 			lds_explosion.getItemstring(i, "cgoods_recipient") )
						
						dsCenOutDetail.SetItem(lRow,"nitem_index_key_ori",		lds_explosion.getItemNumber(i, "npackinglist_index_key_ori") )
						dsCenOutDetail.SetItem(lRow,"nitem_detail_key_ori",		lds_explosion.getItemNumber(i, "npackinglist_detail_key_ori") )
						dsCenOutDetail.SetItem(lRow,"citem_pl_ori", 				lds_explosion.getItemstring(i, "cpackinglist_ori") )

						//dsCenOutDetail.SetItem(lRow,"CPROD_CAT_TEXT",is_PROD_CAT_TEXT)
//					                                        citem_text_short      cprod_cat_text  npl_detail2_key  
					else
						guoLog.uf_debug("["+ this.classname( )+".of_generate_sub_pl_explosion_plsql] Found Item " +  lds_explosion.getItemstring(i, "cpackinglist") +" lfind = " +string(lfind))
					end if
				next
			end if
			continue
		...
		sFind = "npl_type = " + string(lPltype) + " and npl_index_key = " + string(lds_explosion.getItemNumber(k,"npackinglist_index_key")) + &
				...
				" and cpackinglist_xsl ='"+isPackinglistXsl+"'" /* 24.07.2013 HR: CBASE-DE-CR-2013-005: Speichern der Servicest$$HEX1$$fc00$$ENDHEX$$ckliste */ +&
				" and NSERVICE_SEQUENCE = "+string(ll_SERVICE_SEQUENCE) /* 26.09.2016 HR: CCS-CR-1541 New Parameter passed through to the lowest item list level */ +&
				" and CSAP_CODE = '"+ ls_CSAP_CODE+"'" /* 26.09.2016 HR: CCS-CR-1541 New Parameter passed through to the lowest item list level */ 
		...
		if lFind > 0 then
			...
			lds_explosion_container.setitem(a,"cprod_cat_text", 					lds_explosion.getItemString(k,"cprod_cat_text"))  
			...
			// --------------------------------------------------------------------------------------------------------------------
			// 19.03.2013 HR: F$$HEX1$$fc00$$ENDHEX$$llen des Zeigers auf das Produktionszeitfensters
			// 02.02.2017 HR: Zwei neue Felder m$$HEX1$$fc00$$ENDHEX$$ssen mit gespeichert werden
			// --------------------------------------------------------------------------------------------------------------------
			lds_explosion_container.setitem(a,"npltimeframe_index", 			lds_explosion.getItemNumber(k,"npltimeframe_index"))
			lds_explosion_container.setitem(a,"nbatch", 							lds_explosion.getItemNumber(k,"nbatch"))
			lds_explosion_container.setitem(a,"dprod_date", 						lds_explosion.getItemDatetime(k,"dprod_date"))
			lds_explosion_container.setitem(a,"nworkschedule_index", 			lds_explosion.getItemNumber(k,"nworkschedule_index"))
			lds_explosion_container.setitem(a,"npltf_flight_index_group",		lds_explosion.getItemNumber(k,"npltf_flight_index_group"))
			lds_explosion_container.setitem(a,"npltf_flight_index", 				lds_explosion.getItemNumber(k,"npltf_flight_index"))
			...
			// 26.09.2016 HR: CCS-CR-1541 New Parameter passed through to the lowest item list level
			// --------------------------------------------------------------------------------------------------------------------
			lds_explosion_container.setitem(a,"NSERVICE_SEQUENCE", 		dsCenOutMasterLevel1.GetItemNumber(j, "NSERVICE_SEQUENCE"))
			lds_explosion_container.setitem(a,"CSAP_CODE", 						dsCenOutMasterLevel1.GetItemString(j, "CSAP_CODE"))
			
			// --------------------------------------------------------------------------------------------------------------------
			// 28.12.2023 HR: AlmID 9307: Copy new Parametes for replacements
			// --------------------------------------------------------------------------------------------------------------------
			lds_explosion_container.setitem(a,"npl_detail2_key", 		 			lds_explosion.getItemNumber(k,"npackinglist_detail2_key"))
			lds_explosion_container.setitem(a,"npl_index_key_ori", 	 			lds_explosion.getItemNumber(k,"npackinglist_index_key_ori"))
			lds_explosion_container.setitem(a,"npl_detail_key_ori", 	 			lds_explosion.getItemNumber(k,"npackinglist_detail_key_ori"))
			lds_explosion_container.setitem(a,"cpackinglist_ori", 		 			lds_explosion.getItemstring(k,"cpackinglist_ori"))
		end if		
	next	
	//lds_explosion_container.saveas("D:\TESTXLS\TEST-"+string(j,"00000")+"-Ziel.XLSX",XLSX!,TRUE)

next

dsCenOutMasterLevel1.reset()
lds_explosion_container.rowscopy(1,lds_explosion_container.rowCount(),primary!,dsCenOutMasterLevel1,1,primary!)
//dsCenOutMasterLevel1.saveas("D:\TESTXLS\TEST-NACH.XLSX",XLSX!,TRUE)

// 29.12.2023 HR:
if li_replacement = 1 then
	if dsCenOutDetail.update() = 1 then
		guoLog.uf_info("["+ this.classname( )+".of_generate_sub_pl_explosion_plsql] saved " + string(dsCenOutDetail.rowcount()) +" Rows in CEN_OUT_DETAIL")
	else
		guoLog.uf_error("["+ this.classname( )+".of_generate_sub_pl_explosion_plsql] Error saveing " + string(dsCenOutDetail.rowcount()) +" Rows in CEN_OUT_DETAIL " + string(dsCenOutDetail.lsqlerrordbcode) + " " + dsCenOutDetail.ssqlerrortext )
	end if
end if	
...
guoLog.uf_allways("["+ this.classname( )+".of_start_for_flight_areachange] "+"Start Packing List Explosion for one flight (AREACHANGE): Client = " + sClient + "; Unit = " + sPlant + "; Flight = " + sAirline +" " + string(lFlightNumber)+" / "+string(dgenerationdate,"dd.mm.yy"))
...
if lds_cen_out_master.rowcount()=0 then
	guoLog.uf_allways("["+ this.classname( )+".of_start_for_flight_areachange] "+"No Rows in cen_out_master for Flight")
	destroy(lds_cen_out_master)
	return false
else
	guoLog.uf_allways("["+ this.classname( )+".of_start_for_flight_areachange] "+string(lds_cen_out_master.rowcount())+" Rows in cen_out_master for Flight" )
end if
...
if lds_explode.rowcount()=0 then
	guoLog.uf_allways("["+ this.classname( )+".of_start_for_flight_areachange] "+"No Rows in Explosion for PL " + string(arg_packinglist_index_key))
	destroy(lds_explode)
	destroy(lds_cen_out_master)
	return false
else
	guoLog.uf_allways("["+ this.classname( )+".of_start_for_flight_areachange] "+string(lds_explode.rowcount())+" Rows in Explosion for PL " + string(arg_packinglist_index_key) )
end if
...
guoLog.uf_allways("["+ this.classname( )+".of_start_for_flight_areachange] "+"Exchange " + string(lreplace) + " Rows in cen_out_master")
...
if lds_cen_out_master.Update() <> 1 then
	...
	guoLog.uf_allways("["+ this.classname( )+".of_start_for_flight_areachange] "+"Error writing to database table cen_out_master: code/row/text: " + string(lSQLCode) + "/" + string(lSQLRow) + "/" + sSQLErrorText )
...
guoLog.uf_allways("["+ this.classname( )+".of_start_for_flight_areachange] "+"Save cen_out_master " + string(lfind - lrow,"#,##0") + " msek")
...
// =============================================================================
//
// F$$HEX1$$fc00$$ENDHEX$$lle die fehlenden Felder der Ebene 1
//
// =============================================================================
long     	 		j,i
long     	 		lFind
...
guoLog.uf_allways("["+ this.classname( )+".of_fill_level1_fields] Start filling missing Fields for Level 1 for flight " + sAirlineCode + " " + string(lFlightNumber) + " , " + &
               string(dgenerationdate) + "! [of_fill_level1_fields]")
...
// of_start_for_ccsjob
//
// Matdispo (Zeitungen und normales Zeug) f$$HEX1$$fc00$$ENDHEX$$r einen Flug, bei dem Level 1 schon aufgel$$HEX1$$f600$$ENDHEX$$st ist aus
//
// =============================================================================
long					i
...
ii_explode_plsql = integer(of_cen_profilestring("OnlineExplosion","ExplodeWithPLSQL","0"))
if ii_explode_plsql >= 1 then
	guoLog.uf_allways("["+ this.classname( )+".of_start_for_ccsjob] "+"Online Item Explosion being performed by PLSQL" )
end if
...
guoLog.uf_allways("["+ this.classname( )+".of_start_for_ccsjob] "+"Start Packing List Explosion for one flight: Client = " + sClient + "; Unit = " + sPlant + "; Flight = " + sAirline  +" " + string(lFlightNumber)+"/"+string(dgenerationdate,"dd.mm.yy"))
...
if ii_explode_plsql = 0 then
	...
	ids_prod_time_frame.retrieve(sPlant, ld_calc_date, ls_calc_time, "%"+string(iDayCalc)+"%", lAirlineKey )
	guoLog.uf_allways("["+ this.classname( )+".of_start_for_ccsjob] Rows in ids_prod_time_frame: "+string(ids_prod_time_frame.rowcount()))
	ids_prod_time_frame_flight.retrieve(lResultKey, ld_calc_date)
	guoLog.uf_allways("["+ this.classname( )+".of_start_for_ccsjob] Rows in ids_prod_time_frame_flight: "+string(ids_prod_time_frame_flight.rowcount()))
end if
...
// =============================================================================
if iCNXFlag = 1 and iDoNotSet0ForCxFlights =0 then
	guoLog.uf_allways("["+ this.classname( )+".of_start_for_ccsjob] "+"CNX-Flag ist gesetzt => Matdispo auf 0" )
	...
	if sqlca.sqlcode <> 0 then
		guoLog.uf_error("["+ this.classname( )+".of_start_for_ccsjob] "+"Error updating cen_out_master for CNX" )
		iErrorCode = 9
		return false
	end if
...
guoLog.uf_allways("["+ this.classname( )+".of_start_for_ccsjob] "+"Packing List Explosion for flight " + sAirlineCode + " " + string(lFlightNumber) + ": ResultKey = " + string(lResultKey) )
...
// --------------------------------------------------------------------------------------------------------------------
// 06.06.2016 HR: Wir lesen die Level 1 Aufl$$HEX1$$f600$$ENDHEX$$sung
// --------------------------------------------------------------------------------------------------------------------
dsCenOutMasterLevel1						= Create uo_generate_datastore
dsCenOutMasterLevel1.DataObject		= "dw_cen_out_master_ccsjob"
dsCenOutMasterLevel1.SetTransObject(SQLCA)

dsCenOutMasterLevelN						= Create uo_generate_datastore
dsCenOutMasterLevelN.DataObject		= "dw_cen_out_master_ccsjob"

dsCenOutMasterLevel1.retrieve(lResultKey)
if dsCenOutMasterLevel1.rowcount() =0 then
	guoLog.uf_error("["+ this.classname( )+".of_start_for_ccsjob] No Level 1 PL found" )
else
	guoLog.uf_error("["+ this.classname( )+".of_start_for_ccsjob] "+string(dsCenOutMasterLevel1.rowcount() )+" Level 1 PL found" )
	
	// Aufl$$HEX1$$f600$$ENDHEX$$sung Sub-Packinglists
	if ii_explode_plsql >= 1 then
		if of_generate_sub_pl_explosion_plsql(1) <> 0 then
			sErrortext = "Error generating Sub-Packing-List Explosion."
			guoLog.uf_error("["+ this.classname( )+".of_start_for_ccsjob] "+sErrortext )
		end if
	else
		for i=1 to dsCenOutMasterLevel1.rowcount()
			dsCenOutMasterLevel1.setitemstatus( i, 0, PRIMARY!, New!)
		next
...
// 26.07.2024 HR: AlmID 10606 activate column "Sales" in Item List Screen for MYPA tool
if not ib_account4goodsrecipient then return 1
...
if sqlca.sqlcode=0 then
	...
	guoLog.uf_error("["+ this.classname( )+".of_get_account4goodsrecipient] Found no ACCOUNT-Info for NAIRLINE_KEY: " + string(arg_customer_key) + " and sGoodsRecipient: " + arg_we + ": "+string(sqlca.sqlcode)  )
	arg_caccount =" "
	setnull(arg_naccount_key)
end if

// 31.08.2016 HR: CBASEalm ID #1555:  Bei abweichenden WE muss der AC2 geleert werden
setnull(arg_caccount2)
...
String			ls_SAP_CODE
String			ls_CPROD_CAT_TEXT
//
// Ausstieg bei Level > 20
//
if arg_level > 20 then
   guoLog.uf_error("["+ this.classname( )+".of_resolve_packinglist] Fatal Error: Resolve Packinglist Level " + &
                  string(arg_level) + " beyond limit of 20!!!")
   return -1
end if

dtnow = datetime(today(),now())

guoLog.uf_debug( "["+ this.classname( )+".of_resolve_packinglist] Result_key = " + string(lResultKey) + "; Resolve Packinglist Level " + &
...
for j = 1 to dsDetail.RowCount()
   Choose Case lPLType
      case 1
         ...
         //
			...
			ls_SAP_CODE	= dsDetail.GetItemString(j,"cprod_cat")
			ls_CPROD_CAT_TEXT = dsDetail.GetItemString(j,"CPROD_CAT_TEXT")
			...
			if ls_SalesRel = "1" then
				// 31.08.2016 HR: CBASEalm ID #1555:  Bei abweichenden WE muss der AC2 geleert werden
				of_get_account4goodsrecipient(lAirlineKey, ls_GoodsRecipient, lsAccount, llaccount_key, sAdditionalAccount)
			end if
   ...
   if liPackinglistLevel = 2 and lPLType = 1 then
	...
	dsCenOutMasterLevelN.setitem(lRow,"nbatch", ii_batch)
	dsCenOutMasterLevelN.setitem(lRow,"dprod_date", id_prod_date)
	dsCenOutMasterLevelN.setitem(lRow,"nworkschedule_index", il_nworkschedule_index)
	dsCenOutMasterLevelN.setitem(lRow,"NPLTF_FLIGHT_INDEX_GROUP", il_NPLTF_FLIGHT_INDEX_GROUP)
	dsCenOutMasterLevelN.setitem(lRow,"npltf_flight_index", il_npltf_flight_index)
...
//      dsCenOutMasterLevelN.SetItem(lRow,"nworkstation_key",lSubWorkstationKey)
	...
	// 26.09.2016 HR: CR  CCS-CR-1541: Neue Werte sollen mit aufgeloesst werden
	// --------------------------------------------------------------------------------------------------------------------
	dsCenOutMasterLevelN.SetItem(lRow,"CSAP_CODE", arg_csap_code)


	// --------------------------------------------------------------------------------------------------------------------
	// 21.08.20176 CR 2314 Product Category Text in Explosion
	// --------------------------------------------------------------------------------------------------------------------	
	dsCenOutMasterLevelN.SetItem(lRow,"CPROD_CAT_TEXT", ls_CPROD_CAT_TEXT)

	
	dsCenOutMasterLevelN.SetItem(lRow,"NSERVICE_SEQUENCE", arg_nservice_sequence)
...
public function integer of_generate_additional_delivery ();long       	i, lFind
string     	sFind
String     	ls_cclass_temp
long 		ll_nclass_number, ll_nhandling_key, ll_nrotation_key, ll_nrotation_name_key

dsCenOutAddDelivery.retrieve(lResultKey, dGenerationDate)

if dsCenOutAddDelivery.rowcount()=0 then return 1
...
// 14.08.2009 HR: Protokoll eingef$$HEX1$$fc00$$ENDHEX$$gt
guoLog.uf_debug( "["+ this.classname( )+".of_generate_additional_delivery] dsCenOutMaster.rowcount() vorher: "+string(dsCenOutMaster.rowcount()))
guoLog.uf_debug( "["+ this.classname( )+".of_generate_additional_delivery] dsCenOutAddDelivery.rowcount()  "+string(dsCenOutAddDelivery.rowcount()))

for i = 1 to dsCenOutAddDelivery.RowCount()
	// 14.08.2009 HR: Protokoll eingef$$HEX1$$fc00$$ENDHEX$$gt
	guoLog.uf_debug( "["+ this.classname( )+".of_generate_additional_delivery] "+dsCenOutAddDelivery.GetItemString(i,"cen_out_add_delivery_cpackinglist")+" "+string(dsCenOutAddDelivery.GetItemNumber(i,"cen_packinglists_npackinglist_index_key"))+" - BillStat: "+string(dsCenOutAddDelivery.GetItemNumber(i,"cen_out_add_delivery_nbilling") ))

	lPLIndexKey				= dsCenOutAddDelivery.GetItemNumber(i,"cen_packinglists_npackinglist_index_key")
	lPLDetailKey 			= dsCenOutAddDelivery.GetItemNumber(i,"cen_packinglists_npackinglist_detail_key")
	lPackinglistKind			= dsCenOutAddDelivery.GetItemNumber(i,"cen_packinglists_npltype_key")
	lPackinglistType		= dsCenOutAddDelivery.GetItemNumber(i,"cen_packinglists_npackinglist_key")
	sPackinglist				= dsCenOutAddDelivery.GetItemString(i,"cen_out_add_delivery_cpackinglist")
	sText						= dsCenOutAddDelivery.GetItemString(i,"cen_packinglists_ctext")
	sUnit						= dsCenOutAddDelivery.GetItemString(i,"cen_packinglists_cunit")
	dcQuantity				= dsCenOutAddDelivery.GetItemDecimal(i,"cen_out_add_delivery_non_req_quantity")
	dcquantity_version 	= dsCenOutAddDelivery.GetItemDecimal(i,"cen_out_add_delivery_non_req_quantity")
	scustomer_pl			= dsCenOutAddDelivery.GetItemString(i,"cen_packinglists_ccustomer_pl")
	scustomer_pl_text		= dsCenOutAddDelivery.GetItemString(i,"cen_packinglists_ccustomer_text")
	sDefStorageLocation	= dsCenOutAddDelivery.GetItemString(i,"cen_packinglists_cdef_storage_location")

	isPackinglistShortText	= of_ctext_short(sText, dsCenOutAddDelivery.GetItemString(i,"cen_packinglists_ctext")) 
	...
	ll_nclass_number = ii_nclass_number
	...
	if lFind > 0 then
		// 14.08.2009 HR: Protokoll eingef$$HEX1$$fc00$$ENDHEX$$gt
		guoLog.uf_debug( "["+ this.classname( )+".of_generate_additional_delivery]  -> QUANTITY ADDED")  
		...
		// 14.08.2009 HR: Protokoll eingef$$HEX1$$fc00$$ENDHEX$$gt
		guoLog.uf_debug( "["+ this.classname( )+".of_generate_additional_delivery] -> Insert New")
	end if
next
// 14.08.2009 HR: Protokoll eingef$$HEX1$$fc00$$ENDHEX$$gt
guoLog.uf_debug( "["+ this.classname( )+".of_generate_additional_delivery] dsCenOutMaster.rowcount() "+string(dsCenOutMaster.rowcount()))
return 0
...
where cen_setup.cclient = sys_login.cclient
   and cen_setup.csection = :sSection
   and cen_setup.ckey = :sKey
   and Upper(cusername)   = 'CBASE';
...
// 26.07.2024 HR: AlmID 10606 activate column "Sales" in Item List Screen for MYPA tool
ib_account4goodsrecipient = ( of_cen_profilestring("OnlineExplosion","Account4Goodsrecipient","1") = "1" )
...
//
dsFlightsOnly									= Create uo_generate_datastore
...
//
dsFlightsOfTheDay								= Create uo_generate_datastore
...
//
dsExplosionHandling						= Create uo_generate_datastore
...
//
dsCenOutSMPL								= Create uo_generate_datastore
...
//
dsPackingList								= Create uo_generate_datastore

//
// Lesematerial Loading Lists
//
dsNewspaperLoading						= Create uo_generate_datastore
...
//
dsNewspaperExtra							= Create uo_generate_datastore
...
//
dsNewsLoadingList							= Create uo_generate_datastore
...
//
dsLoadingNFB							= Create uo_generate_datastore
...
//
dsSimulation							= Create uo_generate_datastore
...
// --------------------------------------------------------------------------------------------------------------------
dsForecast							= Create uo_generate_datastore
dsForecast.DataObject			= "dw_exp_forecast_values"
dsForecast.SetTransObject(SQLCA)

dsForecast_AS							= Create uo_generate_datastore
...
// --------------------------------------------------------------------------------------------------------------------
ids_prod_time_frame = create uo_generate_datastore
ids_prod_time_frame.DataObject = "dw_exp_prod_time_frame"
ids_prod_time_frame.SetTransObject(SQLCA)

ids_prod_time_frame_flight = create uo_generate_datastore
ids_prod_time_frame_flight.DataObject = "dw_exp_prod_time_frame_flight"
ids_prod_time_frame_flight.SetTransObject(SQLCA)


dsCenOutAddDelivery  = create uo_generate_datastore
dsCenOutAddDelivery.DataObject = "dw_exp_cen_out_add_delivery"
...
destroy (ids_prod_time_frame_flight)
