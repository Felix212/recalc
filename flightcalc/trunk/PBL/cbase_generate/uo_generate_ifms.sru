...
string                   sAircraftConfiguration , ls_filter
...
elseif iPaxType >=5 and iPaxType <=8 then //gebuchte und erwarte Passagiere mit und ohne PAD's
		...
		if iImportSpmls=1 and dsCITPFlightData.RowCount() > 0 then 
			...
			// 23.04.2025 HR: Almid 11358 Flight Generation with SPML setting "Forecast" is ignoring SPMLs
...
//		iSearchClass 	= dsCenOutPax.GetItemNumber(lRow,"nclass_number")
//		lFoundRow	 	= dsCITPFlightData.Find("nclass_number = " + string(iSearchClass) + &
...
For i = 1 to dsHandling.Rowcount()
	...
	if lFind = 0 then
		...
		if lFind = 0 then
			...
			dsHandlingMealCover.Filter()
			//dsHandlingMealCover.SaveAs("c:\temp\cbase\dsHandlingMealCover_2.xls", Excel5!, true)
			...
			If dsHandlingMealCover.Rowcount() <= 0 Then 
				...
				//dsHandlingMealCover.SaveAs("c:\temp\cbase\dsHandlingMealCover_3.xls", Excel5!, true)
			Else
...
of_check_depdate()
if ii_Debug_Save =1 then dsGenerateSchedule.saveas(is_Debug_save_path+"of_generate_flights_2_dsGenerateSchedule_nach_check"+string(now(),"hhmmss")+".xls",Excel5!,TRUE)
...
// --------------------------------------------------------------------------------------------------------------------
// 07.11.2019 HR: Filter und Sort zu Beginn auf Standard zur$$HEX1$$fc00$$ENDHEX$$cksetzen
// --------------------------------------------------------------------------------------------------------------------
dsGenerateRoutingPlan.setfilter("")
dsGenerateRoutingPlan.filter()
dsGenerateRoutingPlan.setsort("nflight_number A")
...
next
if ii_Debug_Save =1 then dsGenerateRoutingPlan.saveas(is_Debug_save_path+"of_generate_flights_5_dsGenerateRoutingPlan_nachher_"+string(now(),"hhmmss")+".xls",Excel5!,TRUE)
...
For I = 1 to dsGenerateRoutingPlan.Rowcount()
	...
	if lCurrentGroupKey <> dsGenerateRoutingPlan.Getitemnumber(i,"nrouting_group_key") then
		...
		iOffset_RF			= 0 // 07.11.2019 HR: Wir setzen das RF-Flag auf 0, damit wir bei der Suche ab Leg 2, diese Ber$$HEX1$$fc00$$ENDHEX$$cksichtigt wird
...
if ii_Debug_Save =1 then dsCenOut.SaveAs(is_Debug_save_path+"ds_cen_out_validation_4a_"+string(now(),"hhmmss")+".xls", Excel5!, true)
// --------------------------------------------------------------------------------
...
Next
if ii_Debug_Save =1 then dsCenOut.SaveAs(is_Debug_save_path+"ds_cen_out_validation_5a_"+string(now(),"hhmmss")+".xls", Excel5!, true)
...
Next	
if ii_Debug_Save =1 then dsCenOut.SaveAs(is_Debug_save_path+"ds_cen_out_validation_6_"+string(now(),"hhmmss")+".xls", Excel5!, true)
...
if ii_Debug_Save =1 then dsCenOut.SaveAs(is_Debug_save_path+"ds_cen_out_validation_7_"+string(now(),"hhmmss")+".xls", Excel5!, true)
...
if ii_Debug_Save =1 then dsCenOut.SaveAs(is_Debug_save_path+"ds_cen_out_validation_8_"+string(now(),"hhmmss")+".xls", Excel5!, true)
