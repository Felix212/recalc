...
uo_cart_datastore ids_TR_Cart_Types
...
public function long of_get_tr_cart_key (string as_unit, long al_index_key, date adt_ref_date, long al_airline_key)
public function integer of_set_tr_cart_size (integer ai_pages, integer ai_columns, integer ai_rungs, long al_key)
public function boolean of_exists_cart_key (long al_tr_cart_key, integer ai_tr_group)
public function integer of_add_content (long al_index_key, long al_detail_key, integer ai_fill_order, boolean ab_force_new_cart, long al_source_row, long al_galley_group, string as_stowage, long al_airline_key, integer ai_tr_group)
public function integer of_fit_into_cart (integer ai_content_height, integer ai_fill_order, long al_content, long al_tr_cart_key, long al_galley_group, string as_stowage, integer ai_tr_group)
public function integer of_add_cart (integer ai_pages, integer ai_columns, integer ai_rows, long al_tr_cart_key, long al_galley_group, integer ai_tr_group)
public function integer of_fit_into_cart_orig (integer ai_content_height, integer ai_fill_order, long al_content, long al_tr_cart_key, long al_galley_group, string as_stowage, integer ai_tr_group)
public function integer of_group_and_page (long al_row, ref long ral_page, ref long ral_cart_number, ref long ral_column, ref long ral_rung, ref string ras_group_and_page)
public function integer of_fit_into_cart_xpage (integer ai_content_height, integer ai_fill_order, long al_content, long al_tr_cart_key, long al_galley_group, string as_stowage, integer ai_tr_group)
end prototypes
...
public function boolean of_exists_cart_key (long al_tr_cart_key);/*
* Objekt : uo_tr_cart_allocation
* Methode: of_exists_cart_key (Function)
* Autor  : Oliver H$$HEX1$$f600$$ENDHEX$$fer
*
* Argument(e):
* long al_tr_cart_key
*
* Beschreibung:		Check if Cart Key already allocated
*
* Aenderungshistorie:
* Version 		Wer			Wann			Was und warum
* 1.0 			O.Hoefer	10.02.2020		Erstkommentar
*
*
* Return: boolean
*
...
f_trace(classname() + ".of_get_tr_cart_key " + as_Unit + " Airline " +String(al_airline_key) + " " + ls_PL)
...
If ll_Rows > 0 Then
	...
	f_trace(classname() +  ".of_get_tr_cart_key "+is_TR_Name+ "  "	+ String(ll_Return) + " " + String(ii_Number_of_Rungs) + " x " + String(ii_Number_of_Columns))
...
*	 integer ai_columns
*
* Beschreibung:		Festlegen der Abmessungen des zu verwendenden TR Cart Typs
*
* Aenderungshistorie:
* Version 		Wer			Wann			Was und warum
* 1.0 			O.Hoefer	11.08.2010		Erstellung
...
//dw_ext_tr_carts
	...
	ll_Row = ids_TR_Cart_Types.Find("ntrcartkey=" + String( al_Key ), 1, ids_TR_Cart_Types.RowCount())
	If ll_Row < 1 Then
		ll_Row = ids_tr_cart_types.InsertRow(0)
//		li_Succ	al_Key
	End If
End If
...
* integer ai_tr_group
*
* Beschreibung:		Check if Cart Key + TR Group Number already allocated
*
* Aenderungshistorie:
* Version 		Wer			Wann			Was und warum
* 1.0 			O.Hoefer	10.02.2020		Erstkommentar
...
ls_Find  = "ntr_type=" + String(al_TR_Cart_Key)
ls_Find += " and ntr_group=" + String(ai_tr_group)
...
* Version 		Wer			Wann			Was und warum
* 1.0 			O.Hoefer	03.09.2012		Erstkommentar
* 1.3 			O.Hoefer	10.02.2020		#5168 TR Cart Group hinzu
*
*
* Return: integer
*
...
// --------------------------------------------------
If ib_extended_tr_handling Then
	ll_Key = of_get_tr_cart_key( is_Unit, al_index_key, idt_Ref_Date, al_Airline_key )
...
If ib_extended_tr_handling Then
	If NOT of_Exists_cart_key(ll_Key, ai_tr_group) Then
		lb_New_Cart = TRUE
	End If
End If
...
If lb_New_Cart Then
	li_Succ = of_add_cart(ii_Number_of_Pages, ii_Number_Of_Columns, ii_Number_of_Rungs, ll_Key, al_galley_group, ai_tr_group)
End if
...
//If of_fit_into_cart (li_Content_Height, ai_Fill_Order, al_source_row, ll_Key, al_galley_group, as_stowage ) = 1 Then
If of_fit_into_cart (li_Content_Height, ai_Fill_Order, al_source_row, ll_Key, al_galley_group, as_stowage, ai_tr_group ) = 1 Then
...
* 1.2 			O.Hoefer	20.10.2017		Fix f$$HEX1$$fc00$$ENDHEX$$r "mehr als 3 H$$HEX1$$f600$$ENDHEX$$heneinheiten"
* 1.3 			O.Hoefer	10.02.2020		#5168 TR Cart Group hinzu
*
*
* Return: integer
...
If ai_content_height = 1 Then
		...
		If Upperbound(ll_Free_Rungs) > 0 Then
			If ai_fill_order = FILL_TOP_DOWN Then
				...
				If ll_Found > 0 Then
					...
					li_Succ = ids_tr_cart_allocation.SetItem(ll_Found, "cstowage", as_stowage)
					li_Succ = ids_tr_cart_allocation.SetItem(ll_Found, "ngalley_group", al_galley_group)									
					li_Succ = ids_tr_cart_allocation.SetItem(ll_Found, "ntr_group", ai_tr_group)									
				End If
	...
	Else
		...
		li_Succ = of_add_cart(ii_Number_of_Pages, ii_Number_Of_Columns, ii_Number_of_Rungs, al_tr_cart_key , al_galley_group, ai_tr_group)
		//Return of_fit_into_cart( ai_content_height, ai_fill_order, al_content, al_tr_cart_key, al_galley_group, as_stowage )	
		Return of_fit_into_cart( ai_content_height, ai_fill_order, al_content, al_tr_cart_key, al_galley_group, as_stowage , ai_tr_group )	
	End If
...
ElseIf ai_content_height > 2 Then
	...
	If ll_Found > 0 Then
		...
		Else
			...
			If Upperbound(ll_Free_Rungs) < ai_content_height Then
				// -----------------------------------------------------------------------
				// Es gibt zu wenig freie Einsch$$HEX1$$fc00$$ENDHEX$$be,  belegen mit DUMMY
				// -----------------------------------------------------------------------
				For li_Row = 1 to upperbound(ll_Free_Rungs)
					ls_Find = "ntrcart=" + String(il_Cart_Counter) +  " and ncontents=0 and npage=" + String(li_Page) 
					ls_Find += " and ncolumn=" + String(li_Column) + " and nrow=" + String(ll_Free_Rungs[li_Row])
					ll_Found = ids_tr_cart_allocation.Find(ls_Find, 1, ids_tr_cart_allocation.RowCount())
					If ll_Found > 0 Then
						li_Succ = ids_tr_cart_allocation.SetItem(ll_Found, "ncontents", -1 )
						li_Succ = ids_tr_cart_allocation.SetItem(ll_Found, "cinfo", "DUMMY")	
...
* 1.2 			O.Hoefer	10.02.2020		Hinzu: #5168 TR Group als zus$$HEX1$$e400$$ENDHEX$$tzliches Trenn-Kriterium
*
*
* Return: integer
*
...
For ll_Count_P = 1 To ai_Pages
	...
	For ll_Count_C = 1 To ai_Columns
		For ll_Count_R = 1 To ai_rows  
			...
			li_Succ = ids_TR_Cart_Allocation.SetItem(ll_New_Row, "ntr_group",	0)
			...
			If ib_Extended_TR_Handling Then
				...
				// #5168 TR Group als zus$$HEX1$$e400$$ENDHEX$$tzliches Trenn-Kriterium
				li_Succ = ids_TR_Cart_Allocation.SetItem(ll_New_Row, "ntr_group",	ai_tr_group )				
...
public function integer of_group_and_page (long al_row, ref long ral_page, ref long ral_cart_number, ref long ral_column, ref long ral_rung, ref string ras_group_and_page);/*
* Objekt : uo_tr_cart_allocation
* Methode: of_group_and_page (Function)
* Autor  : Oliver H$$HEX1$$f600$$ENDHEX$$fer
* Datum  : 11.08.2010
*
* Argument(e):
...
String	ls_Find
Long		ll_Found
Integer	li_Group, li_Compute
String	ls_Temp


ids_tr_cart_allocation.groupcalc()
...
If ll_Found > 0 Then
	...
	ral_rung				= ids_tr_cart_allocation.GetItemNumber(ll_Found, "nrow")
	ral_page				= ids_tr_cart_allocation.GetItemNumber(ll_Found, "nprintpage")
	li_Group				= ids_tr_cart_allocation.GetItemNumber(ll_Found, "ntr_group")
	
	// Description = "TR " plus page counter
	ras_group_and_page = "TR " + String(ral_page)
	If li_Group > 0 Then
		// if group is maintained => description = "TR " plus group number
		ras_group_and_page = "TR-G" + String(li_Group)
	End If
	
	li_Compute = ids_tr_cart_allocation.GetItemNumber(ll_Found, "compute_pg")
	If li_Group > 0 AND li_Compute > 0 Then
		// if group is maintained and group uses more than one page:
		// => description = "TR " plus group number + page number within group
		ras_group_and_page += "-" + String(li_Compute)
	End if
...
If ai_content_height = 1 Then
	...
	If ll_Found > 0 Then
		li_Page		= ids_tr_cart_allocation.GetItemNumber(ll_Found, "npage")
		li_Column	= ids_tr_cart_allocation.GetItemNumber(ll_Found, "ncolumn")
		ll_Cart_Counter = ids_tr_cart_allocation.GetItemNumber(ll_Found, "ntrcart")
		
		Do While ll_Found > 0
			ll_Free_Rungs[Upperbound(ll_Free_Rungs) + 1] = 	 ids_tr_cart_allocation.GetItemNumber(ll_Found, "nrow")
			ls_Find = "ntrcart=" + String(ll_Cart_Counter) +  " and ncontents=0 and npage=" + String(li_Page) + " and ncolumn=" + String(li_Column)
			ll_Found = ids_tr_cart_allocation.Find(ls_Find, ll_Found + 1, ids_tr_cart_allocation.RowCount() + 1)
		Loop
		...
		If Upperbound(ll_Free_Rungs) > 0 Then
			If ai_fill_order = FILL_TOP_DOWN Then
				...
				li_Row = ll_Free_Rungs[1]
				//ls_Find = "ntrcart=" + String(il_Cart_Counter) +  " and ncontents=0 and npage=" + String(li_Page) 
				ls_Find = "ntrcart=" + String(ll_Cart_Counter) +  " and ncontents=0 and npage=" + String(li_Page) 
				ls_Find += " and ncolumn=" + String(li_Column) + " and nrow=" + String(li_Row)
				ll_Found = ids_tr_cart_allocation.Find(ls_Find, 1, ids_tr_cart_allocation.RowCount())
				If ll_Found > 0 Then
					li_Succ = ids_tr_cart_allocation.SetItem(ll_Found, "ncontents", al_content )
...
ids_TR_Cart_Types = CREATE uo_cart_datastore 
ids_TR_Cart_Types.DataObject = "dw_ext_tr_carts"
