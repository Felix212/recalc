...
boolean  ib_dynamicMeals 								= false
boolean	ib_create_docenginejob_after_recalc 		= false //25.05.2023 HR AlmID 8358: 2 new generation jobs requested

// --------------------------------------------------------------------------------------------------------------------
// 06.05.2025 HR:
// --------------------------------------------------------------------------------------------------------------------
uo_generate_datastore			 	idw_loading
uo_generate_datastore			 	idw_OldLoading
...
this.ib_dynamicMeals =  (ifunction = 21 ) // 26.01.2023 HR: AlmID 6658: Dynamic Meal CBASE Integration
...
if iprocess_status = 1 or iprocess_status = 4 then // 29.11.2011 HR:		// TSC: ProcessStatus 4 = retry
	if ifunction = 18 or ifunction = 19 then
		...
		// --------------------------------------------------------------------------------------------------------------------
		// 18.08.2020 HR: AlmID 6446: Wenn noch offen OE f$$HEX1$$fc00$$ENDHEX$$r Quelle vorhanden, dann Status auf 4 setzen
		// --------------------------------------------------------------------------------------------------------------------
		choose case lreturn
			case 1
				dsJobs.SetItem(lRow, "nerror", 				1)
				dsJobs.SetItem(lRow, "nprocess_status", 	3) 
				dsJobs.SetItem(lRow, "dstop_computing", 	f_jetzt())
				
			case 3
				dsJobs.SetItem(lRow, "nerror", 				1)
				dsJobs.SetItem(lRow, "nprocess_status",	4) 
	...
	elseif ifunction = 14 then
		...
		if of_process_new_flight( lrow) = 1 then
			...
			if sqlca.sqlcode=0 then
				guoLog.uf_allways("["+ this.classname( )+".of_process_row] Staus des Fluges von 0 auf "+string(istatus)+" gesetzt")
				...
				guoLog.uf_error("["+ this.classname( )+".of_process_row] Fehler Staus$$HEX1$$e400$$ENDHEX$$nderung des Fluges von 0 auf "+string(istatus)+": "+string(sqlca.sqlcode)+" "+sqlca.sqlerrtext)
	...
	*/  	
	...
	elseif of_search_sys_queue_function(ifunction) = 0 then
		if this.iinternal_function = 1 or &
		 	...
		 	ifunction = 15 or &
			ifunction = 17 or &
			 ifunction = 21 then // 26.01.2023 HR: AlmID 6658: Dynamic Meal CBASE Integration
			...
			if bReadFromHistory and  this.dsJobs.GetItemNumber(lRow, "sys_queue_flight_calc_nhistory")=0 then
				...
				guoLog.uf_allways("["+ this.classname( )+".of_process_row] bReadFromHistory is set to false")
			elseif not bReadFromHistory and  this.dsJobs.GetItemNumber(lRow, "sys_queue_flight_calc_nhistory")=1 then
				...
				guoLog.uf_allways("["+ this.classname( )+".of_process_row] bReadFromHistory is set to true")
			end if
			...
			if iReturn >= 0 then
				if ii_actual_flight_status=0 and this.ireturn_error_status = -1 then
					...
					if sqlca.sqlcode=0 then
						guoLog.uf_allways("["+ this.classname( )+".of_process_row] Staus des Fluges von 0 auf "+string(iChangeStatus)+" gesetzt")
						...
						guoLog.uf_error("["+ this.classname( )+".of_process_row] Fehler Staus$$HEX1$$e400$$ENDHEX$$nderung des Fluges von 0 auf "+string(iChangeStatus)+": "+string(sqlca.sqlcode)+" "+sqlca.sqlerrtext)
			...
			if this.ireturn_error_status = 0 then
				guoLog.uf_allways("["+ this.classname( )+".of_process_row] "+ this.sreturn_error_message)
			else
				guoLog.uf_error("["+ this.classname( )+".of_process_row] "+ this.sreturn_error_message)
			end if	
...
//			this.dsJobs.SetItem(lRow, "cerror", this.sTypeText  +this.sreturn_error_message)
		...
		elseif ifunction=8 then
			...
			guoLog.uf_allways("["+ this.classname( )+".of_process_row] Flight Delete Result: "+this.sreturn_error_message )
		...
		elseif  ifunction=10 then
			...
			// 19.06.2023 HR: Wenn ii_numberrunmzvspawn = 0 dann darf auch nicht Spawn auch nicht benutzt werden 
			if ii_use_mzvspawn=1 and  this.ii_numberrunmzvspawn > 0 then
				...
				if this.dsJobs.GetItemNumber(lRow, "nprocess_status") = 2 then 
					...
					if f_dt_diff(dsJobs.GetItemDateTime(lRow, "dstart_computing"), f_jetzt()) > gl_max_mzv_run then
						...
						guoLog.uf_allways("["+ this.classname( )+".of_process_row] Job set to ignore, because runtime of job is longer than "+string(gl_max_mzv_run)+" sec. Sent to cbase_mzv_spawn at " + string(dsJobs.GetItemDateTime(lRow, "dstart_computing")))
					else	
						guoLog.uf_allways("["+ this.classname( )+".of_process_row] Job send to cbase_mzv_spawn at " + string(dsJobs.GetItemDateTime(lRow, "dstart_computing")))
					end if
					return 0
				end if
				...
				if fileexists(this.is_path2mzvspawn+"cbase_mzv_spawn.exe") then				
					if this.dsJobs.getitemnumber(lRow, "comp_sum_open_mlzspawn") < this.ii_numberrunmzvspawn then
						...
						IF sqlca.sqlcode < 0 THEN
							guoLog.uf_error("["+ this.classname( )+".of_process_row] Error checking Flight Jobs: " + string(sqlca.sqlcode))
						elseif ll_count = 0 then
							...
							if run(a) = 1 then
								...
								guoLog.uf_allways("["+ this.classname( )+".of_process_row] Mealdistributuion: Start  ("+string(this.dsJobs.getitemnumber(lRow, "comp_sum_open_mlzspawn")) +") "+a)
								sleep(2)
								yield()
								
							else
								guoLog.uf_error("["+ this.classname( )+".of_process_row] Mealdistributuion: Error while starting "+a)
							...
							guoLog.uf_allways("["+ this.classname( )+".of_process_row] Mealdistributuion: Job defered, because another MZV Spawn is running for this flight")
						end if
					else
						guoLog.uf_allways("["+ this.classname( )+".of_process_row] Mealdistributuion: Still "+string(ii_numberrunmzvspawn) +" cbase_mzv_spawn processes running" )
					end if
				else
					guoLog.uf_allways("["+ this.classname( )+".of_process_row] Mealdistributuion: File " + this.is_path2mzvspawn+"cbase_mzv_spawn.exe not found")
				end if
				return 0
			else
				// --------------------------------------------------------------------------------------------------------------------
				...
				guoLog.uf_allways("["+ this.classname( )+".of_process_row] Mealdistributuion for Flight: "+this.sreturn_error_message )
				
				return 0
			end if
		else
			guoLog.uf_error("["+ this.classname( )+".of_process_row] Unknown internal function " + String(this.iinternal_function))
			dsJobs.SetItem(lRow, "nerror", 2)
			dsJobs.SetItem(lRow, "nprocess_status",9) // 29.11.2011 HR:
			...
			guoLog.uf_error("["+ this.classname( )+".of_process_row] function " + String(ifunction) + " not found in SYS_FLIGHT_FL_FUNCTION")
			dsJobs.SetItem(lRow, "nerror", 2)
			dsJobs.SetItem(lRow, "nprocess_status",9) // 29.11.2011 HR:
...
elseif dsJobs.GetItemNumber(lRow, "nprocess_status")=2 and ifunction=10 then
	...
	if f_dt_diff(dsJobs.GetItemDateTime(lRow, "dstart_computing"), f_jetzt()) > gl_max_mzv_run then
		...
		guoLog.uf_error("["+ this.classname( )+".of_process_row] Job set to ignore, because runtime of job is longer than "+string(gl_max_mzv_run)+" sec. Sent to cbase_mzv_spawn at " + string(dsJobs.GetItemDateTime(lRow, "dstart_computing")))
	else	
		guoLog.uf_allways("["+ this.classname( )+".of_process_row] Job send to cbase_mzv_spawn at " + string(dsJobs.GetItemDateTime(lRow, "dstart_computing")))
	end if
end if	
...
if lrows=0 then guoLog.uf_allways("["+ this.classname( )+".of_start] No flights to calculate") 

// --------------------------------------------------------------------------------------------------------------------
// 17.05.2024 HR: Wir schauen erst mal, ob wir ggf. mehrere Jobs f$$HEX1$$fc00$$ENDHEX$$r einen Flug/Funktion existiert und setzen diese auf Ignore
// --------------------------------------------------------------------------------------------------------------------
dsJobs.setfilter("nresult_key A, nfunction A, compute_process_status A, njob_nr A")
dsJobs.filter()

For lRows = 2 to dsJobs.RowCount()
	
   if dsJobs.GetItemNumber(lRows, "nresult_key") = dsJobs.GetItemNumber(lRows -1, "nresult_key") AND &   
      dsJobs.GetItemNumber(lRows, "nfunction") = dsJobs.GetItemNumber(lRows -1, "nfunction") AND &
      dsJobs.GetItemNumber(lRows, "compute_process_status") = dsJobs.GetItemNumber(lRows -1, "compute_process_status") then
      ...
      guoLog.uf_info("["+ this.classname( )+".of_start] Set Job to ignore we have more then one open jobs for it -> " + string(this.ilJobNr) + " for flight " + sFlightText) 
      
      dsJobs.SetItem(lRows, "dstart_computing", f_jetzt() )  
      dsJobs.SetItem(lRows, "dstop_computing",    f_jetzt() )  
      dsJobs.SetItem(lRows, "nprocess_status",    7 ) 
   end if
next
...
dsJobs.setfilter("compute_process_status A, sys_queue_flight_calc_nsort A, cen_out_ddeparture A, cen_out_cdeparture_time A, cunit A, cairline A, nflight_number A, nresult_key A, nfunction A, njob_nr A")   
dsJobs.filter()   
...
For lRows = 1 to dsJobs.RowCount()
	...
	//07.06.2024 HR: Logging Job-Nr and Result_key to Graylog
	gll_log_result_key	= dsJobs.GetItemNumber(lRows, "nresult_key")
	gll_log_job_key		= dsJobs.GetItemNumber(lRows, "njob_nr")
	...
	//25.05.2023 HR AlmID 8358: 2 new generation jobs requested
	ib_create_docenginejob_after_recalc = ( dsJobs.GetItemNumber(lRows, "ndoc_generation") = 1 )
	...
	guoLog.uf_allways("["+ this.classname( )+".of_start]        S T A R T I N G   J O B -> " + string(this.ilJobNr) + " for flight " + sFlightText) 
	guoLog.uf_allways("["+ this.classname( )+".of_start]        Function: " + string(dsJobs.GetItemNumber(lRows, "nfunction")))
	guoLog.uf_allways("["+ this.classname( )+".of_start]        Result_Key: "  + string(this.lResultKeyCenOut) + " in row " + string(lRows) + " of " + string(dsJobs.RowCount()))
	guoLog.uf_allways("["+ this.classname( )+".of_start] ----------------------------------------------------------------------------------")
	...
	if ib_check_jobstate then
		...
		if sqlca.sqlcode <> 0 then
			guoLog.uf_error("["+ this.classname( )+".of_start] Flight ignored: Error finding jobstate "  + string(sqlca.sqlcode) + " " + sqlca.sqlerrtext)
			continue
		elseif 	isnull(iFunction) or iFunction = -1 then
			guoLog.uf_error("["+ this.classname( )+".of_start] Flight ignored: State not found")
			continue
		elseif iFunction >= 3 and iFunction <> 4 then
			guoLog.uf_error("["+ this.classname( )+".of_start] Flight ignored: New State: "+ string(iFunction))
			continue
		end if
	end if
	...
	guoLog.uf_allways("["+ this.classname( )+".of_start] Flight Calculation " + this.sFlightText + " took " + string((lStop - lStart) / 1000, "0.000") + " sec.")
	// -------------------------------------------------------------------------------------------------
	...
	if this.dsJobs.Update() <> 1 then
		...
		guoLog.uf_error("["+ this.classname( )+".of_start] Failed to update dsJobs for " +  sFlight + "(JobNr:" + String(this.ilJobNr) + " Instance:" + sInstance + ")")
		guoLog.uf_error("["+ this.classname( )+".of_start] Error: " + sqlca.sqlerrtext)
		rollback;
		continue;
	end if
	...
	// --------------------------------------------------------------------------------------------------------------------
	// 01.07.2025 HR: #12067  automated Freeze & Flight Status Change
	// --------------------------------------------------------------------------------------------------------------------
	if dsJobs.GetItemNumber(lRows, "nfreeze_after_finished") = 1 then
		if dsJobs.GetItemNumber(lRows, "nprocess_status") =  3 then
			INSERT INTO SYS_QUEUE_FLIGHT_CALC 
									(NJOB_NR, NRESULT_KEY, DDEPARTURE, DCREATED, NSTATUS, CDESCRIPTION, 
									 NFUNCTION, CMODIFIED, CUSER, NFORECAST, NHISTORY, NSORT)
						   select SEQ_SYS_QUEUE_FLIGHT_CALC.NEXTVAL,
									CEN_OUT.NRESULT_KEY,
									CEN_OUT.DDEPARTURE_TIME_LOC,
									SYSDATE,
									CEN_OUT.NSTATUS,
									CEN_OUT.CAIRLINE || CEN_OUT.NFLIGHT_NUMBER || ' ' || to_char(CEN_OUT.DDEPARTURE, 'DD.MM.YY'),
									15, 'requested by FC', 'cbase_service', 0, 0, 1
						 from cen_out 
						where nresult_key = :lResultKeyCenOut;
						  
			if sqlca.sqlcode <> 0 then
				guoLog.uf_error("["+ this.classname( )+".of_start] Error Create Freeze Job: "+string(sqlca.sqlcode )+": " + sqlca.sqlerrtext)
				rollback;
			end if
		else
			guoLog.uf_error("["+ this.classname( )+".of_start] Create Freeze Job: Flight Job has wrong state "+string(dsJobs.GetItemNumber(lRows, "nprocess_status") ) +" instead of 3") 
		end if
	end if
...
// 06.05.2025 HR:
// --------------------------------------------------------------------------------------------------------------------
for i = 1 to idw_loading.RowCount()
	idw_loading.SetItem(i, "ntransaction", lTransaction)
next
...
// ---------------------------------------------------------------------
guoLog.uf_debug("["+ this.classname( )+".wf_chc_master_change] Begin")

if wf_chc_validation() <> 0 then
	guoLog.uf_debug("["+ this.classname( )+".wf_chc_master_change] wf_chc_validation returns <> 0")
...
// 06.05.2025 HR:
// --------------------------------------------------------------------------------------------------------------------
s_change.dscenoutloading						= Create uo_generate_datastore
s_change.dscenoutloading.DataObject 		= "dw_change_cen_out_loading"
s_change.dscenoutloading.SetTransObject(SQLCA)
s_change.dsOldCenOutLoading					= Create uo_generate_datastore
s_change.dsOldCenOutLoading.DataObject 	= "dw_change_cen_out_loading"
s_change.dsOldCenOutLoading.SetTransObject(SQLCA)
...
if i=0 then
	...
	if ll_routingdetail_key =-1 or ll_nroutingdetail_key_cp =-1 then
		guoLog.uf_allways("["+ this.classname( )+".wf_chc_master_change] Fehler beim Ermitteln der Flugschl$$HEX1$$fc00$$ENDHEX$$ssel, da die G$$HEX1$$fc00$$ENDHEX$$ltigkeit der Beladedefinition ge$$HEX1$$e400$$ENDHEX$$ndert wurde. Alte Werte werden weiter benutzt.")
	else
...
if dw_SPML.RowsCopy(1,dw_SPML.rowcount(),Primary!,s_change.dsnewspml,s_change.dsnewspml.RowCount() + 1,Primary!) = -1 and dw_SPML.rowcount()>0 then
  guoLog.uf_error("["+ this.classname( )+".wf_chc_master_change] error rowscopy dw_spml to s_change.dsnewspml / Source: " + dw_SPML.dataobject + " / Target: " + s_change.dsnewspml.dataobject)
end if
//
...
if dw_meal.RowsCopy(1,dw_meal.rowcount(),Primary!,s_change.dsCurrentMeals,s_change.dsCurrentMeals.RowCount() + 1,Primary!) = -1 and dw_meal.rowcount()>0 then
  guoLog.uf_error("["+ this.classname( )+".wf_chc_master_change] error rowscopy dw_meal to s_change.dsCurrentMeals / Source: " + dw_meal.dataobject + " / Target: " + s_change.dsCurrentMeals.dataobject)
end if
if dw_extra.RowsCopy(1,dw_extra.rowcount(),Primary!,s_change.dscurrentextra,s_change.dscurrentextra.RowCount() + 1,Primary!) = -1 and dw_extra.rowcount()>0 then
  guoLog.uf_error("["+ this.classname( )+".wf_chc_master_change] error rowscopy dw_extra to s_change.dscurrentextra / Source: " + dw_extra.dataobject + " / Target: " + s_change.dscurrentextra.dataobject)
end if
//
...
if ibGenerateLoading then
	wf_ml_superrowscopy(idw_loading,s_change.dsOldCenOutLoading)
end if
...
if dspackets_all.RowsCopy(1,dspackets_all.rowcount(),Primary!,s_change.dspackets,s_change.dspackets.RowCount() + 1,Primary!) = -1 and dspackets_all.rowcount()>0 then
	guoLog.uf_debug("["+ this.classname( )+".wf_chc_master_change] Fehler rowscopy dspackets")	
end if
...
If (ii_action = 2) Then 	// Aufruf erfolgt $$HEX1$$fc00$$ENDHEX$$ber Web
	...
	s_change.dspriorExtra.reset()
   	if s_change.dscurrentextra.rowscopy(1,s_change.dscurrentextra.rowcount(),primary!, s_change.dsOldExtra, 1, primary!)  = -1  and s_change.dscurrentextra.rowcount()>0 then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_master_change] error rowscopy s_change.dscurrentextra to s_change.dsOldExtra / Source: " + s_change.dscurrentextra.dataobject + " / Target: " + s_change.dsOldExtra.dataobject)
	end if
	if s_change.dscurrentextra.rowscopy(1,s_change.dscurrentextra.rowcount(),primary!, s_change.dspriorExtra, 1, primary!) = -1 and s_change.dscurrentextra.rowcount()>0 then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_master_change] error rowscopy  s_change.dscurrentextra to  s_change.dspriorExtra / Source: " + s_change.dscurrentextra.dataobject + " / Target: " +  s_change.dspriorExtra.dataobject)
	end if
end if

// --------------------------------------------------------------------------------------------------------------------
// 26.01.2023 HR: AlmID 6658: Dynamic Meal CBASE Integration
// --------------------------------------------------------------------------------------------------------------------
if ib_dynamicMeals then
	string ls_CFLIGHT_KEY
	
	ls_CFLIGHT_KEY = String(dw_single.GetItemdatetime(dw_single.getrow(), "dreturn_date"), "YYYYMMDD") 
	ls_CFLIGHT_KEY += dw_single.GetItemString(dw_single.getrow(), "cairline") 
	ls_CFLIGHT_KEY += String(dw_single.GetItemNumber(dw_single.getrow(), "nflight_number"), "0000")

	If IsNULL( dw_single.GetItemString(dw_single.getrow(), "csuffix") ) Then
		ls_CFLIGHT_KEY += " "
	Else
		ls_CFLIGHT_KEY += dw_single.GetItemString(dw_single.getrow(), "csuffix")
	End If

	ls_CFLIGHT_KEY += dw_single.GetItemString(dw_single.getrow(), "ctlc_from") 
	ls_CFLIGHT_KEY += dw_single.GetItemString(dw_single.getrow(), "ctlc_to")

	uo_change.ib_use_dynamic_meals = true
	if uo_change.of_get_dynamicmeals( ls_CFLIGHT_KEY ) < 1 then
		of_log_jobstate(ii_log_info,  "No DynamicMeal Orders found" )
	end if
else

	// --------------------------------------------------------------------------------------------------------------------
	// 04.07.2024 HR: AlmID 9925: Automatic Dynamic Meal import
	// --------------------------------------------------------------------------------------------------------------------
	s_change.dsCenExternalPreorder								= Create uo_generate_datastore
	s_change.dsCenExternalPreorder.DataObject 				= "dw_change_citp_flightdata_preorder_fc"
	s_change.dsCenExternalPreorder.settransobject( SQLCA)
	
	if s_change.dsCenExternalPreorder.retrieve( ilJobNr ) > 0 then
		guoLog.uf_info("["+ this.classname( )+".wf_chc_master_change] Job has " + string( s_change.dsCenExternalPreorder.rowcount() ) + " Preorder rows")
		ib_dynamicMeals = true
	else
		guoLog.uf_debug("["+ this.classname( )+".wf_chc_master_change] No Preorder Infos found")
	end if
...
guoLog.uf_debug("["+ this.classname( )+".wf_chc_master_change] Aufl$$HEX1$$f600$$ENDHEX$$sung Flug Begin")
s_change = uo_change.of_change_handling_objects( s_change )

if s_change.bsuccess  then
	
	guoLog.uf_debug("["+ this.classname( )+".wf_chc_master_change] Aufl$$HEX1$$f600$$ENDHEX$$sung Flug erfolgreich")	
	of_log_jobstate(ii_log_info, uf.translate("Aufl$$HEX1$$f600$$ENDHEX$$sung Flug erfolgreich")) // 12.03.2015 HR: Logging eingebaut

	if ib_dynamicMeals then
		string ls_error
		
		for i = 1 to uo_change.uo_generate_meals.idsCITPFlightDataPreorder.rowcount()
			if uo_change.uo_generate_meals.idsCITPFlightDataPreorder.getitemnumber( i, "nfound")=1 then continue
			
			if uo_change.uo_generate_meals.idsCITPFlightDataPreorder.getitemnumber(i, "nfound")=0 then
				ls_error = "DM not in Masterdata: "
			else
				ls_error = "DM no Order found: "
			end if
			
			ls_error +=  uo_change.uo_generate_meals.idsCITPFlightDataPreorder.getitemstring(i, "cen_request_preorder_cclass") + " - " 
			ls_error +=  uo_change.uo_generate_meals.idsCITPFlightDataPreorder.getitemstring(i, "cen_request_preorder_ccustomer_pl") + " - " 
			ls_error +=  uo_change.uo_generate_meals.idsCITPFlightDataPreorder.getitemstring(i, "cen_request_preorder_cdish_name")
			
			of_log_jobstate(ii_log_info,  ls_error )
		next		
	...
	guoLog.uf_debug("["+ this.classname( )+".wf_chc_master_change] Aufl$$HEX1$$f600$$ENDHEX$$sung Flug mit Fehler abgeschlossen: "+s_change.sMessage)
...
If s_change.bSuccess = False Then
	if bImportMode then
		...
		guoLog.uf_error("["+ this.classname( )+".wf_chc_master_change] Fehler: "+s_change.sMessage)
	end if
	...
	destroy	s_change.dsCenOutLoading
	destroy	s_change.dsOldCenOutLoading
	destroy	uo_change
	
	guoLog.uf_debug("["+ this.classname( )+".wf_chc_master_change] s_change.bSuccess = False")
	return False
Else
	//
...
if (lMasterStatus <= 3 and itimeadjustmentpl = 1) or &
	...
	if bImportMode then
		...
		if dw_single.GetItemString(1,"cramp_time") <> s_change.sRamptime then
			 guoLog.uf_allways("["+ this.classname( )+".wf_chc_master_change] Die errechnete Rampenzeit betr$$HEX1$$e400$$ENDHEX$$gt " + s_change.sRamptime + &
		...
		if dw_single.GetItemString(1,"ckitchen_time") <> s_change.sKitchenTime then
			 guoLog.uf_allways("["+ this.classname( )+".wf_chc_master_change] Die errechnete K$$HEX1$$fc00$$ENDHEX$$chenzeit betr$$HEX1$$e400$$ENDHEX$$gt " + s_change.sKitchenTime + &
...
If wf_chc_change_pax(uo_change.dsCenOutPax) = False Then
	bNoSaving = True
	guoLog.uf_debug("["+ this.classname( )+".wf_chc_master_change] wf_chc_change_pax(uo_change.dsCenOutPax) = False")
End if	
...
If wf_chc_change_handling(uo_change.dsCenOutHandling) = False Then
	bNoSaving = True
	guoLog.uf_debug("["+ this.classname( )+".wf_chc_master_change] wf_chc_change_handling(uo_change.dsCenOutHandling) = False")
End if	
...
If wf_chc_change_newspaper(uo_change.dsCenOutHandlingNews,uo_change.dsCenOutNewsExtra) = False Then
	bNoSaving = True
	guoLog.uf_debug("["+ this.classname( )+".wf_chc_master_change] wf_chc_change_newspaper(uo_change.dsCenOutHandlingNews,uo_change.dsCenOutNewsExtra) = False")
End if	
// ---------------------------------------------------------------------
...
If wf_chc_change_meals(s_change.dsnewmeals, s_change.dscenmealsdetdeduction) = False Then
	bNoSaving = True
	guoLog.uf_debug("["+ this.classname( )+".wf_chc_master_change] wf_chc_change_meals(s_change.dsnewmeals) = False")
End if
...
If wf_chc_change_extra(s_change.dsnewextra) = False Then
	bNoSaving = True
	guoLog.uf_debug("["+ this.classname( )+".wf_chc_master_change] wf_chc_change_extra(s_change.dsnewextra) = False")
End if
...
If wf_chc_change_spml(s_change.dsnewspml) = False Then
	bNoSaving = True
	guoLog.uf_debug("["+ this.classname( )+".wf_chc_master_change] wf_chc_change_spml(s_change.dsnewspml) = False")
End if	
...
if s_change.dsnewspmldetail.RowsCopy(1,s_change.dsnewspmldetail.rowcount(),Primary!,dsSPMLDetail,dsSPMLDetail.RowCount() + 1,Primary!)  = -1 and s_change.dsnewspmldetail.rowcount()>0 then
  guoLog.uf_error("["+ this.classname( )+".wf_chc_master_change] error rowscopy  s_change.dsnewspmldetail to dsSPMLDetail / Source: " + s_change.dsnewspmldetail.dataobject + " / Target: " + dsSPMLDetail.dataobject)
end if

// --------------------------------------------------------------------------------------------------------------------
// 06.05.2025 HR:
// --------------------------------------------------------------------------------------------------------------------
for i = idw_loading.rowcount() to 1 step -1
	idw_loading.deleterow(i)
next

if s_change.dsCenOutLoading.RowsCopy(1,s_change.dsCenOutLoading.rowcount(),Primary!,idw_loading, 1,Primary!)  = -1 and s_change.dsCenOutLoading.rowcount()>0 then
	guoLog.uf_error("["+ this.classname( )+".wf_chc_master_change] error rowscopy  s_change.dsCenOutLoading to idw_loading / Source: " + s_change.dsCenOutLoading.dataobject + " / Target: " + idw_loading.dataobject)
end if
...
guoLog.uf_debug("["+ this.classname( )+".wf_chc_master_change] Finish")
...
if iret < 0 then
	guoLog.uf_debug("["+ this.classname( )+".wf_chc_validation] wf_chc_validate_spml() returns " + string(iret))
	return 1
end if
/*
//
...
if iret < 0 then
	guoLog.uf_debug("["+ this.classname( )+".wf_chc_validation] wf_chc_validate_loadinglist() returns " + string(iret))
	return 1
end if
*/
return 0
...
if itype_target = 0 or itype_source = 0 then
	...
	guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_pax] Achtung: Hallo Entwickler, unbekanntes Datawindow in wf_ml_rowscopy_pax!")
...
if itype_target = itype_source then // Formate gleich
		...
		IF	dwsource.RowCount() > 0 THEN
			...
			IF li_result = -1 THEN guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_pax] 1: Error Rowscopy / Source: " + ls_dataobject_source + " / Target: " + ls_dataobject_target ) // 29.10.2013 TBR
		END IF
	...
	if itype_source = 1 then 	// Format 1: dw_change_cen_out_passenger
		...
		IF	dwsource.RowCount() > 0 THEN
			...
			IF li_result = -1 THEN guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_pax] 2: Error Rowscopy / Source: " + ls_dataobject_source + " / Target: " + ls_dataobject_target ) // 29.10.2013 TBR
		END IF
		...
		if itype_target = 2 then
			...
			IF	dw_format2.RowCount() > 0 THEN
				...
				IF li_result = -1 THEN guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_pax] 3: Error Rowscopy / Source: " + ls_dataobject_source + " / Target: " + ls_dataobject_target ) // 29.10.2013 TBR
			END IF
		...
		if itype_target = 3 then
			...
			IF	dw_format3.RowCount() > 0 THEN
				...
				IF li_result = -1 THEN guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_pax] 4: Error Rowscopy / Source: " + ls_dataobject_source + " / Target: " + ls_dataobject_target ) // 29.10.2013 TBR
			END IF
	...
	elseif itype_source = 2 then  		// Format 2: dw_invoice_ml_cen_out_passenger
		if itype_target = 1 then
			...
			IF	dwsource.RowCount() > 0 THEN
				...
				IF li_result = -1 THEN guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_pax] 5: Error Rowscopy / Source: " + ls_dataobject_source + " / Target: " + ls_dataobject_target ) // 29.10.2013 TBR
			END IF
			...
			IF	dw_format1.RowCount() > 0 THEN
				...
				IF li_result = -1 THEN guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_pax] 6: Error Rowscopy / Source: " + ls_dataobject_source + " / Target: " + ls_dataobject_target ) // 29.10.2013 TBR
			END IF
		...
		if itype_target = 3 then
			...
			IF	dwsource.RowCount() > 0 THEN
				...
				ls_dataobject_target 	= 	dw_format2.DataObject
				li_result = dwsource.Rowscopy(1, dwsource.rowcount(),Primary!,dw_format2,1,PRIMARY!)
				IF li_result = -1 THEN guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_pax] 7: Error Rowscopy / Source: " + ls_dataobject_source + " / Target: " + ls_dataobject_target ) // 29.10.2013 TBR
			END IF
			...
			IF	dw_format3.RowCount() > 0 THEN
				...
				ls_dataobject_target 	= 	dwtarget.DataObject
				li_result = dw_format3.Rowscopy(lstartrow_source, lendrow_source,Primary!,dwtarget,lstartrow_target,PRIMARY!)
				IF li_result = -1 THEN guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_pax] 8: Error Rowscopy / Source: " + ls_dataobject_source + " / Target: " + ls_dataobject_target ) // 29.10.2013 TBR
			END IF
	...
	elseif itype_source = 3 then  		// Format 3: dw_invoice_ml_cen_out_passenger
		if itype_target = 1 then
			...
			IF	dwsource.RowCount() > 0 THEN
				...
				IF li_result = -1 THEN guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_pax] 9: Error Rowscopy / Source: " + ls_dataobject_source + " / Target: " + ls_dataobject_target ) // 29.10.2013 TBR
			END IF
			...
			IF	dw_format1.RowCount() > 0 THEN
				...
				ls_dataobject_target 	= 	dwtarget.DataObject
				li_result = dw_format1.Rowscopy(lstartrow_source, lendrow_source,Primary!,dwtarget,lstartrow_target,PRIMARY!)
				IF li_result = -1 THEN guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_pax] 10: Error Rowscopy / Source: " + ls_dataobject_source + " / Target: " + ls_dataobject_target ) // 29.10.2013 TBR
			END IF
		...
		if itype_target = 2 then
			...
			IF	dwsource.RowCount() > 0 THEN
				...
				ls_dataobject_target 	= 	dw_format3.DataObject
				li_result = dwsource.Rowscopy(1, dwsource.rowcount(),Primary!,dw_format3,1,PRIMARY!)
				IF li_result = -1 THEN guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_pax] 11: Error Rowscopy / Source: " + ls_dataobject_source + " / Target: " + ls_dataobject_target ) // 29.10.2013 TBR
			END IF
			...
			IF	dw_format2.RowCount() > 0 THEN
				...
				ls_dataobject_target 	= 	dwtarget.DataObject
				li_result = dw_format2.Rowscopy(lstartrow_source, lendrow_source,Primary!,dwtarget,lstartrow_target,PRIMARY!)
				IF li_result = -1 THEN guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_pax] 12: Error Rowscopy / Source: " + ls_dataobject_source + " / Target: " + ls_dataobject_target ) // 29.10.2013 TBR
			END IF
...
if lFind > 0 then
 ...
 guoLog.uf_allways("["+ this.classname( )+".wf_chc_add_to_protocol] add to protocol: "+arg_message)
...
if dspax2.RowCount() > 0 then
	...
	//
	guoLog.uf_debug("["+ this.classname( )+".wf_chc_change_pax] dspax2.RowCount =0")
	if not bImportMode then
		 guoLog.uf_error("["+ this.classname( )+".wf_chc_change_pax] Klasseninformationen sind fehlerhaft. $$HEX1$$c400$$ENDHEX$$nderungen k$$HEX1$$f600$$ENDHEX$$nnen nicht vorgenommen werden!")
	end if
...
if bDoMealCalculation = true then
	...
	if dsSPML.RowsCopy(1,dsSPML.RowCount(),Primary!,dw_spml,1,Primary!) = -1 and dsSPML.RowCount()>0 then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_change_spml: error rowscopy  dsSPML to dw_spml / Source: " + dsSPML.dataobject + " / Target: " + dw_spml.dataobject)
	end if
	dw_spml.Sort()
...
If dsChanged_Handling.rowcount()  > 0 Then
	...
	If iReturn <> 1 Then
		if bImportMode then
			...
			guoLog.uf_error("["+ this.classname( )+".wf_chc_change_newspaper] Lesematerial-Beladung kann nicht eingetragen werden.")
		End if
		return False
	End if
End if
...
If dsChanged_Extra.rowcount()  > 0 Then
	...
	If iReturn <> 1 Then
		if bImportMode then
			...
			guoLog.uf_error("["+ this.classname( )+".wf_chc_change_newspaper] Lesematerial-Extrabeladung kann nicht eingetragen werden.")
		End if
		return False
	End if
End if
...
if dw_handling.Find("nmanual_input = 1",1,dw_handling.Rowcount()) > 0 then
	if bImportMode then
		...
		if dw_single.getitemnumber(dw_single.getrow(), "nlock_loadinglist")=  1 then
			guoLog.uf_debug("["+ this.classname( )+".wf_chc_change_handling] Es wurden manuelle $$HEX1$$c400$$ENDHEX$$nderungen an der Beladung vorgenommen, daher keine Neuberechnung")
			return true
		end if
	end if
end if
...
If dsChanged.Rowcount() > 0 Then
	...
	If iReturn <> 1 Then
		if bImportMode then
			...
			guoLog.uf_error("["+ this.classname( )+".wf_chc_change_handling] Handling kann nicht eingetragen werden.")
			return False
		end if
	end if
...
//			if dw_extra.GetItemNumber(i,"nmanual_processing") = 0 then 
	...
	if dsextra.RowsCopy(1,dsextra.RowCount(),Primary!,dw_extra,1,Primary!) = -1 and dsextra.RowCount()>0 then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_change_extra] error rowscopy  dsextra to dw_extra  / Source: " + dsextra.dataobject + " / Target: " + dw_extra.dataobject)
	end if	
	dw_extra.Sort()
...
IF	dw_format2.RowCount() > 0 THEN
	...
	ls_dataobject_target 	= 	dwtarget.DataObject
	li_result = dw_format2.Rowscopy(lstartrow_source, lendrow_source,Primary!,dwtarget,lstartrow_target,PRIMARY!)
	IF li_result = -1 THEN guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_single] Error Rowscopy / Source: " + ls_dataobject_source + " / Target: " + ls_dataobject_target ) // 29.10.2013 TBR
END IF
...
if not bReadFromHistory then
	if dw_pax.RowCount() > 0 then
		...
		if i = -1 then guoLog.uf_error("["+ this.classname( )+".wf_chc_set_old_values] Error Rowscopy dw_pax  / Source: " + dw_pax.dataobject + " / Target: " + dsPaxOld.dataobject) //28.02.2011 HR: Fehlerpr$$HEX1$$fc00$$ENDHEX$$fung eingebaut
	end if

	if dw_extra.RowCount() > 0 then
		i=dw_extra.RowsCopy(1,dw_extra.RowCount(),Primary!,dsExtraOld,dsExtraOld.RowCount() + 1, Primary!)
		if i = -1 then guoLog.uf_error("["+ this.classname( )+".wf_chc_set_old_values] Error Rowscopy dw_extra  / Source: " + dw_extra.dataobject + " / Target: " + dsExtraOld.dataobject) //28.02.2011 HR: Fehlerpr$$HEX1$$fc00$$ENDHEX$$fung eingebaut
	end if

	if dw_meal.RowCount() > 0 then
		i=dw_meal.RowsCopy(1,dw_meal.RowCount(),Primary!,dsMealOld,dsMealOld.RowCount() + 1, Primary!)
		if i = -1 then guoLog.uf_error("["+ this.classname( )+".wf_chc_set_old_values] Error Rowscopy dw_meal  / Source: " + dw_meal.dataobject + " / Target: " + dsMealOld.dataobject) //28.02.2011 HR: Fehlerpr$$HEX1$$fc00$$ENDHEX$$fung eingebaut
	end if

	if dw_spml.RowCount() > 0 then
		i = dw_spml.RowsCopy(1,dw_spml.RowCount(),Primary!,dsSPMLOld,dsSPMLOld.RowCount() + 1, Primary!)
		if i = -1 then guoLog.uf_error("["+ this.classname( )+".wf_chc_set_old_values] Error Rowscopy dw_spml  / Source: " + dw_spml.dataobject + " / Target: " + dsSPMLOld.dataobject) //28.02.2011 HR: Fehlerpr$$HEX1$$fc00$$ENDHEX$$fung eingebaut
	end if
...
guoLog.uf_debug("["+ this.classname( )+".wf_chc_save] Begin")
...
// ------------------------------------------------------------------
if wf_chc_validation() <> 0 then
	guoLog.uf_debug("["+ this.classname( )+".wf_chc_save] wf_chc_validation returns error")
	bInAction = false
	return false
end if
...
If bNoSaving = True Then
	guoLog.uf_debug("["+ this.classname( )+".wf_chc_save] bNoSaving = True")

	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Die aktuellen Ver$$HEX1$$e400$$ENDHEX$$nderungen lassen ein Speichern nicht zu.")
	end if
	bInAction = false
	bforcemealcalculation = false
	bAnychange = false
...
if wf_chc_force_meal_calculation() = -1 then
	if bImportMode then
		...
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Die Erstellung der Beladung ist fehlgeschlagen. Speichern ist nicht m$$HEX1$$f600$$ENDHEX$$glich")
	end if
	bInAction = false
	bforcemealcalculation = false
	bAnychange = false
...
If wf_chc_check_departure() = False Then
	if bImportMode then
		...
		if iSaveAfterDeparture = 0 then
			guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Das aktuelle Tagesdatum-/zeit ist gr$$HEX2$$f600df00$$ENDHEX$$er als das Abflugsdatum-/zeit. Speichern ist nicht m$$HEX1$$f600$$ENDHEX$$glich!",Stopsign!)
			bInAction = false
			bforcemealcalculation = false
			bAnychange = false
			return False		
			...
			//
			//guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Das aktuelle Tagesdatum-/zeit ist gr$$HEX2$$f600df00$$ENDHEX$$er als das Abflugsdatum-/zeit. Es werden keine $$HEX1$$c400$$ENDHEX$$nderungsmitteilungen erzeugt!",Information!)
	
			// F$$HEX1$$fc00$$ENDHEX$$r Monalisa Istdatenbrowser gilt : generell  keine $$HEX1$$c400$$ENDHEX$$nderungsmitteilungen verschicken
		/*	if iForceTimeChange = 0 then
				if guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Das aktuelle Tagesdatum-/zeit ist gr$$HEX2$$f600df00$$ENDHEX$$er als das Abflugsdatum-/zeit. Bitte pr$$HEX1$$fc00$$ENDHEX$$fen Sie die Abflugzeit." + &
								"  M$$HEX1$$f600$$ENDHEX$$chten Sie $$HEX1$$c400$$ENDHEX$$nderungsmitteilungen versenden?",Question!,YesNo!,1) = 1 then
...
If wf_chc_check_loading_list() = False Then
	if bImportMode then
		...
		if dw_single.GetItemNumber(dw_single.GetRow(),"nflight_status") = 0 then
			guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Dieser Flug hat keine Standardbeladung. Speichern ist nicht m$$HEX1$$f600$$ENDHEX$$glich!")
			bInAction = false
			bforcemealcalculation = false
			bAnychange = false
			return False		
...
for i = 1 to dw_handling.RowCount()
	if dw_handling.GetItemNumber(i,"new_row") = 1 then
		...
		if lFind > 0 then
			if not bimportmode then
				guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Sie d$$HEX1$$fc00$$ENDHEX$$rfen nicht mehrmals die gleiche Sammelst$$HEX1$$fc00$$ENDHEX$$ckliste verwenden!")
				bAnychange = false
				return false
			end if
		end if
...
if bPaxModified = true then
	if bMealsCalculated = false then
		if bImportMode then
			...
			guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Es ist keine Neuberechnung der Mahlzeiten erfolgt.")
		end if
		bInAction = false
		bAnychange = false
		return False
...
If dw_single.Update() <> 1 Then
	...
	if bImportMode then
		...
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Fehler beim Speichern der Fluginformationen.")
	end if
	bInAction = false
	bAnychange = false
	return False
...
If dw_single.update() = -1 Then
	...
	if bImportMode then
		...
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Fehler beim Speichern der History Fluginformationen.")
	end if
...
If dw_pax.Update() <> 1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Fehler beim Speichern der Passagierinformationen.")
	end if
	bInAction = false
	bAnychange = false
	return False
...
If dw_pax.update() = -1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Fehler beim Speichern der History Passagierinformationen.")
	end if
...
If idw_loading.Update() <> 1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Fehler beim Speichern der Loadfinginformationen.")
	end if
	bInAction = false
	bAnychange = false
	return False
...
idw_loading.ResetUpdate()

sTableName	= idw_loading.Object.DataWindow.Table.UpdateTable
For i = 1 to idw_loading.Rowcount()
		idw_loading.Setitemstatus(i,0,Primary!,NewModified!)

Next
idw_loading.Object.DataWindow.Table.UpdateTable = sTableName + "_history"
If idw_loading.update() = -1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Fehler beim Speichern der History Loadinginformationen.")
	end if
	idw_loading.Object.DataWindow.Table.UpdateTable = sTableName
	bInAction = false
	bAnychange = false
	return False
Else
...
If dw_handling.Update() <> 1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Fehler beim Speichern der Handlinginformationen.")
	end if
	bInAction = false
	bAnychange = false
	return False
...
If dw_text_info.Update() <> 1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Fehler beim Speichern der Textinformationen.")
	end if
	bInAction = false
	return False
else
...
If dw_text_infos_areas.Update() <> 1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Fehler beim Speichern der Textinformationen f$$HEX1$$fc00$$ENDHEX$$r Bereiche.")
	end if
	bInAction = false
	return False
End if
...
If dw_text_info.update() = -1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Fehler beim Speichern der History Textinformationen.")
	end if
...
If dw_text_infos_areas.update() = -1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Fehler beim Speichern der History Textinformationen f$$HEX1$$fc00$$ENDHEX$$r Bereiche.")
	end if
...
guoLog.uf_debug("["+ this.classname( )+".wf_chc_save] Speichern der Extrabeladung Start")
//wf_setmessage("Speichern der Extrabeladung")
If dw_extra.Update() <> 1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Fehler beim Speichern der Extrabeladung.")
	end if
	bInAction = false
	return False
End if
guoLog.uf_debug("["+ this.classname( )+".wf_chc_save] Speichern der Extrabeladung Ende")
// ------------------------------------------------------------------------
// Speichern von dw_extra History
// ------------------------------------------------------------------------
guoLog.uf_debug("["+ this.classname( )+".wf_chc_save] Speichern der History Extrabeladung Start")
...
If dw_extra.update() = -1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Fehler beim Speichern der History Extrabeladung.")
	end if
...
guoLog.uf_debug("["+ this.classname( )+".wf_chc_save] Speichern der History Extrabeladung Ende")

// ------------------------------------------------------------------------
// Speichern von dw_meal
// ------------------------------------------------------------------------

guoLog.uf_debug("["+ this.classname( )+".wf_chc_save] Speichern der Mahlzeitenbeladung Start")
//wf_setmessage("Speichern der Mahlzeitenbeladung")
i=dw_meal.Rowcount()
If dw_meal.Update() <> 1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Fehler beim Speichern der Mahlzeitenbeladung.")
	end if
	bInAction = false
	return False
End if
guoLog.uf_debug("["+ this.classname( )+".wf_chc_save] Speichern der Mahlzeitenbeladung Ende")
// ------------------------------------------------------------------------
// Speichern von dw_meal History
// ------------------------------------------------------------------------
guoLog.uf_debug("["+ this.classname( )+".wf_chc_save] Speichern der History Mahlzeitenbeladung Start")
...
If dw_meal.update() = -1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Fehler beim Speichern der History Mahlzeitenbeladung.")
	end if
...
guoLog.uf_debug("["+ this.classname( )+".wf_chc_save] Speichern der History Mahlzeitenbeladung Ende")
// ------------------------------------------------------------------------
// Speichern von dw_spml
// ------------------------------------------------------------------------
guoLog.uf_debug("["+ this.classname( )+".wf_chc_save] Speichern der Special Meal Start")
...
If dw_spml.Update() <> 1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Fehler beim Speichern der Special Meal. "+dw_spml.sSQLErrorText)
	end if
...
guoLog.uf_debug("["+ this.classname( )+".wf_chc_save] Speichern der Special Meal Ende")

// ------------------------------------------------------------------------
// Speichern von dw_spml History
// ------------------------------------------------------------------------
guoLog.uf_debug("["+ this.classname( )+".wf_chc_save] Speichern der Special Meal History Start")
...
If dw_spml.update() = -1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Fehler beim Speichern der History Special Meal.")
	end if
...
If dsSPMLDetail.update() <> 1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Fehler beim Speichern der Special Meal Details.")
	end if
...
If dsSPMLDetail.update() <> 1 Then
	...
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Fehler beim Speichern der Special Meal Details History.")
	end if
	//f_print_datastore(dsSPMLDetail)
...
Else
	dsSPMLDetail.Object.DataWindow.Table.UpdateTable = sTableName
End if
guoLog.uf_debug("["+ this.classname( )+".wf_chc_save] Speichern der Special Meal History Ende")
// ------------------------------------------------------------------------
// Zeitungsbeladung
// ------------------------------------------------------------------------
If	bNewspaperChange = True Then
	...
	If SQLCA.SQLCODE <> 0 Then
		rollback ;
		if not bImportMode then
			guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Fehler beim L$$HEX1$$f600$$ENDHEX$$schen der Lesematerial-Beladung.")
		end if
		bInAction = false
		return False
	End if
	...
	If SQLCA.SQLCODE <> 0 Then
		rollback ;
		if not bImportMode then
			guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Fehler beim L$$HEX1$$f600$$ENDHEX$$schen der Lesematerial-Extrabeladung.")
		end if
		bInAction = false
		return False
	End if
...
If dsCenOutHandlingNews.rowcount()  > 0 Then
	...
	If dsCenOutHandlingNews.Update() <> 1 Then
		rollback ;
		if not bImportMode then
			guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Fehler beim Speichern der Lesematerial-Beladung.")
		end if
		bInAction = false
		return False
	End if
...
//	wf_setmessage("Speichern der History Lesematerial-Beladung.")
	...
	If dsCenOutHandlingNews.update() = -1 Then
		rollback ;
		if not bImportMode then
			guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Fehler beim Speichern der History Lesematerial-Beladung.")
		end if
...
If dsCenOutNewsExtra.rowcount()  > 0 Then
	...
	If dsCenOutNewsExtra.Update() <> 1 Then
		rollback ;
		if not bImportMode then
			guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Fehler beim Speichern der Lesematerial-Extrabeladung.")
		end if
		bInAction = false
		return False
	End if
	...
	If dsCenOutNewsExtra.update() = -1 Then
		rollback ;
		if not bImportMode then
			guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Fehler beim Speichern der History Lesematerial-Extrabeladung.")
		end if
...
If dsChangeInformation.Update() <> 1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save] Fehler beim Speichern der $$HEX1$$c400$$ENDHEX$$nderungsinformationen.")
	end if
	bInAction = false
	return False
End if
...
//25.05.2023 HR AlmID 8358: 2 new generation jobs requested
If integer(f_mandant_profilestring(sqlca, s_app.smandant, "DocumentEngine","EnableChangeCenterQueue","0")) = 1 or ib_create_docenginejob_after_recalc Then
	luo_doc_gen_queue = create uo_doc_gen_queue
	
	luo_doc_gen_queue.bCommit = bEnableCommit
	li_Succ = luo_doc_gen_queue.of_enqueue_flight( lResultKeyCenOut)
	guoLog.uf_allways("["+ this.classname( )+".wf_chc_save] Doc Gen Queue returns "+string(li_Succ))
	destroy luo_doc_gen_queue
else
	guoLog.uf_debug("["+ this.classname( )+".wf_chc_save] No DocGenQueue")
End if
...
//	wf_setmessage("Versende $$HEX1$$c400$$ENDHEX$$nderungsmitteilungen.")
	...
	if not uo_messages.uf_generate_message(sViewUnit, lresultkey, ltransactionkey,iLegNumber) then
		guoLog.uf_debug("["+ this.classname( )+".wf_chc_save] Fehler beim Versenden von $$HEX1$$c400$$ENDHEX$$nderungsmitteilungen.")
		rollback ;
		bMultiLegError = true
	else
		guoLog.uf_debug("["+ this.classname( )+".wf_chc_save] $$HEX1$$c400$$ENDHEX$$nderungsmitteilungen erfolgreich gespeichert.")
		commit ;
	end if
...
guoLog.uf_debug("["+ this.classname( )+".wf_chc_save] GarbageCollect")
GarbageCollect()
...
guoLog.uf_debug("["+ this.classname( )+".wf_chc_save] Fertig")
...
// ------------------------------------------------------------------
If bNoSaving = True Then
	guoLog.uf_allways("["+ this.classname( )+".wf_chc_save_final_pax] bNoSaving = True")

	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Die aktuellen Ver$$HEX1$$e400$$ENDHEX$$nderungen lassen ein Speichern nicht zu.")
	end if
	bInAction = false
	bforcemealcalculation = false
	return false
...
If dw_single.Update() <> 1 Then
	...
	if bImportMode then
		...
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Fehler beim Speichern der Fluginformationen.")
	end if
	bInAction = false
	return False
End if
...
If dw_single.update() = -1 Then
	...
	if bImportMode then
		...
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Fehler beim Speichern der History Fluginformationen.")
	end if
	dw_single.Object.DataWindow.Table.UpdateTable = sTableName
	bInAction = false
	return False
...
If dw_pax.Update() <> 1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Fehler beim Speichern der Passagierinformationen.")
	end if
	bInAction = false
	return False
End if
...
If dw_pax.update() = -1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Fehler beim Speichern der History Passagierinformationen.")
	end if
	dw_pax.Object.DataWindow.Table.UpdateTable = sTableName
	bInAction = false
	return False
...
If idw_loading.Update() <> 1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Fehler beim Speichern der Loadfinginformationen.")
	end if
	bInAction = false
	bAnychange = false
	return False
...
If idw_loading.update() = -1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Fehler beim Speichern der History Loadinginformationen.")
	end if
	idw_loading.Object.DataWindow.Table.UpdateTable = sTableName
	bInAction = false
	bAnychange = false
...
If dw_handling.Update() <> 1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Fehler beim Speichern der Handlinginformationen.")
	end if
	bInAction = false
	return False
End if
...
If dw_text_info.Update() <> 1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Fehler beim Speichern der Textinformationen.")
	end if
	bInAction = false
	return False
End if
If dw_text_infos_areas.Update() <> 1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Fehler beim Speichern der Textinformationen f$$HEX1$$fc00$$ENDHEX$$r Bereiche.")
	end if
	bInAction = false
	return False
End if
...
If dw_text_info.update() = -1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Fehler beim Speichern der History Textinformationen.")
	end if
	dw_text_info.Object.DataWindow.Table.UpdateTable = sTableName
	bInAction = false
	return False
...
If dw_text_infos_areas.update() = -1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Fehler beim Speichern der History Textinformationen f$$HEX1$$fc00$$ENDHEX$$r Bereiche.")
	end if
	dw_text_infos_areas.Object.DataWindow.Table.UpdateTable = sTableName
	bInAction = false
	return False
...
guoLog.uf_debug("["+ this.classname( )+".wf_chc_save_final_pax] Speichern der Extrabeladung Start")
//wf_setmessage("Speichern der Extrabeladung")
If dw_extra.Update() <> 1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Fehler beim Speichern der Extrabeladung.")
	end if
	bInAction = false
	return False
End if
guoLog.uf_debug("["+ this.classname( )+".wf_chc_save_final_pax] Speichern der Extrabeladung Ende")
// ------------------------------------------------------------------------
// Speichern von dw_extra History
// ------------------------------------------------------------------------
guoLog.uf_debug("["+ this.classname( )+".wf_chc_save_final_pax] Speichern der History Extrabeladung Start")
//wf_setmessage("Speichern der History Extrabeladung.")
dw_extra.ResetUpdate()
sTableName	= dw_extra.Object.DataWindow.Table.UpdateTable
For i = 1 to dw_extra.Rowcount()
...
If dw_extra.update() = -1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Fehler beim Speichern der History Extrabeladung.")
	end if
	dw_extra.Object.DataWindow.Table.UpdateTable = sTableName
	bInAction = false
	return False
...
guoLog.uf_debug("["+ this.classname( )+".wf_chc_save_final_pax] Speichern der History Extrabeladung Ende")

// ------------------------------------------------------------------------
// Speichern von dw_meal
// ------------------------------------------------------------------------

guoLog.uf_debug("["+ this.classname( )+".wf_chc_save_final_pax] Speichern der Mahlzeitenbeladung Start")
//wf_setmessage("Speichern der Mahlzeitenbeladung")
If dw_meal.Update() <> 1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Fehler beim Speichern der Mahlzeitenbeladung.")
	end if
	bInAction = false
	return False
End if
guoLog.uf_debug("["+ this.classname( )+".wf_chc_save_final_pax] Speichern der Mahlzeitenbeladung Ende")
// ------------------------------------------------------------------------
// Speichern von dw_meal History
// ------------------------------------------------------------------------
guoLog.uf_debug("["+ this.classname( )+".wf_chc_save_final_pax] Speichern der History Mahlzeitenbeladung Start")
//wf_setmessage("Speichern der History Mahlzeitenbeladung.")
dw_meal.ResetUpdate()
sTableName	= dw_meal.Object.DataWindow.Table.UpdateTable
For i = 1 to dw_meal.Rowcount()
...
If dw_meal.update() = -1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Fehler beim Speichern der History Mahlzeitenbeladung.")
	end if
	dw_meal.Object.DataWindow.Table.UpdateTable = sTableName
	bInAction = false
	return False
...
guoLog.uf_debug("["+ this.classname( )+".wf_chc_save_final_pax] Speichern der History Mahlzeitenbeladung Ende")
// ------------------------------------------------------------------------
// Speichern von dw_spml
// ------------------------------------------------------------------------
//wf_setmessage("Speichern der Special Meal")
If dw_spml.Update() <> 1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Fehler beim Speichern der Special Meal.")
	end if
	bInAction = false
	return False
End if
...
If dw_spml.update() = -1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Fehler beim Speichern der History Special Meal.")
	end if
	dw_spml.Object.DataWindow.Table.UpdateTable = sTableName
	bInAction = false
	return False
...
If dsSPMLDetail.update() <> 1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Fehler beim Speichern der Special Meal Details.")
	end if
	//f_print_datastore(dsSPMLDetail)
	bInAction = false
	return False
...
If dsSPMLDetail.update() <> 1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Fehler beim Speichern der Special Meal Details History.")
	end if
	//f_print_datastore(dsSPMLDetail)
	dsSPMLDetail.Object.DataWindow.Table.UpdateTable = sTableName
	bInAction = false
...
If	bNewspaperChange = True Then
	...
	If SQLCA.SQLCODE <> 0 Then
		rollback ;
		if not bImportMode then
			guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Fehler beim L$$HEX1$$f600$$ENDHEX$$schen der Lesematerial-Beladung.")
		end if
		bInAction = false
		return False
	End if
	...
	If SQLCA.SQLCODE <> 0 Then
		rollback ;
		if not bImportMode then
			guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Fehler beim L$$HEX1$$f600$$ENDHEX$$schen der Lesematerial-Extrabeladung.")
		end if
		bInAction = false
		return False
	End if
...
//	wf_setmessage("Speichern der Lesematerial-Beladung")
	If dsCenOutHandlingNews.Update() <> 1 Then
		rollback ;
		if not bImportMode then
			guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Fehler beim Speichern der Lesematerial-Beladung.")
		end if
		bInAction = false
		return False
	End if
...
//	wf_setmessage("Speichern der History Lesematerial-Beladung.")
	...
	If dsCenOutHandlingNews.update() = -1 Then
		rollback ;
		if not bImportMode then
			guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Fehler beim Speichern der History Lesematerial-Beladung.")
		end if
		dsCenOutHandlingNews.Object.DataWindow.Table.UpdateTable = sTableName
		bInAction = false
		return False
...
//	wf_setmessage("Speichern der Lesematerial-Extrabeladung")
	If dsCenOutNewsExtra.Update() <> 1 Then
		rollback ;
		if not bImportMode then
			guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Fehler beim Speichern der Lesematerial-Extrabeladung.")
		end if
		bInAction = false
		return False
	End if
...
//	wf_setmessage("Speichern der History Lesematerial-Extrabeladung.")
	...
	If dsCenOutNewsExtra.update() = -1 Then
		rollback ;
		if not bImportMode then
			guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Fehler beim Speichern der History Lesematerial-Extrabeladung.")
		end if
		dsCenOutNewsExtra.Object.DataWindow.Table.UpdateTable = sTableName
		bInAction = false
		return False
...
If dsChangeInformation.Update() <> 1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_save_final_pax] Fehler beim Speichern der $$HEX1$$c400$$ENDHEX$$nderungsinformationen.")
	end if
	bInAction = false
	return False
End if
...
// -------------------------------------------------------------------------
//wf_setmessage("GarbageCollect")
guoLog.uf_debug("["+ this.classname( )+".wf_chc_save_final_pax] GarbageCollect")
GarbageCollect()
...
guoLog.uf_debug("["+ this.classname( )+".wf_chc_save_final_pax] Rest")
...
// ------------------------------------------------------------------
	...
	If lTransActionKey = -1 Then
		if bImportMode then
			...
			guoLog.uf_error("["+ this.classname( )+".wf_chc_set_changed_status] Schl$$HEX1$$fc00$$ENDHEX$$ssel f$$HEX1$$fc00$$ENDHEX$$r History kann nicht ermittelt werden.")
		end if
		return False
	End if
...
// -----------------------------------------	
	...
	// --------------------------------------------------------------------------------------------------------------------
	for i = 1 to idw_loading.RowCount()
		idw_loading.SetItem(i, "ntransaction", lTransActionKey)
	next
...
guoLog.uf_debug("["+ this.classname( )+".wf_chc_changed_information] Begin")
...
// --------------------------------------------------------------------------------------------------------------------
iStatus = 0
if idw_loading.modifiedcount() > 0 then
	for i = 1 to idw_loading.rowcount()
		if idw_loading.GetItemNumber(i,"nchanged") 	= 1 then
			iStatus = 1
			exit
		end if  
	next	
end if
dsChangeInformation.Setitem(lNewRow,"nloading",iStatus)

guoLog.uf_debug("["+ this.classname( )+".wf_chc_changed_information] Ende")
...
guoLog.uf_allways("["+ this.classname( )+".wf_chc_request_explosion] Create Job for FlightExplosion" )
...
//--------------------------------------------------------------------------------------------------
	...
	For lrow = 1 to dw_pax.rowcount()
		...
		if lfind <= 0 then
			...
			guoLog.uf_trace("["+ this.classname( )+".wf_chc_get_differences] dw_pax   | New entry class " + String(dw_pax.getItemNumber(lrow,"nclass_number")) + " npax "+ String(dw_pax.GetItemNumber(lrow,"npax")) )
		else
			if dw_pax.GetItemNumber(lrow,"npax") <> &
				 dsPaxOld.GetItemNumber(lfind,"npax") then
				
				guoLog.uf_trace("["+ this.classname( )+".wf_chc_get_differences] dw_pax   |Changed - class " + String(dw_pax.getItemNumber(lrow,"nclass_number")) + " npax new: "+ String(dw_pax.GetItemNumber(lrow,"npax")) + " old: "+String(dsPaxOld.GetItemNumber(lfind,"npax") ))
				...
				guoLog.uf_trace("["+ this.classname( )+".wf_chc_get_differences] dw_pax   |Unchanged - class " + String(dw_pax.getItemNumber(lrow,"nclass_number")) + " npax "+ String(dw_pax.GetItemNumber(lrow,"npax")) )
			...
			if dw_pax.GetItemNumber(lrow,"npax_airline") <> &
				...
				guoLog.uf_trace("["+ this.classname( )+".wf_chc_get_differences] dw_pax   |Changed - class " + String(dw_pax.getItemNumber(lrow,"nclass_number")) + " npax_airline new: "+ String(dw_pax.GetItemNumber(lrow,"npax_airline")) + " old: "+String(dsPaxOld.GetItemNumber(lfind,"npax_airline") ))
				dw_pax.SetItemStatus(lrow, "npax_airline", Primary!, DataModified!)
				strUserChanges.bPaxChanged = true
				wf_chc_show_changes(dw_pax,"npax",lrow)
			else
				guoLog.uf_trace("["+ this.classname( )+".wf_chc_get_differences] dw_pax   |Unchanged - class " + String(dw_pax.getItemNumber(lrow,"nclass_number")) + " npax_airline "+ String(dw_pax.GetItemNumber(lrow,"npax_airline")) )
	...
	For lrow = 1 to dw_meal.rowcount()
		...
		if lfind <= 0 then
			dw_meal.SetItemStatus(lrow, 0, Primary!, DataModified!)
			 bAnychange = true
			 bManualChanges = true
		//	 guoLog.uf_trace("["+ this.classname( )+".wf_chc_get_differences] dw_meal | New entry class " + String(dw_pax.getItemNumber(lrow,"nclass_number")) + " npax "+ String(dw_pax.GetItemNumber(lrow,"npax")) )
	...
	//-------------------------------------------------------------------------
	guoLog.uf_trace("["+ this.classname( )+".wf_chc_get_differences] dw_spml | No of entries: new: " + String(dw_spml.rowcount() ) + " old: " + String(dsSpmlOld.rowcount()  ))
	if dw_spml.rowcount() <> dsSpmlold.rowcount() then
			 bAnychange = true
			 bManualChanges = true
		     bForceMealCalculation = true
	...
	For lrow = 1 to dw_spml.rowcount()
		...
		if lfind <= 0 then
			if bReadFromHistory then
		     ...
		     bForceMealCalculation = true
			this.bDoMealCalculation = true
			guoLog.uf_trace("["+ this.classname( )+".wf_chc_get_differences] dw_spml | New entry class " + String(dw_Spml.getItemNumber(lrow,"nclass_number")) + " cspml " + dw_Spml.getItemString(lrow,"cspml") + " nquantity "+ String( dw_Spml.GetItemNumber(lrow,"nquantity")) )
			...
			if dw_Spml.GetItemNumber(lrow,"nquantity") <> &
				 ...
				 //bAnychange = true
				//bManualChanges = true
				guoLog.uf_trace("["+ this.classname( )+".wf_chc_get_differences] dw_spml | Changed class " + String(dw_Spml.getItemNumber(lrow,"nclass_number")) + " cspml " + dw_Spml.getItemString(lrow,"cspml") + " nquantity new: "+ String( dw_Spml.GetItemNumber(lrow,"nquantity")) + " old: " + String(dsSpmlOld.GetItemNumber(lfind,"nquantity")) )
				...
				guoLog.uf_trace("["+ this.classname( )+".wf_chc_get_differences] dw_spml | Unchanged class " + String(dw_Spml.getItemNumber(lrow,"nclass_number")) + " cspml " + dw_Spml.getItemString(lrow,"cspml") + " nquantity "+ String( dw_Spml.GetItemNumber(lrow,"nquantity")) )
		...
		// ---------------------------------------
			if dw_Spml.GetItemNumber(lrow,"ntopoff") <> &
			  dsSpmlOld.GetItemNumber(lfind,"ntopoff") then
					...
					bForceMealCalculation = true
				guoLog.uf_trace("["+ this.classname( )+".wf_chc_get_differences] dw_spml | Changed class " + String(dw_Spml.getItemNumber(lrow,"nclass_number")) + " cspml " + dw_Spml.getItemString(lrow,"cspml") + " ntopoff new: "+ String( dw_Spml.GetItemNumber(lrow,"ntopoff")) + " old: " + String(dsSpmlOld.GetItemNumber(lfind,"ntopoff")) )
		...
		// ---------------------------------------
			...
			if dw_Spml.GetItemString(lrow,"cadd_text") <> &
			  dsSpmlOld.GetItemString(lfind,"cadd_text") then
				...
				guoLog.uf_trace("["+ this.classname( )+".wf_chc_get_differences] dw_spml | Changed class " + String(dw_Spml.getItemNumber(lrow,"nclass_number")) + " cspml " + dw_Spml.getItemString(lrow,"cspml") + " cadd_text new: "+ dw_Spml.GetItemString(lrow,"cadd_text") + " old: " + dsSpmlOld.GetItemString(lfind,"cadd_text") )
	...
	// ---------------------------
		...
		*/
				guoLog.uf_trace("["+ this.classname( )+".wf_chc_get_differences] dw_spml | Changed class " + String(dw_Spml.getItemNumber(lrow,"nclass_number")) + " cspml " + dw_Spml.getItemString(lrow,"cspml") + " cclass new: "+ dw_Spml.GetItemString(lrow,"cclass") + " old: " + dsSpmlOld.GetItemString(lfind,"cclass") )
	...
	// ---------------------------
			if dw_Spml.GetItemString(lrow,"cspml") <> &
			  dsSpmlOld.GetItemString(lfind,"cspml") then
			  	guoLog.uf_trace("["+ this.classname( )+".wf_chc_get_differences] dw_spml | Changed class " + String(dw_Spml.getItemNumber(lrow,"nclass_number")) + " cspml " + dw_Spml.getItemString(lrow,"cspml") + " cspml new: "+ dw_Spml.GetItemString(lrow,"cspml") + " old: " + dsSpmlOld.GetItemString(lfind,"cspml") )
...
//											,1,dsValidSPML.RowCount())
				...
				if lFound <= 0 then
					guoLog.uf_trace("["+ this.classname( )+".wf_chc_get_differences] dw_spml | Changed class " + String(dw_Spml.getItemNumber(lrow,"nclass_number")) + " > No Valid Spml-Key found ") 
					dw_spml.SetItem(lRow,"nspml_key",lNull)
			//	return 1
				else
					guoLog.uf_trace("["+ this.classname( )+".wf_chc_get_differences] dw_spml | Changed class " + String(dw_Spml.getItemNumber(lrow,"nclass_number")) + " > Valid Spml-Key found: " +String( dsValidSPML.GetItemNumber(lFound,"nspml_key")) )
...
if lVersionGroupKey = - 1 then
	guoLog.uf_info("["+ this.classname( )+".w_chc_master_change_selection] Ung$$HEX1$$fc00$$ENDHEX$$ltige Sitzplatzversion f$$HEX1$$fc00$$ENDHEX$$r Flugger$$HEX1$$e400$$ENDHEX$$t!")
end if
...
If strChange.laircraftkey <> dw_single.GetItemNumber(dw_single.Getrow(),"naircraft_key") Then
	...
	dw_single.SetItem(dw_single.Getrow(), "nconfiguration_key",	strChange.lVersionGroupKey) // 22.07.2020 HR: Wir speichern jetzt auch den KonfiigurationsGroupKey
	// -------------------------------------------------------------------------
...
guoLog.uf_allways("["+ this.classname( )+".wf_chc_flight_closed] (" +sMessage  + "), ii_action = " + string(ii_action))
guoLog.uf_allways("["+ this.classname( )+".wf_chc_flight_closed] dw_single.getrow() = " + string(dw_single.getrow()))
guoLog.uf_allways("["+ this.classname( )+".wf_chc_flight_closed] dw_single.GetitemDateTime(dw_single.getrow(),dtimestamp) = " + string(dw_single.GetitemDateTime(dw_single.getrow(),"dtimestamp")))
guoLog.uf_allways("["+ this.classname( )+".wf_chc_flight_closed] dtCheckdate = " + string(dtCheckdate))
...
If (ii_action = 2) Then 	// Aufruf erfolgt $$HEX1$$fc00$$ENDHEX$$ber Web
	If isnull(dw_single.GetitemDateTime(dw_single.getrow(),"dtimestamp")) Then
		...
		guoLog.uf_allways("["+ this.classname( )+".wf_chc_flight_closed] dtimestamp is Null")
	...
	Else
		...
		IF isnull(dtTimestampFromWeb) then
			guoLog.uf_allways("["+ this.classname( )+".wf_chc_flight_closed] dtimestampFromWeb is Null")
			If dw_single.GetitemDateTime(dw_single.getrow(),"dtimestamp") < dtCheckdate Then
				sMessage = uf.translate("Flug hat Abflugzeit noch nicht erreicht.")
				return False
			End if
		else
			guoLog.uf_allways("["+ this.classname( )+".wf_chc_flight_closed] dtimestampFromWeb " + String(dtTimestampFromWeb))
...
//				"' and cname = '" + sName + "' and cclass = '" + sClass + "'"
	...
	for j = 1 to dw_meal.RowCount()
		if dw_meal.GetItemString(j,"cclass") = sClass then
			...
			if lhmealkey <> lhmealkeyold then
				...
				//
				guoLog.uf_debug("["+ this.classname( )+".wf_chc_validate_spml] Spml " + sSpml + " ( " + sClass + ") " + &
	...
	if lFindDef = 0 then
		...
		guoLog.uf_debug("["+ this.classname( )+".wf_chc_validate_spml] Cspml not allowed with meals " + sSpml+ " ( " + sClass + ") ")
	end if
next
...
if dsSysQueueFlightPax.RowCount() > 0 then
	...
	for i = 1 to  dw_Pax.RowCount() 
		...
		if lFound > 0 then
			...
			if this.ifunction_pax_type = 1 	then	// npax
				guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_pax] dw_pax   |class " + sClass + " npax new: "+ String(lpax) + " old: "+String(dw_Pax.GetItemNumber(i,"npax") ))
				guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_pax] dw_pax   |class " + sClass + " nspml new: "+ String(lspml) + " old: "+String(dw_Pax.GetItemNumber(i,"npax_spml") ))
				guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_pax] dw_pax   |class " + sClass + " nstationentry new: "+ String(istation_entry) + " old: "+String(dw_Pax.GetItemNumber(i,"nstationentry") ))
				// ------------------------------------------------------------------------------------
				...
				if ii_fill_forecast=1 then
					...
					// --------------------------------------------------------------------------------
					guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_pax] dw_pax   |class " + sClass + " Pax and SPML werden auch in Forecast-Felder geschrieben")
			...
			elseif this.ifunction_pax_type = 2	then	// npax_airline
				guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_pax] dw_pax   |class " + sClass + " npax_airline new: "+ String(lpax) + " old: "+String(dw_Pax.GetItemNumber(i,"npax_airline") ))
				guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_pax] dw_pax   |class " + sClass + " nstationentry new: "+ String(istation_entry) + " old: "+String(dw_Pax.GetItemNumber(i,"nstationentry") ))
				// ------------------------------------------------------------------------------------
			...
			elseif this.ifunction_pax_type = 3	then 	// npax_system_final
				guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_pax] dw_pax   |class " + sClass + " npax_system new: "+ String(lpax) + " old: "+String(dw_Pax.GetItemNumber(i,"npax_system_final") ))
				guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_pax] dw_pax   |class " + sClass + " nstationentry new: "+ String(istation_entry) + " old: "+String(dw_Pax.GetItemNumber(i,"nstationentry") ))
				// ------------------------------------------------------------------------------------
				...
				guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_pax] unbekannter Pax-typ: " + String (this.ifunction_pax_type))
			end if
		else
			if this.ifunction_pax_type = 1 	then		// npax
				guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_pax] dw_pax   |class " + sClass + " kein neuer Eintrag  old: " + String(dw_Pax.GetItemNumber(i,"npax") ))
			elseif this.ifunction_pax_type = 2	then	// npax_airline
				guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_pax] dw_pax   |class " + sClass + " kein neuer Eintrag  old: " + String(dw_Pax.GetItemNumber(i,"npax_airline") ))
			elseif this.ifunction_pax_type = 3	then	// npax_system_final
				guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_pax] dw_pax   |class " + sClass + " kein neuer Eintrag  old: " + String(dw_Pax.GetItemNumber(i,"npax_system_final") ))
			else
				guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_pax] unbekannter Pax-typ: " + String (this.ifunction_pax_type))
			end if
		end if
	...
	guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_pax] keine Eintr$$HEX1$$e400$$ENDHEX$$ge in sys_queued_flight_pax vorhanden ")
...
if dsSysQueueFlightACType.RowCount() > 0 then
	...
	if isNull(lAircraft_key_New) then
		...
		if lReturn = -1 or uoGetAircraft.lAircraft_key = -1 then
			guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_actype] dw_single |Aircraft key not found for " + &
		...
		if strRet.lowner <> -1 then
			...
			guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_actype] dw_single |Aircraft  key not found: " + string(lAircraft_key_New) + " " + strRet.cactype)
	...
	guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_actype] dw_single |Aircraft  new: " + sTextNew)
	guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_actype] dw_single |Aircraft  old: " + sText)
	 ...
	 if  (sAirlineOwner_new <>sAirlineOwner_old ) or &
		...
		guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_actype] dw_single |Aircraft changed " )
	...
	elseif	(isnull(sRegistration_old) and not isnull(sRegistration_new) ) or &
			...
			dw_single.setitem(dw_single.getrow(),"cregistration",sRegistration_new)
			guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_actype] dw_single |Registration changed " )
			
			of_log_jobstate(ii_log_info, uf.translate("Registration ge$$HEX1$$e400$$ENDHEX$$ndert"))
		  	
			lreturn = 2
	else
		  guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_actype] dw_single |Aircraft/Registr. unchanged" )
	end if		
end if		
...
// --------------------------------------------------------------------------------------------------------------------
// 29.11.2023 HR: #6658 Dynamic Meal CBASE Integration
// --------------------------------------------------------------------------------------------------------------------
if this.ib_dynamicMeals then
	datastore lds
	
	lds = create datastore
	lds.dataobject = "dw_dynamic_spml_import"
	lds.settransobject(SQLCA)
	lds.retrieve( lResult_key )
	
	if lds.rowcount() > 0 then
		i = lds.rowscopy( 1, lds.rowcount() , PRIMARY!, dsSysQueueFlightSpml, 1, PRIMARY! )	
		if i = 1 then
			guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_spml] Copy " + string(lds.rowcount()) +" rows to dsSysQueueFlightSpml")
		else
			guoLog.uf_error("["+ this.classname( )+".of_get_sys_flight_spml] Error Copy " + string(lds.rowcount()) +" rows from " + lds.dataobject + " to " + dsSysQueueFlightSpml.dataobject + " => " + string(i))
		end if
	else
		guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_spml] keine SPML f$$HEX1$$fc00$$ENDHEX$$r den Flug")
	end if
		...
		if lFound > 0 then 
			...
			if not isnull(lTopOffNew) then
				if lTopoffNew > 1 then
					guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_spml] dw_spml   |topoff ung$$HEX1$$fc00$$ENDHEX$$ltig -> auf 1 ge$$HEX1$$e400$$ENDHEX$$ndert von "+string(ltopoffNew) )
					lTopoffNew = 1
				end if
			...
			if lgeaendert = 1 then
				guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_spml] dw_spml   |" + sTraceTextOld )
				guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_spml] dw_spml   |" + sTraceTextNew + " >>> ge$$HEX1$$e400$$ENDHEX$$ndert")
			end if
			// ------------------------------------------------------------------------------------
			...
			guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_spml] dw_spml   |" + sTraceTextOld + " >>> wird gel$$HEX1$$f600$$ENDHEX$$scht")
	...
	for lfound = 1 to  dsSysQueueFlightSpml.RowCount() 	
		...
		if 	not isNull(sClass) & 
			...
			If lSequence <> -1 and &
				...
				sTraceTextNew = "new: class " + sClass + " cspml "+sSpml + " nquantity " + String(lquantityNew) + &
								" cname " + Sname + " cremark " + sRemark + " cadd_text " + sAdd_Text + " ntopoff " + sTopoff+ " sSeat "+sSeat
				guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_spml] dw_spml   |" + sTraceTextNew + " >>> eingef$$HEX1$$fc00$$ENDHEX$$gt")
			else
				if i = 0 then
					guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_spml] dw_spml   | Fehler insert row" +" class " + sClass + " cspml "+sSpml + " nquantity " + String(lquantityNew) + " >>> Satz nicht verarbeitet !!!")
				else
					guoLog.uf_trace("["+ this.classname( )+".of_get_sys_flight_spml] dw_spml   | Fehler sequence seq_cen_out_spml " +" class " + sClass + " cspml "+sSpml + " nquantity " + String(lquantityNew) + " >>> Satz nicht verarbeitet !!! ")
				end if
			   guoLog.uf_error("["+ this.classname( )+".of_get_sys_flight_spml] fehlerhafte bei insert/sequence in dw_spml" + " class " + sClass + " cspml "+sSpml + " nquantity " + String(lquantityNew)  + " >>>  Satz nicht verarbeitet !!!")
			...
			guoLog.uf_error("["+ this.classname( )+".of_get_sys_flight_spml] fehlerhafte Schl$$HEX1$$fc00$$ENDHEX$$ssels$$HEX1$$e400$$ENDHEX$$tze" + " class " + sClass + " cspml "+sSpml + " nquantity " + String(lquantityNew)  + " >>>  Satz nicht verarbeitet !!!")
			of_log_jobstate(ii_log_fehler, "Class: " + sClass + " / SPML: "+sSpml + " / Quantity: " + String(lquantityNew)+" => SPML not valid")
		end if
	Next
	// ------------------------------------------------------------------------------------
	// nprio neu vergeben
...
if this.of_lock() = -1 then
	...
	guoLog.uf_error("["+ this.classname( )+".of_retrieve_dsjobs] Can't start: " + sRunning + " is working!!!")
	return -1
end if 
...
if dsSysQueueFunctions.RowCount() <= 0 then
		...
		guoLog.uf_error("["+ this.classname( )+".of_retrieve_dsjobs] no functions in sys_queue_fl_functions found, no processing possible" )
		return -1
end if
	...
	if upperbound(lstatus) > 0 then
		...
		for lrow = 1 to lrows
			...
			if lfound = 0 then
				...
				If lSequence = -1 Then
					guoLog.uf_error("["+ this.classname( )+".of_retrieve_dsjobs] Sequence Error: seq_sys_queue_flight_calc")
				else
				...
				//	dsSysQueueFlightCalc.SetItem(lrow2,"dstop_computing",dtNull)
					...
					idw_loading.retrieve(dsCenOut.GetItemNumber(lrow,"nresult_key")) 	// 06.05.2025 HR:
					...
					guoLog.uf_error("["+ this.classname( )+".of_retrieve_dsjobs] Insert into sys_queue_flight_calc job_id "  + string(lsequence) + "nfunction " + &
		...
		if lRows > 0 then
			If dsSysQueueFlightCalc.Update() <> 1 Then
				rollback ;
				guoLog.uf_error("["+ this.classname( )+".of_retrieve_dsjobs] Fehler beim Speichern in sys_queue_flight_calc aus cen_out  ")
			else
				guoLog.uf_allways("["+ this.classname( )+".of_retrieve_dsjobs] Erfolgreiches Einf$$HEX1$$fc00$$ENDHEX$$gen in sys_queue_flight_calc job_id aus cen_out (" + String(lrows) + " S$$HEX1$$e400$$ENDHEX$$tze)" )	

			end if	
		else
			guoLog.uf_allways("["+ this.classname( )+".of_retrieve_dsjobs] keine Daten in cen_out relevant  ")	
		end if
		commit;
	end if	
end if
...
If dsJobs.update() <> 1 Then
	rollback ;
	guoLog.uf_error("["+ this.classname( )+".of_retrieve_dsjobs] Couldn't save stamped records")
	//	destroy	dsExplosion		
	this.of_unlock()
	Return -1
else
	...
	guoLog.uf_allways("["+ this.classname( )+".of_retrieve_dsjobs] Stamped "+string(iCount)+" of "+string(dsjobs.RowCount())+" Rows")
...
if sLock = "1" and (lSecondsAfter < 0 or lSecondsAfter > il_maxlock ) then
	guoLog.uf_allways("["+ this.classname( )+".of_lock] Lock value for more then: " + string(lSecondsAfter) + " not refreshed!")
	guoLog.uf_allways("["+ this.classname( )+".of_lock] Lock value resetted (time1 = " + string(time(sTime)) + " time2 = " + string(tNow)+ ")")
	sLock = "0"
end if
...
if sLock = "0" then
	...
	guoLog.uf_error("["+ this.classname( )+".of_lock] Unknown return value : " + sLock)
	return -1
end if
		...
		For j = 1 to upperbound(sSaveClass)
			if sClass = sSaveClass[j]  then
				...
				elseif this.ifunction_use_as_pax_type = 3	then	// npax_system_final
					...
					guoLog.uf_trace("["+ this.classname( )+".of_restore_pax_type] unbekannter Use_as_Pax-typ: " + String (this.ifunction_use_as_pax_type))
					return -1
				end if
			end if
		next
...
istation_entry= this.istation_entry
	...
	for i = 1 to  dw_Pax.RowCount() 
		...
		elseif this.ifunction_pax_type = 3 	then	// npax_system_final
			...
			guoLog.uf_trace("["+ this.classname( )+".of_pax_type_transfer] unbekannter Pax-typ: " + String (this.ifunction_pax_type))
			return -1
		end if
		...
		elseif this.ifunction_use_as_pax_type = 3	then	// npax_system_final
			...
			guoLog.uf_trace("["+ this.classname( )+".of_pax_type_transfer] unbekannter Use-as_Pax-typ: " + String (this.ifunction_use_as_pax_type))
			return -1
		end if
		// alten Stand sichern
		...
		For j = 1 to upperbound(sSaveVersClass)
			if sClass = sSaveVersClass[j]  then
				...
				elseif this.ifunction_use_as_pax_type = 2	then	// nfinal_version
					...
					guoLog.uf_trace("["+ this.classname( )+".of_restore_version] unbekannter Use_as_actyp: " + String (this.ifunction_use_as_actype))
					return -1
				end if
			end if
		next
...
istation_entry= this.istation_entry
	...
	for i = 1 to  dw_Pax.RowCount() 
		...
		elseif this.ifunction_actype = 2 	then	// nfinal_version
			...
			guoLog.uf_trace("["+ this.classname( )+".of_version_type_transfer] unbekannter Versiontyp: " + String (this.ifunction_actype))
			return -1
		end if
		...
		elseif this.ifunction_use_as_actype = 2	then	// nfinal_version
			...
			guoLog.uf_trace("["+ this.classname( )+".of_version_type_transfer] unbekannter Use-as_actyp: " + String (this.ifunction_use_as_actype))
			return -1
		end if
		// alten Stand sichern
...
guoLog.uf_debug("["+ this.classname( )+".of_recalc_save] Begin")

// ------------------------------------------------------------------
// Vorab-Pr$$HEX1$$fc00$$ENDHEX$$fung: Accepttext, SPML
// ------------------------------------------------------------------
if wf_chc_validation() <> 0 then
	guoLog.uf_error("["+ this.classname( )+".of_recalc_save] wf_chc_validation returns error")
	bInAction = false
	return false
end if
...
// ------------------------------------------------------------------
If bNoSaving = True Then
	guoLog.uf_debug("["+ this.classname( )+".of_recalc_save] bNoSaving = True")

	if not bImportMode then
		guoLog.uf_info("["+ this.classname( )+".of_recalc_save] Die aktuellen Ver$$HEX1$$e400$$ENDHEX$$nderungen lassen ein Speichern nicht zu.")
	end if
	bInAction = false
	bforcemealcalculation = false
	bAnychange = false
...
if sqlca.sqlcode <> 0 then
	guoLog.uf_error("["+ this.classname( )+".of_recalc_save] Database error updating cen_out_recalc_status  SQLCODE: " + String(sqlca.sqlcode))
	rollback ;
	bInAction = false
	return False
end if
...
guoLog.uf_debug("["+ this.classname( )+".of_recalc_save] Speichern der Extrabeladung Start")
If dw_recalc_extra.Update() <> 1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".of_recalc_save] Fehler beim Speichern der Extrabeladung.")
	end if
	bInAction = false
	return False
End if
guoLog.uf_debug("["+ this.classname( )+".of_recalc_save] Speichern der Extrabeladung Ende")

// ------------------------------------------------------------------------
// Speichern von dw_recalc_meal
// ------------------------------------------------------------------------

guoLog.uf_debug("["+ this.classname( )+".of_recalc_save] Speichern der Mahlzeitenbeladung Start")
//wf_setmessage("Speichern der Mahlzeitenbeladung")
If dw_recalc_meal.Update() <> 1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".of_recalc_save] Fehler beim Speichern der Mahlzeitenbeladung.")
	end if
	bInAction = false
	return False
End if
guoLog.uf_debug("["+ this.classname( )+".of_recalc_save] Speichern der Mahlzeitenbeladung Ende")
// ------------------------------------------------------------------------
...
guoLog.uf_debug("["+ this.classname( )+".of_recalc_save] Speichern der Extrabeladung2 Start")
If dw_recalc_extra_recalc.Update() <> 1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".of_recalc_save] Fehler beim Speichern der Extrabeladung2.")
	end if
	bInAction = false
	return False
End if
...
guoLog.uf_debug("["+ this.classname( )+".of_recalc_save] Speichern der Mahlzeitenbeladung2 Start")
//wf_setmessage("Speichern der Mahlzeitenbeladung")
If dw_recalc_meal_recalc.Update() <> 1 Then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".of_recalc_save] Fehler beim Speichern der Mahlzeitenbeladung2.")
	end if
	bInAction = false
	return False
End if
guoLog.uf_debug("["+ this.classname( )+".of_recalc_save] Speichern der Mahlzeitenbeladung Ende2")
//
...
If lSequence = -1 Then
	rollback ;
	bInAction = false
	guoLog.uf_error("["+ this.classname( )+".of_recalc_save] Fehler bei Sequence f$$HEX1$$fc00$$ENDHEX$$r sys_flight_pricer.")
	return False
end if
...
if sqlca.sqlcode <> 0 then
	rollback ;
	if not bImportMode then
		guoLog.uf_error("["+ this.classname( )+".of_recalc_save] Fehler beim Speichern in sys_flight_pricer.")
	end if
	bInAction = false
	return False
end if
...
// Speicher aufr$$HEX1$$e400$$ENDHEX$$umen
// -------------------------------------------------------------------------
guoLog.uf_debug("["+ this.classname( )+".of_recalc_save] GarbageCollect")
GarbageCollect()
...
guoLog.uf_debug("["+ this.classname( )+".of_recalc_save] Fertig")
...
if itype_target = 0 or itype_source = 0 then
	...
	guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_meal] Achtung: Hallo Entwickler, unbekanntes Datawindow in wf_ml_rowscopy_meal!")
...
if itype_target = itype_source then // Formate gleich
	...
	IF	dwsource.RowCount() > 0 THEN
		...
		ls_dataobject_target 	= 	dwtarget.DataObject
		li_result = dwsource.Rowscopy(lstartrow_source, lendrow_source,Primary!,dwtarget,lstartrow_target,PRIMARY!)
		IF li_result = -1 THEN guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_meal] - 0: Error Rowscopy / Source: " + ls_dataobject_source + " / Target: " + ls_dataobject_target ) // 29.10.2013 TBR
	END IF
	...
	if itype_source = 1 then 	// Format 1: dw_change_cen_out_passenger
		...
		IF	dwsource.RowCount() > 0 THEN
			...
			ls_dataobject_target 	= 	dw_format1.DataObject
			li_result = dwsource.Rowscopy(1, dwsource.rowcount(),Primary!,dw_format1,1,PRIMARY!)
			IF li_result = -1 THEN guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_meal] - 1: Error Rowscopy / Source: " + ls_dataobject_source + " / Target: " + ls_dataobject_target ) // 29.10.2013 TBR
		END IF
		...
		if itype_target = 2 then
			...
			IF	dw_format2.RowCount() > 0 THEN
				...
				ls_dataobject_target 	= 	dwtarget.DataObject
				li_result = dw_format2.Rowscopy(lstartrow_source, lendrow_source,Primary!,dwtarget,lstartrow_target,PRIMARY!)
				IF li_result = -1 THEN guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_meal] - 2: Error Rowscopy / Source: " + ls_dataobject_source + " / Target: " + ls_dataobject_target ) // 29.10.2013 TBR
			END IF
		...
		if itype_target = 3 then
			...
			IF	dw_format3.RowCount() > 0 THEN
				...
				ls_dataobject_target 	= 	dwtarget.DataObject
				li_result = dw_format3.Rowscopy(lstartrow_source, lendrow_source,Primary!,dwtarget,lstartrow_target,PRIMARY!)
				IF li_result = -1 THEN guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_meal] - 3: Error Rowscopy / Source: " + ls_dataobject_source + " / Target: " + ls_dataobject_target ) // 29.10.2013 TBR
			END IF
	...
	elseif itype_source = 2 then  		// Format 2: dw_invoice_ml_cen_out_passenger
		if itype_target = 1 then
			...
			IF	dwsource.RowCount() > 0 THEN
				...
				ls_dataobject_target 	= 	dw_format2.DataObject
				li_result = dwsource.Rowscopy(1, dwsource.rowcount(),Primary!,dw_format2,1,PRIMARY!)
				IF li_result = -1 THEN guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_meal] - 4: Error Rowscopy / Source: " + ls_dataobject_source + " / Target: " + ls_dataobject_target ) // 29.10.2013 TBR
			END IF
			...
			IF	dw_format1.RowCount() > 0 THEN
				...
				ls_dataobject_target 	= 	dwtarget.DataObject
				li_result = dw_format1.Rowscopy(lstartrow_source, lendrow_source,Primary!,dwtarget,lstartrow_target,PRIMARY!)
				IF li_result = -1 THEN guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_meal] - 5: Error Rowscopy / Source: " + ls_dataobject_source + " / Target: " + ls_dataobject_target ) // 29.10.2013 TBR
			END IF
		...
		if itype_target = 3 then
			...
			IF	dwsource.RowCount() > 0 THEN
				...
				ls_dataobject_target 	= 	dw_format2.DataObject
				li_result = dwsource.Rowscopy(1, dwsource.rowcount(),Primary!,dw_format2,1,PRIMARY!)
				IF li_result = -1 THEN guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_meal] - 6: Error Rowscopy / Source: " + ls_dataobject_source + " / Target: " + ls_dataobject_target ) // 29.10.2013 TBR
			END IF
			...
			IF	dw_format3.RowCount() > 0 THEN
				...
				ls_dataobject_target 	= 	dwtarget.DataObject
				li_result = dw_format3.Rowscopy(lstartrow_source, lendrow_source,Primary!,dwtarget,lstartrow_target,PRIMARY!)
				IF li_result = -1 THEN guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_meal] - 7: Error Rowscopy / Source: " + ls_dataobject_source + " / Target: " + ls_dataobject_target ) // 29.10.2013 TBR
			END IF
	...
	elseif itype_source = 3 then  		// Format 3: dw_invoice_ml_cen_out_passenger
		if itype_target = 1 then
			...
			IF	dwsource.RowCount() > 0 THEN
				...
				ls_dataobject_target 	= 	dw_format3.DataObject
				li_result = dwsource.Rowscopy(1, dwsource.rowcount(),Primary!,dw_format3,1,PRIMARY!)
				IF li_result = -1 THEN guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_meal] - 8: Error Rowscopy / Source: " + ls_dataobject_source + " / Target: " + ls_dataobject_target ) // 29.10.2013 TBR
			END IF
			...
			IF	dw_format1.RowCount() > 0 THEN
				...
				ls_dataobject_target 	= 	dwtarget.DataObject
				li_result = dw_format1.Rowscopy(lstartrow_source, lendrow_source,Primary!,dwtarget,lstartrow_target,PRIMARY!)
				IF li_result = -1 THEN guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_meal] - 9: Error Rowscopy / Source: " + ls_dataobject_source + " / Target: " + ls_dataobject_target ) // 29.10.2013 TBR
			END IF
		...
		if itype_target = 2 then
			...
			IF	dwsource.RowCount() > 0 THEN
				...
				ls_dataobject_target 	= 	dw_format3.DataObject
				li_result = dwsource.Rowscopy(1, dwsource.rowcount(),Primary!,dw_format3,1,PRIMARY!)
				IF li_result = -1 THEN guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_meal] - 10: Error Rowscopy / Source: " + ls_dataobject_source + " / Target: " + ls_dataobject_target ) // 29.10.2013 TBR
			END IF
			...
			IF	dw_format2.RowCount() > 0 THEN
				...
				ls_dataobject_target 	= 	dwtarget.DataObject
				li_result = dw_format2.Rowscopy(lstartrow_source, lendrow_source,Primary!,dwtarget,lstartrow_target,PRIMARY!)
				IF li_result = -1 THEN guoLog.uf_error("["+ this.classname( )+".wf_ml_rowscopy_meal] - 11: Error Rowscopy / Source: " + ls_dataobject_source + " / Target: " + ls_dataobject_target ) // 29.10.2013 TBR
			END IF
...
if uoClientLabel.dsCenOut.Retrieve(lResult_key) = 0 then 
	...
	guoLog.uf_error("["+ this.classname( )+".of_process_mealdistribution] Flug nicht gefunden!")	
...
if uoClientLabel.dsCenOut.GetItemString(1, "chandling_type_text") <> "DLV" then
	...
	if uoClientLabel.of_get_standard_loading() <> 0 then
		...
		guoLog.uf_info("["+ this.classname( )+".of_process_mealdistribution] Keine Standardbeladung gefunden!")
	...
	if uoClientLabel.of_get_local_standard_loading() <= 0 then
		...
		guoLog.uf_info("["+ this.classname( )+".of_process_mealdistribution] Keine individuelle Standardbeladung gefunden!")	
...
guoLog.uf_allways("["+ this.classname( )+".of_process_mealdistribution] uoClientLabel.of_set_labeltype()")
uoClientLabel.of_set_labeltype()

guoLog.uf_allways("["+ this.classname( )+".of_process_mealdistribution] uoClientLabel.of_add_workstations()")
uoClientLabel.of_add_workstations()

guoLog.uf_allways("["+ this.classname( )+".of_process_mealdistribution] uoClientLabel.of_distribution()")

if uoClientLabel.of_distribution() = -1000 then
	this.sreturn_error_message = "Fehler Verteilung "+uoClientLabel.sErrorMessage
	guoLog.uf_error("["+ this.classname( )+".of_process_mealdistribution] uoClientLabel.of_distribution() = -1000 "+uoClientLabel.sErrorMessage)
	...
	guoLog.uf_allways("["+ this.classname( )+".of_process_mealdistribution] uoClientLabel.of_distribution() OK")
end if
guoLog.uf_allways("["+ this.classname( )+".of_process_mealdistribution] uoClientLabel.of_create_cart_diagram()")	
...
if gi_enable_secondary_distribution = 1 then
	...
	guoLog.uf_allways("["+ this.classname( )+".of_process_mealdistribution] uoClientLabel.of_create_cart_diagram() start Secondary Distribtution")	
	
	li_Return = uoClientLabel.of_create_cart_diagram(lResult_key,  "", luo_Product, & 
													true, false, false, true, true, false, & 
													false, "", "")
	
	guoLog.uf_allways("["+ this.classname( )+".of_process_mealdistribution] uoClientLabel.of_create_cart_diagram() end Secondary Distribtution Return: "+string(li_Return))	
End If
...
If integer(f_mandant_profilestring(sqlca, s_app.smandant, "PPM","New2ndDistribution","0")) = 1 Then
	...
	guoLog.uf_allways("["+ this.classname( )+".of_process_mealdistribution] New Secondary Distribtution starts")
	...
	if sqlca.sqlcode < 0 then
		...
		guoLog.uf_error("["+ this.classname( )+".of_process_mealdistribution] Fehler beim Deklarieren der Funktion cbase_distribution_2nd.pf_sd: "+string(sqlca.sqlcode)+" "+f_check_null(sqlca.sqlerrtext))
		return 1
	end if
	...
	if sqlca.sqlcode < 0 then
		...
		guoLog.uf_error("["+ this.classname( )+".of_process_mealdistribution] Fehler beim Ausf$$HEX1$$fc00$$ENDHEX$$hren der Funktion cbase_distribution_2nd.pf_sd: "+string(sqlca.sqlcode)+" "+f_check_null(sqlca.sqlerrtext))
		return 1
	end if
	
	close newDistribution;
	
	guoLog.uf_allways("["+ this.classname( )+".of_process_mealdistribution] New Secondary Distribtution finished")
	
end if

guoLog.uf_allways("["+ this.classname( )+".of_process_mealdistribution] End")	
...
//			IF 	ldt_now >= ldt_departure_diff	AND ldt_now <= ldt_departure THEN
			...
			IF 	ldt_now >= ldt_departure_diff	 or ls_jfe_status_h ="0" THEN
				...
				IF NOT iuo_request_out.uf_request_out_auto(lResultKey) THEN
					guoLog.uf_error("["+ this.classname( )+".wf_transfer_jfe] Fehler beim Transfer der JFE-Daten")
				END IF
...
idw_loading.Reset()					// 06.05.2025 HR:
...
dsPaxOld	.Reset()
dsSingleOld.Reset()
idw_Oldloading.reset()					// 06.05.2025 HR:
...
idw_loading.retrieve(arg_lresultkey)		// 06.05.2025 HR:
...
guoLog.uf_debug("["+ this.classname( )+".of_process_auto_web_calc_cen_out] ljob_nr: " + String (lJob_Nr) + " " + sFlight + " Funktion " + String(this.iinternal_function) )
...
if of_lock_record(arg_lresult_key) = false then
	...
	guoLog.uf_info("["+ this.classname( )+".[of_process_auto_web_calc_cen_out] "+this.sreturn_error_message)
	return -10
end if
...
if dsSingleOld.RowCount() > 0 then
	...
	guoLog.uf_info("["+ this.classname( )+".of_process_auto_web_calc_cen_out] no history record found for {" + sFlight + "}, default used")
	l_nstatus_hist = 1
...
if of_get_sys_flight_pax(ljob_nr) = -1 then
	 guoLog.uf_error("["+ this.classname( )+".of_process_auto_web_calc_cen_out] Error for flight " + sFlight + " in using pax data of of_get_sys_flight_pax!")
end if
...
if lreturn = -1 then
	 guoLog.uf_error("["+ this.classname( )+".of_process_auto_web_calc_cen_out] Error for flight " + sFlight + " in using aircraftdata of of_get_sys_flight_actype!")
else
	if lreturn = 1 then
		// --------------------------------------------------------
		...
		if bAircraftchange = False then
			guoLog.uf_info("["+ this.classname( )+".of_process_auto_web_calc_cen_out] Set flight  " + sFlight + " to aircraft change due to changed aircraftdata")
			bAircraftchange = True
		end if
...
If sOldRegistration <> sCurrentRegistration Then
	...
	guoLog.uf_info("["+ this.classname( )+".of_process_auto_web_calc_cen_out] Registration is changed.")
...
if of_get_sys_flight_spml(ljob_nr) = -1 then
	 guoLog.uf_error("["+ this.classname( )+".of_process_auto_web_calc_cen_out] "+Stype + ": Error for flight " + sFlight + " in using SPML data of of_get_sys_flight_spml!")
end if
...
if 1=1 then 
	guoLog.uf_trace("["+ this.classname( )+".of_process_auto_web_calc_cen_out] Web processing for "+sFlight)
	...
	if not bForceMealCalculation then
		...
		if sqlca.sqlcode = 0 then
			if  lcount > 0 then
				 guoLog.uf_info("["+ this.classname( )+".of_process_auto_web_calc_cen_out] Forced meal recalculation for flight " + sFlight + " due to invalid meal definiton references!")
				bForceMealCalculation = True
			end if
		else	
			guoLog.uf_error("["+ this.classname( )+".of_process_auto_web_calc_cen_out]  SQL-Error for selection of invalid meal definiton references for flight " + sFlight + " !"+String(sqlca.sqlcode))
	...
	if bDoAllwaysMealCalc then 
		 guoLog.uf_info("["+ this.classname( )+".of_process_auto_web_calc_cen_out]  Forced meal recalculation for flight " + sFlight + ", although there were no changes !")
...
if ll_supplier <> 1300 then
	guoLog.uf_debug("["+ this.classname( )+".of_check_do_aenderungsmitteilung] Keine $$HEX1$$c400$$ENDHEX$$nderungsmitteilung f$$HEX1$$fc00$$ENDHEX$$r ML-Fl$$HEX1$$fc00$$ENDHEX$$ge")
...
if DateTime(Today(),Now()) > dtCheckdate Then
	guoLog.uf_debug("["+ this.classname( )+".of_check_do_aenderungsmitteilung] Keine $$HEX1$$c400$$ENDHEX$$nderungsmitteilung da nach Abflug")
...
if dsSysQueueFunctions.RowCount() <= 0 then
		...
		guoLog.uf_allways("["+ this.classname( )+".of_retrieve_dsjobs_new] no functions in sys_queue_fl_functions found, no processing possible" )
		return -1
end if
...
guoLog.uf_allways("["+ this.classname( )+".of_retrieve_dsjobs_new] Delete "+string(sqlca.sqlnrows)+" old Jobs older "+string(ii_del_offset_ok)+" days: "+string(sqlca.sqlcode) )
...
guoLog.uf_allways("["+ this.classname( )+".of_retrieve_dsjobs_new] Delete "+string(sqlca.sqlnrows)+" old Jobs with errors older "+string(ii_del_offset_fehler)+" days: "+string(sqlca.sqlcode) )

  DELETE FROM sys_queue_flight_calc  
   WHERE ( sys_queue_flight_calc.dstop_computing is null ) AND  
	    ( sys_queue_flight_calc.dcreated < sysdate - :ii_del_offset_fehler ) AND
         ( sys_queue_flight_calc.nprocess_status > 4 ) AND  
         ( rownum <= 1000 );

guoLog.uf_allways("["+ this.classname( )+".of_retrieve_dsjobs_new] Delete "+string(sqlca.sqlnrows)+" old Jobs with dstop_computing is null and nprocess_status > 4: "+string(sqlca.sqlcode) )
...
guoLog.uf_allways("["+ this.classname( )+".of_retrieve_dsjobs_new] Delete "+string(sqlca.sqlnrows)+" Jobs without reference to CEN_OUT: "+string(sqlca.sqlcode) )
...
if lds_instances.rowcount()=0 then
	...
	guoLog.uf_allways("["+ this.classname( )+".of_retrieve_dsjobs_new] No Instances with capacity found" )
	...
	if upperbound(lstatus) > 0 then
		...
		for lrow = 1 to lrows
			...
			if lfound = 0 then
				...
				lSequence = f_Sequence("seq_sys_queue_flight_calc", sqlca)
				If lSequence = -1 Then
					guoLog.uf_allways("["+ this.classname( )+".of_retrieve_dsjobs_new] Sequence Error: seq_sys_queue_flight_calc")
				else
					lrow2=dsSysQueueFlightCalc.insertRow(0)
					dsSysQueueFlightCalc.SetItem(lrow2,"njob_nr",lsequence)
					dsSysQueueFlightCalc.SetItem(lrow2,"nresult_key",dsCenOut.GetItemNumber(lrow,"nresult_key"))
					...
					guoLog.uf_allways("["+ this.classname( )+".of_retrieve_dsjobs_new] Insert into sys_queue_flight_calc job_id "  + string(lsequence) + "nfunction " + &
						String(ifunction) + " Flug " + sFlight + " (cen_out_nqueued_release_interface= " + String(lqueued_release_interface) +")")
		...
		if lRows > 0 then
			If dsSysQueueFlightCalc.Update() <> 1 Then
				rollback ;
				guoLog.uf_error("["+ this.classname( )+".of_retrieve_dsjobs_new] Fehler beim Speichern in sys_queue_flight_calc aus cen_out  ")
			else
				guoLog.uf_allways("["+ this.classname( )+".of_retrieve_dsjobs_new] Erfolgreiches Einf$$HEX1$$fc00$$ENDHEX$$gen in sys_queue_flight_calc job_id aus cen_out (" + String(lrows) + " S$$HEX1$$e400$$ENDHEX$$tze)" )	

			end if	
		else
			guoLog.uf_allways("["+ this.classname( )+".of_retrieve_dsjobs_new] keine Daten in cen_out relevant  ")	
		end if
		commit;
	end if	
end if
...
if lds_jobs.rowcount()=0 then
	...
	guoLog.uf_allways("["+ this.classname( )+".of_retrieve_dsjobs_new] No open jobs to distribute found" )
...
guoLog.uf_allways("["+ this.classname( )+".of_retrieve_dsjobs_new] Found "+string( lds_jobs.rowcount())+" open Jobs.")
...
For i = 1 to lds_jobs.RowCount()
	...
	if lds_instances.setfilter("nfunction"+string(lds_jobs.GetItemNumber(i, "nfunction"))+"=1 and  nanz < "+string(gi_max_jobs_to_distribute)) <> 1 then
		guoLog.uf_error("["+ this.classname( )+".of_retrieve_dsjobs_new] Fehler Setfilter DW_JOB_INSTANCES (nfunction"+string(lds_jobs.GetItemNumber(i, "nfunction"))+"=1 and  nanz < "+string(gi_max_jobs_to_distribute)+")")
	end if
	...
	if lds_instances.rowcount()=0 then
		...
		if sNO=""  or pos(sNO, " "+string(lds_jobs.GetItemNumber(i, "nfunction"))+",")=0 then
			guoLog.uf_error("["+ this.classname( )+".of_retrieve_dsjobs_new] No more Instance with capacity for Function "+string(lds_jobs.GetItemNumber(i, "nfunction")) +" found")
		...
		*/
		...
		guoLog.uf_info("["+ this.classname( )+".of_retrieve_dsjobs_new] No more Instance with capacity for Function "+string(lds_jobs.GetItemNumber(i, "nfunction")) +" found")
			
		lds_jobs.setfilter(sNO)
		lds_jobs.filter()
		
		if lds_jobs.rowcount()=0 then
			guoLog.uf_info("["+ this.classname( )+".of_retrieve_dsjobs_new] No more Jobs for Instances with capacity.")
			exit
		end if
		i = 0
		continue
...
If lds_jobs.update() <> 1 Then
	rollback ;
	guoLog.uf_error("["+ this.classname( )+".of_retrieve_dsjobs_new] Couldn't save stamped records")

	iRet =  -1
else
	Commit;
	
	guoLog.uf_info("["+ this.classname( )+".of_retrieve_dsjobs_new] Stamped "+string(iCount)+" of "+string(lds_jobs.RowCount())+" Rows")
...
if isnull(arg_mes_text) or arg_mes_text="" then
   arg_mes_text = " "
   guoLog.uf_allways("["+this.classname()+".of_log_jobstate] arg_mes_text is set to BLANK")
end if
...
if SQLCA_LOG.SQLCODE <> 0 then
	guoLog.uf_error("["+this.classname()+".of_log_jobstate] Error Insert into SYS_QUEUE_FLIGHT_LOG("+string(arg_mes_type)+" "+arg_mes_text+")")
	guoLog.uf_error("["+this.classname()+".of_log_jobstate] Error: "+string(SQLCA_LOG.SQLCODE)+" "+SQLCA_LOG.SQLERRTEXT)
...
guoLog.uf_debug("["+ this.classname( )+".wf_chc_check_against_old] Begin")

if this.dssingleold.rowcount()=0 then
	guoLog.uf_debug("["+ this.classname( )+".wf_chc_check_against_old] No old Flightdata")
	return true
end if
...
if dw_pax.rowcount() <> dspaxold.rowcount() then
	guoLog.uf_debug("["+ this.classname( )+".wf_chc_check_against_old] Different Rowcount dw_pax() - dspaxold")
else
	For i = 1 to dw_pax.rowcount()
...
guoLog.uf_debug("["+ this.classname( )+".wf_chc_check_against_old] Ende")
...
if not ab_OKSave then
	...
	// --------------------------------------------------------------------------------------------------------------------
	if sqlca.sqlcode <> 0 then
		guoLog.uf_error("["+ this.classname( )+".of_check_save] - Error Update CEN_OUT. nqueued_release_interface: " + string( sqlca.sqlcode) +  sqlca.sqlerrtext)		
		
		this.ireturn_error_status  = -1
		this.sreturn_error_message = ": Database error: " + sqlca.sqlerrtext
		this.sreturn_error_message_short = ": Database error: " + sqlca.sqlerrtext
	else
		guoLog.uf_debug("["+ this.classname( )+".of_check_save] - Update CEN_OUT.nstatus - " + string(dw_single.getitemNumber(dw_single.getrow(),"nstatus")) + " /  " + string(iChangeStatus))		
	end if
	 ...
	 if dw_single.getitemNumber(dw_single.getrow(),"nstatus") = 6 or &
		...
		if not wf_chc_flight_closed(ref ls_message) then
			...
			if sqlca.sqlcode <> 0 then
				...
				this.sreturn_error_message = ": Database error: " + sqlca.sqlerrtext
				this.sreturn_error_message_short = ": Database error: " + sqlca.sqlerrtext
				guoLog.uf_error("["+ this.classname( )+".of_check_save] - Error Update CEN_OUT. nstatus = 3, nstatus_release=NULL: " + string( sqlca.sqlcode) +  sqlca.sqlerrtext)		
...
bMultiLegError 		= false
		...
		dsClasses.Retrieve(uoGetAircraft.lAirline_key)
			...
			if lReturn = -1 or uoGetAircraft.lAircraft_key = -1 then
				...
				guoLog.uf_allways("["+ this.classname( )+".wf_chc_multileg_change] dw_single |Aircraft  key not found for " + &
		...
		if not wf_chc_save() Then
			...
			guoLog.uf_error("["+ this.classname( )+".wf_chc_multileg_change] "+arg_message)
			bMultiLegError = true
		end if
		...
		dw_meal.ResetUpdate() 
		dw_spml.ResetUpdate() 
		idw_loading.ResetUpdate() 	// 06.05.2025 HR:
...
guoLog.uf_allways("["+ this.classname( )+".of_process_meals] al_job_nr: " + String (al_job_nr))
...
guoLog.uf_allways("["+ this.classname( )+".of_process_meals] nstatus in queue: " + String (li_status))
// Details retrieven
ll_rowcount_details = lds_details.Retrieve(al_job_nr, ld_departure)
guoLog.uf_allways("["+ this.classname( )+".of_process_meals] Number Details retrieved: " + String (ll_rowcount_details))
...
FOR ll_row = 1 TO ll_rowcount_details
	...
	lll_detail_key = lds_details.GetItemNumber(ll_row, "ndetail_key")
	ll_row_work = dw_meal.Find("ndetail_key = " + String(lll_detail_key), 1, dw_meal.Rowcount())
	IF ll_row_work > 0 THEN
		// Gefunden, also Meal
		ldw_work = dw_meal
	ELSE
		// Nicht gefunden, jetzt in dw_extra suchen
		ll_row_work = dw_extra.Find("ndetail_key = " + String(lll_detail_key), 1, dw_extra.Rowcount())
		IF ll_row_work > 0 THEN
			// Gefunden, also Extra
			ldw_work = dw_extra
		END IF
	...
	ELSE
		...
		ldw_work.SetItem(ll_row_work, "ndetail_key", lll_detail_key)
...
//	dw_meal.SetRedraw(false)
	...
	if dsmeals2.RowsCopy(1,dsmeals2.RowCount(),Primary!,dw_meal,1,Primary!)  = -1 and dsmeals2.RowCount()>0 then
		guoLog.uf_error("["+ this.classname( )+".wf_chc_change_meals] error rowscopy  dsmeals2 to dw_meal  / Source: " + dsmeals2.dataobject + " / Target: " + dw_meal.dataobject)
	end if
	dw_meal.Sort()
	...
	for i = 1 to dw_meal.RowCount()
		...
		lllcurrentDetailKey 	= dw_meal.GetItemNumber(i,"ndetail_key")
		sPL					= dw_meal.GetItemString(i,"cpackinglist")
		
		lfind = dw_meal.find("ndetail_key = " + string(lllcurrentDetailKey),1,i)
		if lfind <> i then
			if lfind > 0 then
				lllSequence = f_Sequence_ll("seq_cen_out_meals", sqlca)
				dw_meal.SetItem(i,"ndetail_key",lllSequence)
			end if
		end if
		lfind = dw_meal.find("ndetail_key = " + string(lllcurrentDetailKey),i + 1,dw_meal.RowCount())
		if lfind <> i then
			if lfind > 0 then
				lllSequence = f_Sequence_ll("seq_cen_out_meals", sqlca)
				dw_meal.SetItem(i,"ndetail_key",lllSequence)
...
IF NOT luo_request_out.uf_request_out_auto_wab(al_resultkey, ls_msg) THEN
	guoLog.uf_error("["+ this.classname( )+".wf_wab_send_galleyweights] Error sending weights for flight " + String(al_resultkey) + ": " + ls_msg)
ELSE
	guoLog.uf_allways("["+ this.classname( )+".wf_wab_send_galleyweights] Send weights for flight " + String(al_resultkey) + ": " + ls_msg)
END IF
...
guoLog.uf_trace("["+ this.classname( )+".of_recalc_pax] Start")
...
if is_set = "none" then 
	guoLog.uf_trace("["+ this.classname( )+".of_recalc_pax] " + ls_flgnr + "Keine lokalen Anpassungen verwenden.")
	return true
end if
...
guoLog.uf_trace("["+ this.classname( )+".of_recalc_pax] " + ls_flgnr + "Anzahl m$$HEX1$$f600$$ENDHEX$$glicher Regeln f$$HEX1$$fc00$$ENDHEX$$r Set "+is_set+": "+string(lds_adjust.rowcount()))
...
for i=1 to upperbound(arg_sClassName)
	...
	if arg_iForecast[i] <>  ll_pax_new then
		...
		guoLog.uf_trace("["+ this.classname( )+".of_recalc_pax] " + ls_flgnr + "Paxanpassung Klasse "+ls_cclass+" wegen "+ls_kind +"-Regel : Pax Alt: "+string(ll_pax_old)+" Pax Neu: "+string(ll_pax_new)+" Offset "+ls_offset)
	...
	if ll_found=1 then
		...
		for i=1 to upperbound(arg_sClassName)
			...
			if ll_found=0 then 
				if lds_acsearch.rowcount()>0 then
					...
					guoLog.uf_trace("["+ this.classname( )+".of_recalc_pax] " + ls_flgnr + "Versionsanpassung: kein Zeile mehr in lds_acsearch. Paxzeile "+ string(i)+"; ls_version: "+ls_version +";ls_pax: "+ls_pax)
					ll_found = -1
					exit
				end if
			end if
		...
		if ll_found<> -1 then
			...
			if ls_version <>  dw_single.GetItemString(arg_lrow,"cconfiguration") then
				guoLog.uf_trace("["+ this.classname( )+".of_recalc_pax] " + ls_flgnr + "Versionsanpassung: Alt: "+ dw_single.GetItemString(arg_lrow,"cconfiguration")+"; Pax: "+ls_pax +"; Neu: "+ls_version)
...
for i =  upperbound(arg_sClassName) to 1 step -1
	...
	if li_overload[i]=0 then
		if ll_pax_old > ll_version then
			...
			if ls_upclass[i]="none" then
				guoLog.uf_trace("["+ this.classname( )+".of_recalc_pax] " + ls_flgnr + "Paxanpassung Klasse "+ls_cclass+", da $$HEX1$$dc00$$ENDHEX$$berbeladung abgeschnitten werden soll. Pax Alt: "+string(ll_pax_old)+" Pax Neu: "+string(ll_version)+" keine Upgradeklasse definiert")
				...
				guoLog.uf_trace("["+ this.classname( )+".of_recalc_pax] " + ls_flgnr + "Paxanpassung Klasse "+ls_cclass+", da $$HEX1$$dc00$$ENDHEX$$berbeladung abgeschnitten werden soll. Pax Alt: "+string(ll_pax_old)+" Pax Neu: "+string(ll_version)+" Upgrade "+string(ll_pax_new)+" Paxe ("+string( li_upgrade[i])+"%) nach "+ ls_upclass[i] )
	...
	elseif  ll_pax_old > ll_version then
		guoLog.uf_trace("["+ this.classname( )+".of_recalc_pax] " + ls_flgnr + "keine Paxanpassung Klasse "+ls_cclass+", da $$HEX1$$dc00$$ENDHEX$$berbeladung gew$$HEX1$$fc00$$ENDHEX$$nscht")		
	end if
next
...
for i = 1 to  upperbound(arg_sClassName)
	...
	if ll_pax_old >= ll_version - li_red_add_delivery[i] and ll_pax_old < ll_version then
		arg_iForecast[i]  =  ll_version
		guoLog.uf_trace("["+ this.classname( )+".of_recalc_pax] " + ls_flgnr + "Paxanpassung Nachfahrtenreduktion Klasse "+ls_cclass+" Pax old: "+string(ll_pax_old)+" Version: "+string(ll_version)+" FH ab "+string(ll_version - li_red_add_delivery[i])+" Paxe")
...
guoLog.uf_allways("["+ this.classname( )+".wf_regenerate_paclos] "+string(sqlca.sqlnrows )+" Packages deleted")

if lds.rowcount()> 0 then
	i =  lds.rowscopy(1,lds.rowcount(),primary!, lds_CenOutPackets, 1, primary!)	
	if i<>1 then
		guoLog.uf_error("["+ this.classname( )+".wf_regenerate_paclos] error copy "+string(lds.rowcount())+" cfs-packets to flight")
	else
		If lds_CenOutPackets.Update() <> 1 Then
			guoLog.uf_error("["+ this.classname( )+".wf_regenerate_paclos] Save Packages with error " + &
			...
			guoLog.uf_allways("["+ this.classname( )+".wf_regenerate_paclos] "+string(lds.rowcount())+" Packages saved succesful")
		end if
	end if
end if
...
guoLog.uf_allways("["+ this.classname( )+".wf_regenerate_paclos] "+string(sqlca.sqlnrows )+" LOS deleted")
...
if lds.rowcount()> 0 then
	...
	if i<>1 then
		...
		If lds_CenOutLOS.update() <> 1 Then
			guoLog.uf_error("["+ this.classname( )+".wf_regenerate_paclos] Save LOS with error " + &
			...
			guoLog.uf_allways("["+ this.classname( )+".wf_regenerate_paclos] "+string(lds.rowcount())+" LOS saved succesful")
		end if
	end if
end if
...
guoLog.uf_allways("["+ this.classname( )+".of_process_freeze] al_job_nr: " + String (al_job_nr))
...
IF ll_count > 0 THEN
	...
	guoLog.uf_info("["+ this.classname( )+".of_process_freeze] Running Online-Explosion for nesult_key " + String(al_result_key) + ". Process Status set to 4 (retry).")
...
if gb_wait_for_mzv then
	...
	IF ll_count > 0 THEN
		...
		li_row = dsJobs.Find("njob_nr = " + String(al_job_nr), 1, dsJobs.RowCount())
		dsJobs.SetItem(li_row, "nprocess_status", 4)
		guoLog.uf_info("["+ this.classname( )+".of_process_freeze] Open MZV Jobs for nesult_key " + String(al_result_key) + ". Process Status set to 4 (retry).")
...
IF  SQLCA.sqlcode <> 0 THEN
	guoLog.uf_error("["+ this.classname( )+".of_process_freeze] Error DECLAREing Procedure cbase_ppm.pf_freeze_flight: " + SQLCA.SQLErrText)
...
IF SQLCA.sqlcode <> 0 AND SQLCA.sqlcode <> 100 THEN
	...
	guoLog.uf_error("["+ this.classname( )+".of_process_freeze] " + ls_msg)
	this.sreturn_error_message = ": Error during Freeze Flight"
	this.sreturn_error_message_short = "Error during Freeze Flight"
	this.is_web_message = uf.translate("Error during Freeze Flight")
	of_log_jobstate(ii_log_fehler, this.is_web_message)
...
IF ll_count > 0 THEN
	...
	END IF
	guoLog.uf_info("["+ this.classname( )+".of_process_freeze]" + ls_msg)
...
if i=0 then
	...
	if ll_routingdetail_key =-1 or ll_nroutingdetail_key_cp =-1 then
		guoLog.uf_allways("["+ this.classname( )+".of_check_schedule_validity] Fehler beim Ermitteln der Flugschl$$HEX1$$fc00$$ENDHEX$$ssel, da die G$$HEX1$$fc00$$ENDHEX$$ltigkeit der Beladedefinition ge$$HEX1$$e400$$ENDHEX$$ndert wurde. Alte Werte werden weiter benutzt.")
	else
		dw_single.SetItem(dw_single.Getrow(), "nroutingdetail_key",ll_routingdetail_key) 
		dw_single.SetItem(dw_single.Getrow(), "nroutingdetail_key_cp",ll_nroutingdetail_key_cp)
		dw_single.SetItem(dw_single.Getrow(), "nroutingplan_index",llroutingplan_index_key)
...
if ldsBobSched.rowcount()=0 then
	...
	if ll_flight_key = -1 then
		guoLog.uf_error("["+ this.classname( )+".of_retrieve_bob_figures] Fehler bei der Ermittlung der SEQ_CEN_BOB_SCHEDULE" )
		destroy ldsBobSched
		return 0
	end if		
	...
	if sqlca.sqlcode <> 0 then
		guoLog.uf_error("["+ this.classname( )+".of_retrieve_bob_figures] Fehler bei der Anlage cen_bob_schedule (" + string(sqlca.sqlcode)+") "+sqlca.sqlerrtext )
		destroy ldsBobSched
		return 0
	end if	
else
...
for li_row = 1 to dsSysQueueFlightBob.rowcount()
	...
	if li_found > 0 then
		...
		if ll_seq = -1 then
			guoLog.uf_error("["+ this.classname( )+".of_retrieve_bob_figures] Fehler bei der Ermittlung der SEQ_CEN_BOB_PAX_COUNTS" )
			destroy ldsBobSched
...
guoLog.uf_allways("["+ this.classname( )+".of_process_preprod_freeze] al_job_nr: " + String (al_job_nr))
...
IF ll_count > 0 THEN
	...
	li_row = dsJobs.Find("njob_nr = " + String(al_job_nr), 1, dsJobs.RowCount())
	dsJobs.SetItem(li_row, "nprocess_status", 4)
	guoLog.uf_info("["+ this.classname( )+".of_process_preprod_freeze] Running Online-Explosion for nesult_key " + String(al_result_key) + ". Process Status set to 4 (retry).")
	this.sreturn_error_message = ": Running Online-Explosion for Flight ${" + this.sFlightText + "} !"
	this.sreturn_error_message_short = "Running Online-Explosion for Flight!"
	this.is_web_message = uf.translate("Running Online-Explosion for Flight")
...
IF  SQLCA.sqlcode <> 0 THEN
	guoLog.uf_error("["+ this.classname( )+".of_process_preprod_freeze] Error DECLAREing Procedure cbase_ppm_preprod.pf_freeze_preprod: " + SQLCA.SQLErrText)
...
IF SQLCA.sqlcode <> 0 AND SQLCA.sqlcode <> 100 THEN
	...
	guoLog.uf_error("["+ this.classname( )+".of_process_preprod_freeze] " + ls_msg)
	this.sreturn_error_message = ": Error during Freeze Preprod"
	this.sreturn_error_message_short = "Error during Freeze Preprod"
	this.is_web_message = uf.translate("Error during Freeze Preprod")
	of_log_jobstate(ii_log_fehler, this.is_web_message)
...
ELSEIF ll_ret > 0 THEN
	...
	guoLog.uf_info("["+ this.classname( )+".of_process_preprod_freeze]" + ls_msg)
	this.sreturn_error_message = ": " +  + ls_msg2 + " ${" + this.sFlightText + "} !"
	this.sreturn_error_message_short = ls_msg2
	this.is_web_message = uf.translate(ls_msg)
	of_log_jobstate(ii_log_info, this.is_web_message)
...
guoLog.uf_allways("["+ this.classname( )+".of_process_skyscope_copy] Start " )

DECLARE skyscope_copy PROCEDURE FOR CBASE_SKYSCOPE.pp_flight_transfer(:al_result_key, :ilJobNr) USING SQLCA;
IF  SQLCA.sqlcode <> 0 AND SQLCA.sqlcode <> 100 THEN
	guoLog.uf_error("["+ this.classname( )+".of_process_skyscope_copy] Error DECLAREing Procedure CBASE_SKYSCOPE.pp_flight_transfer: " + SQLCA.SQLErrText)
...
IF SQLCA.sqlcode <> 0 AND SQLCA.sqlcode <> 100 THEN
	...
	guoLog.uf_error("["+ this.classname( )+".of_process_skyscope_copy] " + ls_msg)
	this.sreturn_error_message = ": Error during Skyscope Copy"
	this.sreturn_error_message_short = "Error during Skyscope Copy"
	this.is_web_message = uf.translate("Error during Skyscope Copy")
	of_log_jobstate(ii_log_fehler, this.is_web_message)
...
guoLog.uf_allways("["+ this.classname( )+".of_process_skyscope_copy] Finish with return code: " + string(li_ret) +" => " + this.sreturn_error_message_short)

if li_ret <> 3 then of_log_jobstate(li_ret, 'Job finished' )
...
guoLog.uf_allways("["+ this.classname( )+".of_process_skyscope_refresh] Start " )

DECLARE skyscope_refresh PROCEDURE FOR CBASE_SKYSCOPE.pp_flight_refresh(:al_result_key, :ilJobNr) USING SQLCA;
IF  SQLCA.sqlcode <> 0 AND SQLCA.sqlcode <> 100  THEN
	guoLog.uf_error("["+ this.classname( )+".of_process_skyscope_refresh] Error DECLAREing Procedure CBASE_SKYSCOPE.pp_flight_refresh: " + SQLCA.SQLErrText)
...
IF SQLCA.sqlcode <> 0 AND SQLCA.sqlcode <> 100 THEN
	...
	guoLog.uf_error("["+ this.classname( )+".of_process_skyscope_refresh] " + ls_msg)
	this.sreturn_error_message = ": Error during Skyscope Refresh"
	this.sreturn_error_message_short = "Error during Skyscope Refresh"
	this.is_web_message = uf.translate("Error during Skyscope Refresh")
	of_log_jobstate(ii_log_fehler, this.is_web_message)
...
guoLog.uf_allways("["+ this.classname( )+".of_process_skyscope_refresh] Finish with return code: " + string(li_ret) +" => " + this.sreturn_error_message_short)
if li_ret <> 3 then of_log_jobstate(li_ret, 'Job finished' )
...
// --------------------------------------------------------------------------------------------------------------------
// 06.05.2025 HR: request #11290 New dynamic loading settings on loading lists on CBASEclassic
// --------------------------------------------------------------------------------------------------------------------
idw_loading = create uo_generate_datastore
idw_loading.dataobject="dw_change_cen_out_loading"
idw_loading.settransobject(SQLCA)

idw_OldLoading = create uo_generate_datastore
idw_OldLoading.dataobject="dw_change_cen_out_loading"
idw_OldLoading.settransobject(SQLCA)
...
destroy idw_loading	 		// 06.05.2025 HR: request #11290 New dynamic loading settings on loading lists on CBASEclassic
destroy idw_OldLoading	 	// 06.05.2025 HR: request #11290 New dynamic loading settings on loading lists on CBASEclassic
