...
public function long of_get_rpt_file_dlv_reports (long al_airline_key, datetime adt_departure, ref string ras_rpt_file)
public function string of_get_pdf_filename (long arg_result_key, long arg_transaction, long arg_number)
public function integer of_save_delivery (long al_result_key, long al_transaction, long al_number, long al_airline_key, datetime adt_departure)
end prototypes
...
* 11.12.2024   2.2         Andrii Melnychenko #10919: Flight Browser - Delivery App (CBASE Classic Part) 
*************************************************************/
Long 		ll_status
...
//#2024.12.11 AM: #10919: Flight Browser - Delivery App (CBASE Classic Part) 
select nairline_key, ddeparture, ntransaction, cunit, nflight_number, csuffix into :this.lAirlineKey, :ldt_departure, :this.lTransaction, :ls_unit, :ll_flight_number, :ls_csuffix from cen_out where nresult_key = :this.lResultKey;
if sqlca.sqlcode <> 0 then
	this.iStatus 	= -1
...
select count(1)
 into  :li_i
 from cen_airline_dlv_flights t
  where t.nairline_key = :this.lAirlineKey
     and :ll_flight_number between t.nflgnr_from and t.nflgnr_to
     and :ldt_departure between t.dvalid_from and t.dvalid_to;
	  
if sqlca.sqlcode <> 0 then
	this.iStatus 	= -1
	this.sError		= "Error getting Airline Delivery ~r~r Errorcode: " + string(sqlca.sqlcode) + "~r~r" + sqlca.sqlerrtext
end if	
...
if  li_i >= 1  then
	...
	from cen_out_delivery_note
	     where nresult_key = :this.lResultKey
			and ntransaction = :this.lTransaction
			and nnumber = :this.lBillingKey;		
...
//#2024.12.11 AM: #10919: Flight Browser - Delivery App (CBASE Classic Part)  - New Logic
if  li_i >= 1  then
		//////////////////////////////////////////
		...
		if  len(lblb_blob) >= 0  or not IsNull(lblb_blob)  then
			
			if this.iStatus = 0 then
				ls_PDF_File = is_filepath + of_get_pdf_filename( this.lResultKey,  this.lTransaction, this.lBillingKey)
				f_blob_to_file(ls_PDF_File, lblb_blob)
			end if
			
		else
				
			this.iStatus = of_save_delivery(this.lResultKey, this.lTransaction, this.lBillingKey, this.lAirlineKey, ldt_departure)
				
		end if

//#2024.12.11 AM: #10919: Flight Browser - Delivery App (CBASE Classic Part)  - Old Logic
else
...
// Return: 
// --------------------------------------------------------------------------------
//  Beschreibung: aus w_instruction_center.wf_cr_unload_reports()
//                       
// 
// --------------------------------------------------------------------------------
//  Modifikationen:
//  Datum         Version      Autor              Kommentar
// --------------------------------------------------------------------------------
//  2014-06-27     MHO		   Erstellung
// --------------------------------------------------------------------------------

long ll_Return
string ls_Crystal_Report_File, ls_TmpPath, ls_Crystal_Report_FileNameBody
blob bBlob

ls_TmpPath = f_gettemppath()

ls_Crystal_Report_FileNameBody = "CBASE-CR_CUS_REP_" + String(al_Airline_Key) +  "_" + String(Rand(32767)) + "_" + String(now(), "hhmmss") 
ls_Crystal_Report_File = ls_TmpPath + ls_Crystal_Report_FileNameBody + ".RPT"  

SELECTBLOB breport 
		INTO	:bBlob
  from cen_custom_bbills_app t
WHERE NAIRLINE_KEY = :al_Airline_Key		
	and :adt_departure between dvalid_from and dvalid_to
	and NACTIVE = 1
 USING SQLCA;

if sqlca.sqlcode = 0 then
	FileDelete(ls_Crystal_Report_File)
	f_blob_to_file(ls_Crystal_Report_File, bBlob)
	ll_Return = 0
else
	ls_Crystal_Report_File = ""
	ll_Return = -1
	guoLog.uf_error("["+ this.classname( )+".of_get_rpt_file_dlv_reports] No active Report found for Airlinekey " + string(al_airline_key)+" and departure " + string(adt_departure, "dd.mm.yyyy"))
end if

ras_rpt_file = ls_Crystal_Report_File

return ll_Return
end function

public function string of_get_pdf_filename (long arg_result_key, long arg_transaction, long arg_number);string ls_ret, ls_alc, ls_suffix
integer li_flgnr
datetime ld_dat

//ls_ret = "DeliveryNote_"
...
//        :ld_dat
//    FROM cen_out   
//	 where cen_out.nresult_key = :arg_result_key;
//ls_ret += ls_alc + string(li_flgnr, "000")+trim(ls_suffix)
//ls_ret += "_" + string(ld_dat, "YYYY-MM-DD")
//ls_ret += "_" + string(arg_transaction)
//ls_ret += "_" + string(arg_number)
//ls_ret += ".PDF"

//#2025.03.03 AM: #11499: Delivery Note Report - Naming Convention
ls_ret = ""

 select to_char(t.cairline) || trim(to_char(t.nflight_number,
                                           (case
                                             when t.nflight_number < 100 then
                                              '099'
                                             else
                                              '9999'
                                           end))) || '_' || 
       t.cairline_owner||t.cactype || '_' ||
       t.ctlc_from || '_' || 
       to_char(t.ddeparture, 'dd-mm-yyyy') || '_' ||
       to_char(d.nnumber)  
into :ls_ret
 from cen_out  t, cen_out_delivery_note d
where t.nresult_key = d.nresult_key
and t.ntransaction = d.ntransaction
and t.nresult_key = :arg_result_key;
...
IF IsNull(ls_ret) OR Trim(ls_ret) = "" THEN
	...
	//#2025.06.24 AM: #12306 DDN Report - Naming Convention
	 select to_char(t.cairline) || trim(to_char(t.nflight_number,
															 (case
																when t.nflight_number < 100 then
																 '099'
			 ...
			 to_char(d.nnumber)  
	into :ls_ret
	 from cen_out_history  t, cen_out_delivery_note d
	where t.nresult_key = d.nresult_key
	and t.ntransaction = d.ntransaction
	and t.nresult_key = :arg_result_key;	
...
// --------------------------------------------------------------------------------
integer li_Return
string ls_RPT, ls_Temp_Dir, ls_random_number
string ls_File_Name, ls_File_Fullpath
...
if li_Return = 0 then
	// -------------------------------------------------
	// Report Template File entladen
	// -------------------------------------------------
	li_Return = of_get_rpt_file_dlv_reports(al_airline_key, adt_departure, ls_RPT)

	if li_Return < 0 then
		this.iStatus 	= -1
		this.sError = "No RPT File defined"
	end if
end if

if li_Return = 0 then
	// -------------------------------------------------
	// Report Template File $$HEX1$$f600$$ENDHEX$$ffnen/laden
	// -------------------------------------------------
	li_Return = luo_CR.of_load(ls_RPT)
	if li_Return < 0 then
		this.iStatus 	= -1
		this.sError =  "Error Load RPT File"
	else
		FileDelete(ls_RPT)
	end if
end if
...
if li_Return = 0 then
	ls_Temp_Dir = is_filepath

	// Alle Ausgabedateinamen bekommen die gleiche Random-Nummer
	// und werden zus$$HEX1$$e400$$ENDHEX$$tzlich mit der laufwariablen der Schleife $$HEX1$$fc00$$ENDHEX$$ber die Details
	// markiert
	ls_random_number = String(Rand(32767))
	...
	if f_mandant_profilestring(sqlca, s_app.smandant, "PaperFormat", "UseLetterFormat", "0") = "1" then
		luo_CR.of_set_paper_format(luo_CR.PAPER_SIZE_LETTER)
	else
		luo_CR.of_set_paper_format(luo_CR.PAPER_SIZE_DIN_A4)
	end if

	luo_CR.of_set_parameter("NNumber",al_number)				
	luo_CR.of_set_parameter("Resultkey",al_result_key)
	luo_CR.of_set_parameter("Transaction",al_transaction)

	// Zieldateiname ...
	ls_File_Name = of_get_pdf_filename( al_result_key, al_transaction, al_number)
	ls_File_Fullpath = ls_Temp_Dir + ls_File_Name

		// Exportieren ...
		// MHO 2014-07-03: Gepr$$HEX1$$fc00$$ENDHEX$$ft: ob export sauber mit neuen Parametern retrieved wird oder ob ein of_retrieve() vorweggesetzt werden muss,
		// weil RPT-File nur einmal geladen werden sollte
		// Gem$$HEX3$$e400df002000$$ENDHEX$$DB Doku f$$HEX1$$fc00$$ENDHEX$$hrt ein export implizit ein retrieve aus. Also nur einmal Laden n$$HEX1$$f600$$ENDHEX$$tig!
	li_Return = luo_CR.of_export_pdf(ls_File_Fullpath)

	if li_Return < 0 then
		this.iStatus 	= -1
		this.sError = "Error Create PDF"	
	end if
