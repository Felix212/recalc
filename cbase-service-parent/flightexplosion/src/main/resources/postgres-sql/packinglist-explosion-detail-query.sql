...
FROM cen_packinglist_detail plsdetail
    ...
    LEFT JOIN (
        ...
        WHERE 
            ...
            AND DATE(:arg_calc_date) - MAKE_INTERVAL(0, 0, 0, pltf.noffset) BETWEEN ppmval.dvalid_from AND ppmval.dvalid_to
            AND :arg_calc_time BETWEEN prodtf.ctimefrom AND prodtf.ctimeto
            AND prodtf.nairline_key = :arg_airline_key 
            AND pltfvar.cfrequency LIKE '%' || TO_CHAR(DATE(:arg_calc_date) - MAKE_INTERVAL(0, 0, 0, :arg_iso_offset - pltf.noffset), 'D') || '%'
    ...
    LEFT JOIN (
        ...
        WHERE DATE(:arg_calc_date) - MAKE_INTERVAL(0, 0, 0, pltfflight.noffset) BETWEEN ppmval.dvalid_from AND ppmval.dvalid_to
            AND pltfflight.nfreq = CAST(TO_CHAR(DATE(co.ddeparture) - MAKE_INTERVAL(0, 0, 0, :arg_iso_offset), 'D') AS INTEGER)
